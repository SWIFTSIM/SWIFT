"""
IC making file for idealised clusters,
Script written by Folkert Nobels -
Edited by Filip Husko 2023,
...
def calculate_pressure(r, T, circ):
    ...
    for i in range(1, len(r)):
        r_mean = 10 ** ((np.log10(r[i]) + np.log10(r[i - 1])) / 2.0)
...
if galaxy == "H12":
    ...
    minimum_temperature = True
    Tscale_length = 30.0  # kpc
    gas_mass_used = 1.6218e11  # -8e12*0.015
    if cc == 0:
        cc = 7.2
elif galaxy == "H13.5":
    M200 = 3e13
...
Rs = R200 / cc
bb = 2.0 / cc**2 * (np.log(1 + cc) - cc / (1 + cc))
a_hernquist = (bb + bb**0.5) / (1 - bb) * R200
...
print(
    f"particle_mass at R200: {particle_mass_function(np.array([R200]), adaptive_mass_radius)}"
)
print("-----------")
print(f"Model used:")
...
results_R500_old = root_scalar(
    rho_NFW_minus_rhox, args=(Rs, M200, cc, rho_500), method="bisect", bracket=[1, R200]
)
results_R1000_old = root_scalar(
    rho_NFW_minus_rhox,
    args=(Rs, M200, cc, 2 * rho_500),
    method="bisect",
    bracket=[1, R200],
)

results_R500 = root_scalar(
    average_rho_NFW_minus_rhox,
    args=(Rs, M200, cc, rho_500),
    method="bisect",
    bracket=[1, R200],
)
results_R1000 = root_scalar(
    average_rho_NFW_minus_rhox,
    args=(Rs, M200, cc, 2 * rho_500),
    method="bisect",
    bracket=[1, R200],
...
results_R500 = root_scalar(
    average_rho_hern_minus_rhox,
    args=(a_hernquist, Mdm_hern, rho_500),
    method="bisect",
    bracket=[1, R200],
)
results_R1000 = root_scalar(
    average_rho_hern_minus_rhox,
    args=(a_hernquist, Mdm_hern, 2 * rho_500),
    method="bisect",
    bracket=[1, R200],
)
...
hydrostatic_gas_fraction_hern = (
    m500_gas_fraction_hern + amount_of_sigma_used * m500_gas_fraction_1std
)
mgas_mass_guess_hern = (
    m500_gas_fraction_hern + amount_of_sigma_used * m500_gas_fraction_1std
) * M500_hern

print(f"The hydrostatic gas fraction at R500: {hydrostatic_gas_fraction:1.4f}")
print(
    f"The hydrostatic gas fraction at R500 (Hernquist case): {hydrostatic_gas_fraction_hern:1.4f}"
)
...
coords_stars_int[:, 0] = Coordinates[:, 0] + boxsize[0] / 2.0
coords_stars_int[:, 1] = Coordinates[:, 1] + boxsize[1] / 2.0
coords_stars_int[:, 2] = Coordinates[:, 2] + boxsize[2] / 2.0
...
np.savetxt(
    f"temp_vs_circ_v_{Tmin_model:1.4e}.txt", (T_profile, circ_v_tot), fmt="%1.6e"
)
...
dr = np.diff(r_array)
r_mean = (r_array[1:] + r_array[:-1]) / 2.0
rho_mean = (rho[1:] + rho[:-1]) / 2.0
...
# get random radii
r_values_guess_inner = np.random.uniform(
    r_array[0], adaptive_mass_radius, array_length_inner
)
...
# get random radii
r_values_guess_outer = np.random.uniform(
    adaptive_mass_radius, r_array[-1], array_length_outer
)
...
outer_particle_mass = (
    particle_mass_function(r_values_using_outer, adaptive_mass_radius) * Mres
)
...
print(
    f"Realized gas mass within R500: {np.sum(final_mass_particles[final_radius<R500]):1.4e} Msun"
)
...
x_gas += boxsize[0] / 2.0  # boxsize[0]/2.
y_gas += boxsize[1] / 2.0  # boxsize[1]/2.
z_gas += boxsize[2] / 2.0  # boxsize[2]/2.
...
smoothing_lengths = (
    sw.visualisation.smoothing_length.generate.generate_smoothing_lengths(
...
coords = np.zeros((N, 3))
coords[:, 0] = x_gas[selection_gas]
coords[:, 1] = y_gas[selection_gas]
coords[:, 2] = z_gas[selection_gas]

ds = grp.create_dataset("Coordinates", (N, 3), coord_dtype)
ds[()] = coords

v = np.zeros((N, 3))
v[:, 0] = velocity_x[selection_gas] * 1e-5
v[:, 1] = velocity_y[selection_gas] * 1e-5
v[:, 2] = velocity_z[selection_gas] * 1e-5

ds = grp.create_dataset("Velocities", (N, 3), vels_dtype)
ds[()] = v

ds = grp.create_dataset("Masses", (N,), m_dtype)
ds[()] = final_mass_particles[selection_gas] / 1e10

ds = grp.create_dataset("SmoothingLength", (N,), h_dtype)
ds[()] = np.array(smoothing_lengths.value)

ds = grp.create_dataset("InternalEnergy", (N,), u_dtype)
ds[()] = internal_energy[selection_gas] * 1e-10
N_particles_outer
N_particles_inner
...
if "/PartType5" not in f:
    ...
    # Add the black hole
    bh_coordinates = boxsize / 2.0 + 0.001 * unyt.kpc

    # default spin, can be changed
    spin = 0.5
    del f["/PartType5"]
    ...
    bh_coordinates = (
        np.array([boxsize[0] / 2.0, boxsize[1] / 2.0, boxsize[2] / 2.0]) + 0.001
    )
    ds[()] = bh_coordinates
