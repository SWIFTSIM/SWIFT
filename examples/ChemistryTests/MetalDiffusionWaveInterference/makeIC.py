...
# Get the particles in a sphere of radius epsilon centered on each peak
if dimension == 1:
    r_1 = np.abs(pos[:, 0] - L/4)
    r_2 = np.abs(pos[:, 0] - 3*L/4)
elif dimension == 2:
    print("not implemented yet")
else:
    peak_pos_1 = np.array([L/4, L/2, L/2])
    r_1 = np.linalg.norm(pos-peak_pos_1, axis=1)
    peak_pos_2 = np.array([3*L/4, L/2, L/2])
    r_2 = np.linalg.norm(pos-peak_pos_2, axis=1)
