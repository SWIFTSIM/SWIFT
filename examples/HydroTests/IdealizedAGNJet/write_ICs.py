...
box_size = 500. # Make sure this is consistent with parameter file
...
ambient_medium_temperature = 10**7 # Typical of cool core galaxy clusters (of halo mass 10**15 Msun)
...
opening_angle = 10. # in degrees
cos_opening_angle = np.cos(opening_angle / 180. * np.pi)
...
T_jet = 0.10228 # 100 Myr in internal units. Make sure it is consistent with parameter file
E_jet = P_jet * T_jet
...
N_ambient_medium = int(np.floor(ambient_medium_density * box_size * (box_size * box_aspect_ratio)**2 / m_part))
...
N_z = int(np.floor(N_ambient_medium**(1./3.) *1 / box_aspect_ratio**(2./3.))) # This isn't a simple cube root of N_ambient_medium because we have a rectangular box

# Shorter two sides
N_xy = int(np.floor(N_z * box_aspect_ratio))

# Create the actual lattice
x_grid = np.linspace(-(1. - 1. / (1. + N_xy)) * box_size * 0.5 * box_aspect_ratio, (1. - 1. / (1. + N_xy)) * box_size * 0.5 * box_aspect_ratio, N_xy)
y_grid = np.linspace(-(1. - 1. / (1. + N_xy)) * box_size * 0.5 * box_aspect_ratio, (1. - 1. / (1. + N_xy)) * box_size * 0.5 * box_aspect_ratio, N_xy)
z_grid = np.linspace(-(1. - 1. / (1. + N_z)) * box_size * 0.5, (1. - 1. / (1. + N_z)) * box_size * 0.5, N_z)
...
N_cone_box = 6 / np.pi * 1. / (1. - cos_opening_angle) * N_jet_target * 1.1

# Side length of the necessary cube
N_side = int(np.floor(N_cone_box**(1. / 3.)))
...
if (N_jet_final < N_jet_target):
    print("The final particle number in the cone reservoir is less than the target number. Increase the numerical prefactor in the N_cone_box variabe.")
    exit()
...
# Assign particle IDs
IDs_gas = np.concatenate((np.arange(N_jet_final), 10**7*2 + np.arange(N_ambient_medium)),axis=None)

# Assign masses
masses_gas = np.concatenate((m_part_jet * np.ones(N_jet_final), m_part * np.ones(N_ambient_medium)),axis=None)
...
smoothing_length_ambient_medium = 0.5 * (50 * m_part / (ambient_medium_density))**(1. / 3.)
volume_jet_cone = 4. * np.pi / 3. * R**3 * np.sin(opening_angle)**2
smoothing_length_jet_cone = 0.5 * (50 / (N_jet_final / volume_jet_cone))**(1. / 3.)
smoothing_lengths_gas = np.concatenate((smoothing_length_jet_cone * np.ones(N_jet_final), smoothing_length_ambient_medium * np.ones(N_ambient_medium)),axis=None)
...
with h5.File("ICs.hdf5", "w") as f:
    ...
    write_units(
        ...
        mass=1.98848*10**43., # 10**10 Msun
        temperature=1., # Kelvin
        time=3.156*10**16. # Gyr
    )
