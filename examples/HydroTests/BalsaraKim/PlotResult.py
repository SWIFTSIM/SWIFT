...
def make_projection(data, key):
    res = project_gas(
        data, resolution=img_res, project=key, parallel=True, periodic=True
    )
    return res
...
for i in range(3):
    ...
    axes[i].tick_params(
        which="both", axis="both", direction="in", color="white", top=True, right=True
    )
...
fig.subplots_adjust(left=0, right=0.9, top=0.95, bottom=0.0, wspace=0, hspace=0)
