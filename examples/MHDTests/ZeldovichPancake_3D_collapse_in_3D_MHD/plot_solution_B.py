...
rho_s_x_upper = (
    rho_0
    / (1.0 - zfac_x * np.cos(k_x * q))
    / (1.0 - zfac_y * np.cos(k_y * q_max))
    / (1.0 - zfac_z * np.cos(k_z * q_max))
)
rho_s_x_lower = (
    rho_0
    / (1.0 - zfac_x * np.cos(k_x * q))
    / (1.0 - zfac_y * np.cos(k_y * q_min))
    / (1.0 - zfac_z * np.cos(k_z * q_min))
)
rho_s_y_upper = (
    rho_0
    / (1.0 - zfac_x * np.cos(k_x * q_max))
    / (1.0 - zfac_y * np.cos(k_y * q))
    / (1.0 - zfac_z * np.cos(k_z * q_max))
)
rho_s_y_lower = (
    rho_0
    / (1.0 - zfac_x * np.cos(k_x * q_min))
    / (1.0 - zfac_y * np.cos(k_y * q))
    / (1.0 - zfac_z * np.cos(k_z * q_min))
)
rho_s_z_upper = (
...
T_s_x_upper = T_i * (
    ((1.0 + redshift) / (1.0 + z_i)) ** 3.0 * rho_s_x_upper / rho_0
) ** (2.0 / 3.0)
T_s_x_lower = T_i * (
    ((1.0 + redshift) / (1.0 + z_i)) ** 3.0 * rho_s_x_lower / rho_0
) ** (2.0 / 3.0)
P_s_x_upper = (
    T_s_x_upper * rho_s_x_upper * unit_rho_in_si * k_in_J_K / mH_in_kg / (a / a_i) ** 3
)  ## rho_s is not SI units! So multiply it
P_s_x_lower = (
    T_s_x_lower * rho_s_x_lower * unit_rho_in_si * k_in_J_K / mH_in_kg / (a / a_i) ** 3
)

# For magnetic field
y_term = (1.0 - (1.0 + z_c_y) / (1.0 + z_i) * np.cos(k_y * q)) / (
    1.0 - (1.0 + z_c_y) / (1.0 + redshift) * np.cos(k_y * q)
)
z_term = (1.0 - (1.0 + z_c_z) / (1.0 + z_i) * np.cos(k_z * q)) / (
    1.0 - (1.0 + z_c_z) / (1.0 + redshift) * np.cos(k_z * q)
)
y_term_max = (1.0 - (1.0 + z_c_y) / (1.0 + z_i) * np.cos(k_y * q_max)) / (
    1.0 - (1.0 + z_c_y) / (1.0 + redshift) * np.cos(k_y * q_max)
)
z_term_max = (1.0 - (1.0 + z_c_z) / (1.0 + z_i) * np.cos(k_z * q_max)) / (
    1.0 - (1.0 + z_c_z) / (1.0 + redshift) * np.cos(k_z * q_max)
)
y_term_min = (1.0 - (1.0 + z_c_y) / (1.0 + z_i) * np.cos(k_y * q_min)) / (
    1.0 - (1.0 + z_c_y) / (1.0 + redshift) * np.cos(k_y * q_min)
)
z_term_min = (1.0 - (1.0 + z_c_z) / (1.0 + z_i) * np.cos(k_z * q_min)) / (
    1.0 - (1.0 + z_c_z) / (1.0 + redshift) * np.cos(k_z * q_min)
)
...
plt.text(
    -0.45,
    0.9,
    "Zeldovich pancake in 3D at z=%.2f " % (redshift),
