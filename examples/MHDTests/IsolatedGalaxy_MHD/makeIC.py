...
def open_snapshot(fileName: str,):
    ...
    data = {
        "PartType0": {"pos": pos, "u": u, "m": m, "ids": ids, "h": h, "vel": vel},
        "Units": {"UI": UI, "UL": UL, "UM": UM, "UT": UT, "UTM": UTM},
        "BoxSize": BoxSize,
...
def add_atmospere_particles(
    ...
    if IAsource == "grid":
        ...
        grid_1d = np.linspace(
            0.0, 1.0, Nside
        )  # Cube side is 1, centered at (0.5,0.5,0.5)
...
def makeIC(fileInputName: str, fileOutputName: str, PrintToVTK: bool = True):
    ...
    # Add magnetic fields
    data = add_magnetic_fields(data, B0_Gaussian_Units=1e-3)  # micro Gauss
    ...
    # parameter dict
    pars = {
        "density": {"profile": "beta", "rho0": rho0, "r0": 0.33, "beta": 2 / 3},
    ...
    write_or_replace_dataset(
        grp, "Coordinates", data["PartType0"]["pos"], size=(Ntot, 3), dtype="d"
    )
    write_or_replace_dataset(
        grp, "Velocities", data["PartType0"]["vel"], size=(Ntot, 3), dtype="f"
    )
    write_or_replace_dataset(
        grp, "InternalEnergy", data["PartType0"]["u"], size=(Ntot,), dtype="f"
    )
    write_or_replace_dataset(
        grp, "SmoothingLength", data["PartType0"]["h"], size=(Ntot,), dtype="f"
    )
    write_or_replace_dataset(
        grp, "ParticleIDs", data["PartType0"]["ids"], size=(Ntot,), dtype="L"
    )
    write_or_replace_dataset(
        grp, "Masses", data["PartType0"]["m"], size=(Ntot,), dtype="f"
    )
    write_or_replace_dataset(
        grp, "MagneticFluxDensities", data["PartType0"]["B"], size=(Ntot, 3), dtype="f"
    )
    write_or_replace_dataset(
        grp,
        "MagneticVectorPotentials",
        data["PartType0"]["A"],
        size=(Ntot, 3),
        dtype="f",
    )
