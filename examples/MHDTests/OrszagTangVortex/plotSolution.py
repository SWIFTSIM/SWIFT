...
fig, ax = plt.subplots(3,2,figsize=(12.25,17)) #, layout='constrained')
...
a01 = ax[0,1].contourf(
    pressure_map.value.T,
    cmap="gist_heat",
    levels=np.linspace(0.0, 0.4, 100),
)
a10 = ax[1,0].contourf(
...
a20 = ax[2,0].contourf(
    error_map.value.T, cmap="jet", levels=np.linspace(-5.0, 0.0, 6)
)

# Add panel with infromation about the run 
text_common_args = dict(fontsize=10, ha='center', va='center', transform=ax[2,1].transAxes)
...
ax[2,1].text(0.5, 0.4, kernel.decode("utf-8") + " with $%.2f$ neighbours" % (neighbours), **text_common_args)
ax[2,1].text(0.5, 0.3, "Artificial diffusion: $%.2f$ " % (artDiffusion), **text_common_args)
ax[2,1].text(0.5, 0.2, "Dedner Hyp, Hyp_div(v), Par: $%.2f,%.2f,%.2f$ " % (dedHyp,dedHypDivv,dedPar), **text_common_args)
ax[2,1].text(0.5, 0.1, "Physical resistivity $\eta$: $%.2f$ " % (eta), **text_common_args)
ax[2,1].axis('off')
...
fig.colorbar(
    a00, ax=ax[0,0], label=r"$\rho$", fraction=0.042, pad=0.04, location='left', ticks=np.linspace(0.0,0.5,6)
)

fig.colorbar(a01, ax=ax[0,1], label=r"$P$", fraction=0.042, pad=0.04, ticks=np.linspace(0.0,0.4,5))

fig.colorbar(
    a10, ax=ax[1,0], label=r"$|\mathbf{v}|$", fraction=0.042, pad=0.04, location='left', ticks=np.linspace(0.0,1.5,7)
)

fig.colorbar(a11, ax=ax[1,1], label=r"$|\mathbf{B}|$", fraction=0.042, pad=0.04, ticks=np.linspace(0.0,0.75,6))

fig.colorbar(a20, ax=ax[2,0], label=r"$\mathrm{log}_{10} \frac{h \: \nabla \cdot B}{|B|}$", fraction=0.042, pad=0.04, location='left')
