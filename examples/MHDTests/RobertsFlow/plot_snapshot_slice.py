...
def prepare_sliced_quantity(quantity, isvec=False):
    if isvec:
        ...
        sliced_quantity[:, 0] = (
            make_slice("mass_weighted_temp_qx").value.flatten() / mass_map
        )
        sliced_quantity[:, 1] = (
            make_slice("mass_weighted_temp_qy").value.flatten() / mass_map
        )
        sliced_quantity[:, 2] = (
            make_slice("mass_weighted_temp_qz").value.flatten() / mass_map
        )
    else:
...
new_x = np.linspace(
    0.0, data.metadata.boxsize.value[0], dimx
)  # np.linspace(np.min(x), np.max(x), dimx)
new_y = np.linspace(
    0.0, data.metadata.boxsize.value[1], dimy
)  # np.linspace(np.min(y), np.max(y), dimy)
...
def make_density_plot(
    ...
    if draw_cbar:
        fig.colorbar(
            to_plot, ax=ax.ravel().tolist(), ticks=ticks_to_plot, location="left"
        )
