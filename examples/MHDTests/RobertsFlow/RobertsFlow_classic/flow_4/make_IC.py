...
v[:, 2] = np.sin(kv0 * (pos[:, 0] + pos[:, 1]))

# Note that the average rms velocity of such field configuration is 1.224!
# vv = np.sqrt(np.mean(v[:,0]**2+v[:,1]**2+v[:,2]**2)) #, therefore rms in physical field will be Vrms=1.224*V0!
