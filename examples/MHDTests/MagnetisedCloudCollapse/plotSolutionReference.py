...
Rcloud = 4.628516371e16 * unyt.cm
M = 1.99e33 * unyt.g  # total mass of the sphere
rhocloud0 = M/(4/3 * np.pi * Rcloud**3 )
rhouniform = rhocloud0/360
t_ff_lit = np.sqrt(3/(2*np.pi*G*rhocloud0)) * unyt.s
t_ff_wiki = np.sqrt(3*np.pi/(32*G*rhocloud0)) * unyt.s
tsim = data.metadata.time
toplot = tsim / t_ff_lit
print("Showing results at %2f free fall times" % toplot)
toplot = tsim / t_ff_wiki
print("Showing results at %2f free fall times" % toplot)
...
tsim = data.metadata.time
tplot = tsim / tff
print(f'from script t_ff = {tff}')
print("Showing results at %3f free fall times" % tplot)
...
print(normB.to(1e-7*unyt.g / (unyt.s**2 * unyt.A)))
...
va2 = normB**2/(mu0*rho)
vsignal = np.sqrt(cs2+va2)

# print id of a particle with largest mf
idx = np.argmax(normB)
print(f'Largest MF for particle {idx}, at x,y,z = {r[idx]/R0}  B={normB[idx]}, r/R0={norm_r[idx]/R0}, vr={vr[idx]}')
...
mass_weighted_density_zoom_map = slice_gas(
    **common_arguments_zoom, project="mass_weighted_densities"
)

mass_weighted_error_map = slice_gas(**common_arguments, project="mass_weighted_error")

mass_weighted_error_zoom_map = slice_gas(**common_arguments_zoom, project="mass_weighted_error")
...
mass_weighted_Bz_zoom_map = slice_gas(
    **common_arguments_zoom, project="mass_weighted_Bz"
)

mass_weighted_Btheta_zoom_map = slice_gas(
    **common_arguments_zoom, project="mass_weighted_Btheta"
)
...
density_zoom_map = mass_weighted_density_zoom_map / mass_zoom_map
error_map = mass_weighted_error_map / mass_map
error_zoom_map = mass_weighted_error_zoom_map / mass_zoom_map
...
Btheta_zoom_map =  mass_weighted_Btheta_zoom_map / mass_zoom_map
Bz_zoom_map = mass_weighted_Bz_zoom_map / mass_zoom_map

density_map_xy.convert_to_units(unyt.g * unyt.cm ** (-3))
density_map.convert_to_units(unyt.g * unyt.cm ** (-3))
density_zoom_map.convert_to_units(unyt.g * unyt.cm ** (-3))
vr_map.convert_to_units(unyt.km / unyt.s)
vr_zoom_map.convert_to_units(unyt.km / unyt.s)

Btheta_map.convert_to_units(1e-7 * unyt.g / (unyt.A * unyt.s * unyt.s))
Bz_map.convert_to_units(1e-7 * unyt.g / (unyt.A * unyt.s * unyt.s))
Bz_zoom_map.convert_to_units(1e-7 * unyt.g / (unyt.A * unyt.s * unyt.s))
Btheta_zoom_map.convert_to_units(1e-7 * unyt.g / (unyt.A * unyt.s * unyt.s))
...
fig.suptitle("Plotting at %.3f free fall times" % tplot)
...
a20 = ax[2, 0].contourf(
    np.log10(np.maximum(J_map.value, -15)),# 1.0
    cmap='nipy_spectral',#"plasma",
    extend="both",
    levels=np.linspace(-15, -11, 100), # 0.0
)
a21 = ax[2, 1].contourf(
    np.log10(np.maximum(Bz_map.value, -6.9)), # 1.0
    cmap="nipy_spectral_r", # jet
    extend="both",
    levels=np.linspace(2.0, 5.0, 100), #1.0, 4.5, 100
)
...
#ax[1,1].set_xticks(locs,[-0.01, 0.0, 0.01])
...
#cbar4 = plt.colorbar(
#    a11, ax=ax[1, 1], fraction=0.046, pad=0.04, ticks=np.linspace(-1.0, 1.0, 9)
cbar4.set_label(r"$\mathrm{log}_{10} \frac{h \: \nabla \cdot B}{|B|}$")
#cbar4.set_label(r"$\mathrm{log}_{10} v_{signal} \; [km s^{-1}]$")
#cbar4.set_label(r"$\psi \;$")
cbar5 = plt.colorbar(
    a20, ax=ax[2, 0], fraction=0.046, pad=0.04, ticks=np.linspace(-15, -11, 5)
)
#cbar5.set_label(r"$\psi \;$")
