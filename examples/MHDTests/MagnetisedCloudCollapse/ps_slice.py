...
visualise_region = [
   center[0] - 0.15*R0, center[0] + 0.15*R0,
   center[2] - 0.15*R0, center[2] + 0.15*R0
]

visualise_region_zoom = [
   center[0] - 0.015*R0, center[0] + 0.015*R0,
   center[2] - 0.015*R0, center[2] + 0.015*R0
]
...
mass_zoom_map = slice_gas(
    **common_arguments_zoom,
    project="masses",
)
...
mass_weighted_vr_map = slice_gas(
    **common_arguments,
    project="mass_weighted_vr",
)
...
mass_weighted_Btheta_map = slice_gas(
    **common_arguments,
    project="mass_weighted_Btheta",
)

mass_weighted_Bz_map = slice_gas(
    **common_arguments,
    project="mass_weighted_Bz",
)
...
cbar1 = plt.colorbar(a00, ax=ax[0,0], fraction=0.046, pad=0.04, ticks=np.linspace(-17, -11, 7))
...
cbar3 = plt.colorbar(a10, ax=ax[1,0], fraction=0.046, pad=0.04, ticks=np.linspace(-1.5, 2.5, 9))
cbar3.set_label(r'$v_r \; [km s^{-1}$')
cbar4 = plt.colorbar(a11, ax=ax[1,1], fraction=0.046, pad=0.04, ticks=np.linspace(-1.5, 2.5, 9))
cbar4.set_label(r'$v_r \; [km s^{-1}$')
cbar5 = plt.colorbar(a20, ax=ax[2,0], fraction=0.046, pad=0.04, ticks=np.linspace(0.0, 5.0, 6))
cbar5.set_label(r'$\mathrm{log}_{10} B_\theta \; [\mu G]$')
cbar6= plt.colorbar(a21, ax=ax[2,1], fraction=0.046,  pad=0.04, ticks=np.linspace(1.0, 4.5, 8))    
