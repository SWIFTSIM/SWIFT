...
data.gas.mass_weighted_error = data.gas.masses * np.maximum(
    h * abs(divB) / (normB + 0.01 * np.max(normB)), 1e-6
)
...
def compute_power_spectrum_vec(Qx, Qy, Qz, dx, nbins):
    ...
    # Power in k-space
    Pk = (
        np.abs(Qx_k) ** 2 + np.abs(Qy_k) ** 2 + np.abs(Qz_k) ** 2
    ...
    power_spectrum = np.where(
        counts > 0, power_spectrum / counts, 0 * power_spectrum.units
    )
    power_spectrum_div = np.where(
        counts > 0, power_spectrum_div / counts, 0 * power_spectrum_div.units
    )
...
ks, Eb, Eb_div = compute_power_spectrum_vec(
    Bx_cube, By_cube, Bz_cube, dx=dx, nbins=res - 1
)
...
# plot spectral lines
ksmock = (
    np.logspace(np.log10(ksoft.value) - 1, np.log10(ksoft.value) - 0.5, 10)
    * ksoft.units
)
p = -5 / 3
...
# plot spectral lines
ksmock = (
    np.logspace(np.log10(k_slice.value), np.log10(k_slice.value) + 0.5, 10)
    * k_slice.units
)
p = 3 / 2
...
ax.set_ylabel(
    r"$\mathrm{P}(\mathrm{k})$ $[\mathrm{erg}\cdot\mathrm{kpc}]$", fontsize=30
)
