...
Eb.convert_to_units(unyt.erg*(unyt.cm))
Ev.convert_to_units(unyt.erg*(unyt.cm))
ks.convert_to_units(1/unyt.cm)
...
# plot spectral lines
ksmock = np.logspace(np.log10(k_res.value)-0.5,np.log10(k_res.value)-0.25,10)*k_res.units
p = -5/3
Eright = 5e2 * unyt.erg*(unyt.cm)
kright = k_res
kright.convert_to_units(1/unyt.cm)
Emock = Eright*(ksmock/kright)**(p)
Emock.convert_to_units(unyt.erg*(unyt.cm))
ax.plot(ksmock,Emock,color='black',linestyle='dashed')#,label = '$k^{-5/3}$')

klabel = k_res/10**0.15
klabel.convert_to_units(1/unyt.cm)
Elabel = Eright*(klabel/kright)**(p)
Elabel.convert_to_units(unyt.erg*(unyt.cm))
ax.text(klabel,Elabel,r'$k^{-5/3}$',fontsize=15)

# plot spectral lines
ksmock = np.logspace(np.log10(k_slice.value),np.log10(k_slice.value)+0.25,10)*k_slice.units
p = 3/2
Eleft = 5e-1 * unyt.erg*(unyt.cm)
kleft = k_slice
kleft.convert_to_units(1/unyt.cm)
Emock = Eleft*(ksmock/kleft)**(p)
Emock.convert_to_units(unyt.erg*(unyt.cm))
ax.plot(ksmock,Emock,color='black',linestyle='dashed')#,label = '$k^{-5/3}$')

klabel = k_slice/10**0.1
klabel.convert_to_units(1/unyt.cm)
Elabel = Eleft*(klabel/kleft)**(p)
Elabel.convert_to_units(unyt.erg*(1*unyt.cm))
ax.text(klabel,Elabel,r'$k^{3/2}$',fontsize=15)

ax.set_yscale('log')
ax.set_xscale('log')
ax.set_xlabel(r'$\mathrm{k}$ $[\mathrm{cm}^{-1}]$', fontsize=30)
ax.set_ylabel(r'$\mathrm{P}(\mathrm{k})$ $[\mathrm{erg}\cdot\mathrm{cm}]$', fontsize=30)
