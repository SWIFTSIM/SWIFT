...
# Load snapshot
filename = snap#'./Monopole_0000.hdf5'
data = load(filename)
...
# get typical smoothing length
print(data.gas.smoothing_lengths)
mean_h = np.mean(data.gas.smoothing_lengths.value)
print(mean_h)




k_diff_fact = 0.048/mean_h#0.0525
ax[0].plot(t,k_diff_fact*divB_err, label = 'SWIFT (rescaled)', color='black')
...
ax[0].set_xlabel(r'$time$',fontsize=15)
ax[0].set_ylabel(r'$ h |\nabla \cdot B|/ B $',fontsize=15)
