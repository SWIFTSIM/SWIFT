...
magnetic_energy = data[34] * energy_units
total_energy = kinetic_energy + thermal_energy + potential_energy + magnetic_energy
...
ax.plot(time.to(Gyr), (kinetic_energy).to(erg), label=r"$\epsilon_{kinetic}$")

ax.plot(
    time.to(Gyr), (thermal_energy - total_background_energy).to(erg), label=r"$\epsilon_{thermal}$"
)

ax.plot(time.to(Gyr), (radiative_cool).to(erg), label=r"$\epsilon_{cooling-losses}$")

ax.plot(time.to(Gyr), (magnetic_energy).to(erg), label=r"$\epsilon_{magnetic}$")

#ax.set_xlim(0, 0.05 * Gyr)
ax.set_xlim(0, 0.3 * Gyr)

ax.set_xlabel(r"Time [Gyr]")
ax.set_ylabel(r"$\epsilon_i$ [erg]")

ax.legend(loc='best')

ax_twin = ax.twinx()

ax_twin.plot(time.to(Gyr), np.log10(divB_err), 'k--')

ax_twin.set_ylim(-5.0,-1.0)

ax_twin.set_ylabel(r"$h \, \nabla \cdot B / |B|$") 
