...
a00 = ax[0, 0].contourf(
    density_map.value.T, cmap="gist_heat", levels=np.linspace(0.0, 2.5, 100)
)
a01 = ax[0, 1].contourf(
    pressure_map.value.T, cmap="gist_heat", levels=np.linspace(0.0, 1.0, 100)
)
a10 = ax[1, 0].contourf(
    normv_map.value.T, cmap="gist_heat", levels=np.linspace(0.0, 1.0, 100)
)
a11 = ax[1, 1].contourf(
    normB_map.value.T, cmap="gist_heat", levels=np.linspace(0.5, 1.75, 100)
)
...
ax[2, 1].text(
    0.5,
    0.8,
    "Magnetic Blast Wave at time $t=%.2f$" % data.metadata.time,
    **text_common_args,
)
