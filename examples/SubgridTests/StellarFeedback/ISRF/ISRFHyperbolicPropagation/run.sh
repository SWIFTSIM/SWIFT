...
time_end=${time_end:=3.1e-3} #TimeIntegration:time_end override (internal units)
dt_max=${dt_max:=1e-5} #TimeIntegration:dt_max override (internal units)
delta_time=${delta_time:=1e-4} #Snapshots:delta_time override (internal units)
initial_metallicity=${initial_metallicity:=1} #GEARChemistry:initial_metallicity override (Z/Zsun)
run_name=${run_name:=""}

# Remove the ICs
if [ -e ICs_isrf_hyperbolic_propagation.hdf5 ]
then
    rm ICs_isrf_hyperbolic_propagation.hdf5
fi
...
ic_output=$(python3 makeIC.py --level $level --rho $gas_density \
	...
	--star_type $star_type \
    -o ICs_isrf_hyperbolic_propagation.hdf5)
echo "$ic_output"
