...
def extract_stellar_feedback(log_file_path: str):
    ...
    if energy_rate > 0.0 or mass_loss > 0.0:
        print(f"Extracted Cumulative Feedback: Energy = {energy_rate:.3e} erg/yr, Mass = {mass_loss:.3e} Msol/yr, Star_Lifetime = {GLOBAL_LIFETIME:.3e} Myr")
    else:
        print("Warning: Log file read, but no positive energy or mass ejection rates were found.")
...
def calculate_snowplow_phase(energy_rate, time, mu, wind_velocity, mass_loss):
    ...
    R_e = 0.88 * (energy_rate.value / mu / GLOBAL_ISM_DENSITY / mH_in_g)**(1/5) * (time.value)**(3/5)
    R_e *= cm_to_kpc
    R_time_end = 0.88 * (energy_rate.value / mu / GLOBAL_ISM_DENSITY / mH_in_g)**(1/5) * (GLOBAL_LIFETIME * 10**6 * yr_to_s)**(3/5) * cm_to_kpc
    R_p = ((3 / 2 / np.pi * mass_loss * wind_velocity / mu / GLOBAL_ISM_DENSITY / mH_in_g)**(1/4) * (time.value)**(1/2) * cm_to_kpc).value# + R_time_end

    # Formula from the book "Introduction to Stellar Winds-Cambridge University Press (1999)"
    R_b_e = (28 * (L36 / mu / GLOBAL_ISM_DENSITY )**(1/5) * t6.value**(3/5)).value / 1000 
    R_b_time_end = (28 * (L36 / mu / GLOBAL_ISM_DENSITY)**(1/5) * (GLOBAL_LIFETIME * 10**6 * yr_to_s)**(3/5)).value * cm_to_kpc
    R_b_p = (5.0 * (L36 * v3 / GLOBAL_ISM_DENSITY)**(1/4) * (t6.value)**(1/2)).value / 1000 #+ R_b_time_end 

    v_e = (17 * (L36 / mu / GLOBAL_ISM_DENSITY)**(1/5) * t6.value**(-2/5)).value
    v_time_end = (17 * (L36 / mu / GLOBAL_ISM_DENSITY)**(1/5) * (GLOBAL_LIFETIME * 10**6 * yr_to_s)**(-2/5)).value
    v_p = (7.9 * (L36 / v3 / GLOBAL_ISM_DENSITY)**(1/4) * (t6.value)**(-1/2)).value # + v_time_end

    T = np.log10(1.6e6 * L36**(8/35) * GLOBAL_ISM_DENSITY**(2/35) * t6.value**(-6/35))

    n = np.log10(0.01 * L36**(6/35) * GLOBAL_ISM_DENSITY**(19/35) * t6.value**(-22/35))
...
def update_plot_scatter(frame_index):
    ...
    if time > GLOBAL_LIFETIME and GLOBAL_LIFETIME != 0.:
        GLOBAL_TEXT.set_visible(True)

    GLOBAL_SUPERTITLE.set_text(f"Time: {time.value:.4f} Myr")

    return [line for line in GLOBAL_LINES.values()] + [GLOBAL_SUPERTITLE] + [GLOBAL_TEXT]


def write_quantities(coordinates, velocities, densities, temperatures, mu, frame_index, time, energy_rate, wind_velocity, mass_loss):
    """
    ...
    idx_rho_max = np.argmin(np.where(densities == rho_max, r_val, np.inf))  # In case of multiple maxima, take the smallest radius, because at the begining the all have the same values 
    ...
    # theoretical part
    R_e,R_p,R_b_e,R_b_p,v_e,v_p,T_e,n_e, = calculate_snowplow_phase(energy_rate,time,np.mean(mu), wind_velocity, mass_loss)
...
def main(file_list,log_path): 
    ...
    print(f"Loading files and getting graph limits from {len(file_list)} potential files...")

    # Initialize Limits
    R_MAX = 0.0
    Y_MIN_MAX = {"velocity": [np.inf, -np.inf], "density": [np.inf, -np.inf], "temperature": [np.inf, -np.inf]}
    ...
    print(f"Get Energy rate: {energy_rate}, Mass-Loss: {mass_loss}, and calculated wind velocity: {wind_velocity:.3f}")
    ...
    for file_path in tqdm(file_list, desc="Limit and Data Loading"):
        ...
        if data_tuple is not None:
            ...
            Y_MIN_MAX["temperature"][0] = min(Y_MIN_MAX["temperature"][0], temperatures.min())
            Y_MIN_MAX["temperature"][1] = max(Y_MIN_MAX["temperature"][1], temperatures.max())
    ...
    for i, data_tuple in tqdm(enumerate(GLOBAL_DATA_LIST), desc="Frame Gen/Logging", total=len(GLOBAL_DATA_LIST)):
        r, v_r, densities, temperatures, mu, time = data_tuple 

        write_quantities(r, v_r, densities, temperatures, mu, i, time, energy_rate, wind_velocity, mass_loss)
...
if __name__ == '__main__':
    ...
    if not file_list:
        print(f"Error: No files found matching pattern '{args.pattern}' in directory '{args.data_dir}'.")
