...
def get_gas_mu(data):
    ...
    if cooling in [b"Grackle3", b"Grackle2", b"Grackle1"]:
        ...
        H_frac = float(
            data.metadata.parameters.get("GrackleCooling:HydrogenFractionByMass", 1.0)
        )
        T_mu = (gamma - 1.0) * u.value * mH_in_g.value / k_B.value
        mu = np.where(
            T_mu > 1.1e4 / (4.0 / (8.0 - 5.0 * (1.0 - H_frac))),
            4.0 / (8.0 - 5.0 * (1.0 - H_frac)),
            4.0 / (1.0 + 3.0 * H_frac),
        )
        return mu
...
def extract_feedback(log_path: str):
    ...
    with open(log_path, "r") as f:
        for line in f:
            ...
            mmc = re.search(
                r"Mass_ejected_per_progenitor_mass\[Msol/yr/Msol\]=" + fp, line
            )
            massc = re.search(r"init_mass\[M_odot\]=" + fp, line)
            if me:
                e_rate = float(me.group(1))
            if mec and massc:
                e_rate = float(mec.group(1)) * float(massc.group(1))
            if mm:
                m_rate = float(mm.group(1))
            if mmc and massc:
                m_rate = float(mmc.group(1)) * float(massc.group(1))
...
def write_line(f, r, log_rho, log_T, mu, time, e_rate, v_wind, n0):
    ...
    if t6 > 0 and L36 > 0:
        ...
        R_e = (
            28 * (L36 / mu / n0) ** (1 / 5) * t6 ** (3 / 5)
        )  # Radius vs time for energy-conserving phase
        R_p = (
            16 * (L36 / (v_wind.value / 1000) / n0) ** (1 / 4) * (t6) ** (1 / 2)
        )  # Radius vs time for momentum-conserving phase
        v_e = (
            17 * (L36 / mu / n0) ** (1 / 5) * t6 ** (-2 / 5)
        )  # Velocity vs time for energy-conserving phase
        v_p = (
            7.9 * (L36 / (v_wind.value / 1000) / n0) ** (1 / 4) * t6 ** (-1 / 2)
        )  # Velocity vs time for momentum-conserving phase
        T = (
            1.6e6 * L36 ** (8 / 35) * n0 ** (2 / 35) * t6 ** (-6 / 35)
        )  # Temperature vs time for energy-conserving phase
        n = (
            0.01 * L36 ** (6 / 35) * n0 ** (19 / 35) * t6 ** (-22 / 35)
        )  # + n0                   # Density vs time for energy-conserving phase
        ...
        R_e_art = (
            0.88
            * (e_rate.to("erg/s").value / (mu * n0 * mh.to("g").value)) ** (1 / 5)
            * time.to("s").value ** (3 / 5)
            * cm_to_pc.value
        )
        R_p_art = (
            (
                3
                / (2 * np.pi) ** (1 / 4)
                * (2 * e_rate.to("erg/s").value / (v_wind.to("cm/s").value))
                / (n0 * mu * mh.to("g").value)
            )
            ** (1 / 4)
            * time.to("s").value ** (1 / 2)
    ...
    if t6 < 0 or L36 < 0:
        print(
            f"Warning: Non-physical values for theory at time={time.value:.3f} Myr, Energy={e_rate:.2e}. Setting theory to zero."
        )
...
def main():
    ...
    v_wind = (
        np.sqrt(2 * e_rate / m_loss).to("km/s") if m_val > 0 else 0 * unyt.km / unyt.s
    )

    print(
        f"Extracted Feedback: Energy={e_rate:.2e}, MassLoss={m_loss:.2e}, WindVel={v_wind:.2f}"
    )
    ...
    mid = len(data_list) // 2
    axes[0].set_ylim(
        data_list[mid][1].min().value * 1.2, data_list[mid][1].max().value * 1.2
    )
    ...
    with open("radial_peak_positions.txt", "w") as f_log:
        f_log.write(
            "# Time R_peak Rho_max R_min Rho_min R_Vmax Vmax V_at_r R_Tmax Tmax ... \n"
        )
