...
def compute_face_area(
    ...
    r_ij = np.linalg.norm(
        star_position[:, None, :] - gas_position[None, :, :], axis=-1
    )  # Shape (Ns, Ng)

    # Compute unit vector r_ij / |r_ij| safely
    unit_vector = (gas_position[None, :, :] - star_position[:, None, :]) / np.where(
        r_ij[..., None] > 0, r_ij[..., None], 1
    )  # Shape (Ns, Ng, 3)

    # Compute gradients of the Wendland C2 kernel correctly
    grad_w_star = np.abs(
        dwendland_c2(r_ij, star_smoothing_length[:, None])
    )  # Shape (Ns, Ng)
    grad_w_gas = np.abs(
        dwendland_c2(r_ij, gas_smoothing_length[None, :])
    )  # Shape (Ns, Ng)
    ...
    face_area_vector = (
        ((grad_w_star / (n_star_density ** 2)) + (grad_w_gas / (n_gas_density ** 2)))[
            :, :, None
        ]
        * unit_vector
    )  # Shape (Ns, Ng, 3)
