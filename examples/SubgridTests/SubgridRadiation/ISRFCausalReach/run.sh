...
delta_time=${delta_time:=3.2e-5} #Snapshots:delta_time override (internal units)
...
hot_particle_temperature_K=${hot_particle_temperature_K:=0} #>0: pinned-neighbour variant (see README)
run_name=${run_name:=""}

# Remove the ICs
if [ -e ICs_isrf_causal_reach.hdf5 ]
then
    rm ICs_isrf_causal_reach.hdf5
fi
...
# force. SPH:initial_temperature is forced back to bulk_temperature_K here
# (params.yml leaves it at 0 so makeIC.py's own per-particle InternalEnergy
# survives) unless the pinned-neighbour variant needs it to stay at 0 --
# see README.
initial_temperature_override=$bulk_temperature_K
if [ "$hot_particle_temperature_K" != "0" ]; then
    initial_temperature_override=0
fi
...
../../../../swift --hydro --stars --external-gravity --feedback --cooling \
		   ...
		   -P SPH:initial_temperature:$initial_temperature_override \
		   params.yml 2>&1 | tee output.log

# Check the propagated field's causal reach against c_hyp*t (see README).
python3 isrf_causal_reach_check.py
