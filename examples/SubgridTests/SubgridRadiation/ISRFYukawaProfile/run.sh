...
# Remove the ICs
if [ -e ICs_isrf_yukawa_profile.hdf5 ]
then
    rm ICs_isrf_yukawa_profile.hdf5
fi
...
ic_output=$(python3 makeIC.py --level $level --rho $gas_density \
	...
	--star_type $star_type \
    -o ICs_isrf_yukawa_profile.hdf5)
echo "$ic_output"
...
# Check the steady-state Yukawa profile against the analytic prediction
python3 isrf_yukawa_profile_check.py
