...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# This is for dev purposes only and should never find
# itself in the master branch
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
def check_hydro_sanity(snapdata):
    ...
    for snap in snapdata:
        ...
        # --------------------------------------------------------------
        mask = gas.TransportDone != RT_RESCHEDULE_MAX
        fishy = np.logical_and(mask, called)
        if fishy.any():
            print("- checking hydro sanity pt2.3; snapshot", snap.snapnr)
            if np.count_nonzero(mask) == npart:
                print("--- WARNING: zero particles transport with transport done == RT_RESCHEDULE_MAX")
            else:
                print(
                    "--- Some transport was finalised != RT_RESCHEDULE_MAX",
                    np.count_nonzero(called),
                    "/",
                    npart,
                )
                if print_diffs:
                    print("----- IDs with transport done != RT_RESCHEDULE_MAX:")
        ...
        # --------------------------------------------------------------
        mask = gas.ThermochemistryDone != RT_RESCHEDULE_MAX
        fishy = np.logical_and(mask, called)
        if fishy.any():
            ...
            if np.count_nonzero(mask) == npart:
                ...
                print(
                    "--- Some thermochemistry done != RT_RESCHEDULE_MAX",
                    np.count_nonzero(called),
                    "/",
                    npart,
                )
                if print_diffs:
                    print("----- IDs with Thermochemistry_done != RT_RESCHEDULE_MAX:")
