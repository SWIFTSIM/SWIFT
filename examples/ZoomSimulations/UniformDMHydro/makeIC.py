"""Generate the DM-only IC used by UniformDMHydro.

This wrapper reuses the UniformDMGravity IC generator so the hydro example uses
exactly the same deterministic DM layout, then copies the result to the local
IC file name expected by the hydro parameter file.

Why this wrapper exists:
- keeps the DM IC construction logic in one place (UniformDMGravity),
- guarantees the hydro and gravity examples start from identical DM ICs,
- avoids copy/pasting and drifting IC-generation implementations.

Output:
- ``zoom_uniform_dm_hydro.hdf5`` in this directory.
"""
...
TARGET_IC = HERE / "zoom_uniform_dm_hydro.hdf5"
