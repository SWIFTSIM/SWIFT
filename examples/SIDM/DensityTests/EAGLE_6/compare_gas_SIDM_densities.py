...
def main():
    ...
    with h5py.File(sidm_fname, "r") as f:
        ...
        sidm_x = np.array(f["SIDMParticles"]["Coordinates"][:, 0])
        sidm_rho_sph = np.array(f["SIDMParticles"]["Densities"]) * get_unit_conversions(
            f
        )
    df_sidm = pd.DataFrame(
        {"ParticleIDs": sidm_pid, "Densities": sidm_rho_sph, "x": sidm_x}
    )

    with h5py.File(gas_fname, "r") as f:
        gas_pid = np.array(f["GasParticles"]["ParticleIDs"])
        gas_x = np.array(f["GasParticles"]["Coordinates"][:, 0])
        gas_rho_sph = np.array(f["GasParticles"]["Densities"]) * get_unit_conversions(f)
    df_gas = pd.DataFrame(
        {"ParticleIDs": gas_pid, "Densities": gas_rho_sph, "x": gas_x}
    )

    # match on coordinates (ParticleIDs reassigned so can't match on those)
    df = df_sidm.merge(df_gas, on="x", how="left", suffixes=["_sidm", "_gas"])
    ...
    relative_error = (
        2.0
        * np.abs(df["Densities_sidm"] - df["Densities_gas"])
        / (df["Densities_sidm"] + df["Densities_gas"])
    )
    print("Max relative error:", np.max(relative_error))
    print("99th percentile relative error:", np.percentile(relative_error, 99))
