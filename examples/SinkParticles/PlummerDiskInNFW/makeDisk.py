...
UnitMass_in_g           = 4.435693e+44
UnitVelocity_in_cm_per_s= 97824708.2699

k   = (c.k_B.to(u.g * u.cm**2/u.s**2/u.K)/UnitVelocity_in_cm_per_s**2 /UnitMass_in_g).value             # Boltzman constant   in code units
...
m_ref = 5000.     # particle gas mass in solar mass
...
eps = 0.15/(mscale*m_ref/50000)**(1/3.)   # softening normalized to m_ref = 50000
...
Hr_halo = 50.            # for plummer=30, for nfw, determined rmax
Rs_halo = 15.            # for nfw
dR_halo = 0.25
fr_halo = 3.0
...
def g_halo(r): return np.log(r / rc_halo + 1.)


def gm_halo(r): return rc_halo * (np.exp(r) - 1.)
...
def g_bulge(r): return np.log(r / rc_bulge + 1.)


def gm_bulge(r): return rc_bulge * (np.exp(r) - 1.)
...
def g_disk(r): return np.log(r / rc_disk + 1.)


def gm_disk(r): return rc_disk * (np.exp(r) - 1.)
...
mode_sigma_r = {"name": "epicyclic_approximation", "param": 0.5}      # this is still bad, the SigmaR decrease at the center
...
def g_gas(r): return np.log(r / rc_gas + 1.)


def gm_gas(r): return rc_gas * (np.exp(r) - 1.)
...
if nb_disk is not None:
    ...
    nb_disk, phi, stats_disk = nb.Get_Velocities_From_Cylindrical_Grid(select='disk', disk=('disk', 'gas'), eps=eps_disk, nR=nr_disk, nz=nz_disk, nt=nt_disk, Rmax=grmax_disk,
                                                                       zmin=gzmin_disk, zmax=gzmax_disk, params=params_disk, Phi=None, g=g_disk, gm=gm_disk, ErrTolTheta=ErrTolTheta, AdaptativeSoftenning=AdaptativeSoftenning)
...
if nb_gas is not None:
    ...
    print("------------------------")
    nb_gas, phi, stats_gas = nb.Get_Velocities_From_Cylindrical_Grid(select='gas', disk=('disk', 'gas'), eps=eps_gas, nR=nr_gas, nz=nz_gas, nt=nt_gas, Rmax=grmax_gas,
                                                                     zmin=gzmin_gas, zmax=gzmax_gas, params=params_gas, Phi=None, g=g_gas, gm=gm_gas, ErrTolTheta=ErrTolTheta, AdaptativeSoftenning=AdaptativeSoftenning)
...
if nb_bulge is not None:
    ...
    nb_bulge, phi, stats_bulge = nb.Get_Velocities_From_Spherical_Grid(
        select='bulge', eps=eps_bulge, nr=nr_bulge, rmax=grmax_bulge, phi=None, g=g_bulge, gm=gm_bulge, UseTree=True, ErrTolTheta=ErrTolTheta)
...
if nb_halo is not None:
    ...
    nb_halo, phi, stats_halo = nb.Get_Velocities_From_Spherical_Grid(
        select='halo', eps=eps_halo, nr=nr_halo, rmax=grmax_halo, phi=None, g=g_halo, gm=gm_halo, UseTree=True, ErrTolTheta=ErrTolTheta)
...
if hydro:
  ...
  mumh = m_p*mu
  nb.u = T_gas / (gamma - 1.) * k / mumh  * np.ones(nb.nbody)
...
nb.ChimieSolarMassAbundances={'Fe': 0.0017660372, 
                              'Mg': 0.00092431647, 
                               'O': 0.010816922, 
                               'S': 0.00068551616, 
                              'Zn': 2.6028247e-06, 
