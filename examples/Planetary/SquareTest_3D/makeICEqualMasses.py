...
# Generates a swift IC file for the 3D square test with equal particle mass
...
for i in range(N_l_out):
    for j in range(N_l_out):
        for k in range(N_l_out):
            ...
            pos_out[index, 0] =  i / (float(N_l_out)) + 1./(2. * N_l_out)
            pos_out[index, 1] =  j / (float(N_l_out)) + 1./(2. * N_l_out)
            pos_out[index, 2] =  k / (float(N_l_out)) + 1./(2. * N_l_out)

h_out = np.ones(numPart_out) * (1. / N_l_out)
m_out = np.ones(numPart_out) * vol  * rho_out / numPart_out
u_out = np.ones(numPart_out) * P_out / (rho_out * (gamma - 1.))
...
for i in range(N_l_in):
    for j in range(N_l_in):
        for k in range(N_l_in):
            ...
            pos_in[index, 0] =  0.25 + i / float(2 * N_l_in) + 1./(2. * 2 * N_l_in)
            pos_in[index, 1] =  0.25 + j / float(2 * N_l_in) + 1./(2. * 2 * N_l_in)
            pos_in[index, 2] =  0.25 + k / float(2 * N_l_in) + 1./(2. * 2 * N_l_in)

h_in = np.ones(numPart_in) * (1. / N_l_in)
m_in = np.ones(numPart_in) * m_out[0]
u_in = np.ones(numPart_in) * P_in / (rho_in * (gamma - 1.))
rho_in = np.ones(numPart_in) * rho_in

# Remove the particles within the central cube from the outer region
mask_out = np.logical_or.reduce((pos_out[:,0] < 0.25, pos_out[:,0] > 0.75,
                                 pos_out[:,1] < 0.25, pos_out[:,1] > 0.75,
                                 pos_out[:,2] < 0.25, pos_out[:,2] > 0.75))


#mask_out = np.logical_or(np.logical_or(pos_out[:,2] < 0.25 , pos_out[:,2] > 0.75),
#                      np.logical_or(np.logical_or(pos_out[:,0] < 0.25 , pos_out[:,0] > 0.75),
#                      np.logical_or(pos_out[:,1] < 0.25, pos_out[:,1] > 0.75)))
