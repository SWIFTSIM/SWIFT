...
__attribute__((always_inline)) INLINE static int tracers_write_sparticles(
  ...
  list[11] = io_make_output_field(
      "NumberOfWindInjectionSteps", INT, 1, UNIT_CONV_NO_UNITS, 0.f, sparts,
      tracers_data.winds.n_injection_steps,
      "Number of timesteps in which this star injected a non-zero wind "
      "budget into gas neighbours.");
  ...
  if (with_cosmology) {
    list[13] = io_make_physical_output_field(
        "ScaleFactorAtLastWindInjection", FLOAT, 1, UNIT_CONV_NO_UNITS, 0.f,
        sparts, tracers_data.winds.last_injection_scale_factor,
        /*can convert to comoving=*/0,
        "Scale-factor at this star's most recent wind injection step. 0 if "
        "it has never had one.");
  } else {
    list[13] = io_make_output_field(
        "TimeAtLastWindInjection", FLOAT, 1, UNIT_CONV_TIME, 0.f, sparts,
        tracers_data.winds.last_injection_time,
        "Simulation time at this star's most recent wind injection step. 0 "
