...
#define HASHMAP_UF_GROWTH_FACTOR (2)
#define HASHMAP_UF_MAX_FILL_RATIO (1.0)
#define HASHMAP_UF_MAX_CHUNK_FILL_RATIO (0.5)

/**
 * @brief Pre-allocate a number of chunks for the graveyard.
 */
void hashmap_uf_allocate_chunks(hashmap_uf_t *m, int num_chunks) {
  /* Allocate a fresh set of chunks. */
  hashmap_uf_chunk_t *alloc;
  if ((alloc = (hashmap_uf_chunk_t *)swift_calloc(
           "hashmap_uf", num_chunks, sizeof(hashmap_uf_chunk_t))) == NULL) {
...
void hashmap_uf_init(hashmap_uf_t *m) {
  ...
  if ((m->chunks = (hashmap_uf_chunk_t **)swift_calloc(
           "hashmap_uf", m->nr_chunks, sizeof(hashmap_uf_chunk_t *))) == NULL) {
    error("Unable to allocate hashmap_uf chunks.");
  }
  ...
  m->allocs_size = HASHMAP_UF_ALLOCS_INITIAL_SIZE;
  if ((m->allocs = (void **)swift_malloc(
           "hashmap_uf", sizeof(void *) * m->allocs_size)) == NULL) {
 ...
 */
void hashmap_uf_release_chunk(hashmap_uf_t *m, hashmap_uf_chunk_t *chunk) {
  /* Clear all the chunk's data. */
  memset(chunk, 0, sizeof(hashmap_uf_chunk_t));
 ...
 */
hashmap_uf_chunk_t *hashmap_uf_get_chunk(hashmap_uf_t *m) {
  int num_chunks = m->nr_chunks * HASHMAP_UF_ALLOC_SIZE_FRACTION;
  if (!num_chunks) num_chunks = 1;
  if (m->graveyard == NULL) {
    hashmap_uf_allocate_chunks(m, num_chunks);
  }

  hashmap_uf_chunk_t *res = (hashmap_uf_chunk_t *)m->graveyard;
  m->graveyard = (hashmap_uf_chunk_t *)res->next;
  res->next = NULL;
...
hashmap_uf_element_t *hashmap_uf_find(hashmap_uf_t *m, hashmap_uf_key_t key,
  ...
  if (create_new && m->size > m->table_size * HASHMAP_UF_MAX_FILL_RATIO) {
    if (HASHMAP_UF_DEBUG_OUTPUT) {
      message("hashmap_uf is too full (%zu of %zu elements used), re-hashing.",
              m->size, m->table_size);
    }
    return NULL;
  }
  ...
  if (m->chunks[chunk_offset] == NULL) {
    ...
    m->chunks[chunk_offset] = hashmap_uf_get_chunk(m);
  }
  hashmap_uf_chunk_t *chunk = m->chunks[chunk_offset];
  ...
  if (create_new && chunk_count > HASHMAP_UF_ELEMENTS_PER_CHUNK *
    ...
    if (HASHMAP_UF_DEBUG_OUTPUT) {
      message(
          ...
          "%.2f%%), re-hashing.",
          chunk_offset, chunk_count, HASHMAP_UF_ELEMENTS_PER_CHUNK,
  ...
  for (int i = 0; i < HASHMAP_UF_MAX_CHAIN_LENGTH; i++) {
    /* Record the chain_length, if not NULL. */
    if (chain_length) *chain_length = i;

    /* Compute the offsets within the masks of this chunk. */
    const int mask_offset = offset_in_chunk / HASHMAP_UF_BITS_PER_MASK;
    const int offset_in_mask =
        offset_in_chunk - mask_offset * HASHMAP_UF_BITS_PER_MASK;

    /* Is the offset empty? */
    hashmap_uf_mask_t search_mask = ((hashmap_uf_mask_t)1) << offset_in_mask;
    ...
    /* None of the above, so this is a collision. Re-hash, but within the same
      ...
      offset_in_chunk = rand_r(&curr) % HASHMAP_UF_ELEMENTS_PER_CHUNK;
    }
  }
...
void hashmap_uf_grow(hashmap_uf_t *m, size_t new_size) {
  ...
  hashmap_uf_element_t *strays = NULL;
  size_t strays_count = 0;
  size_t strays_size = 0;

  /* Iterate over the chunks and add their entries to the new table. */
  for (size_t cid = 0; cid < old_table_size / HASHMAP_UF_ELEMENTS_PER_CHUNK;
       cid++) {
    /* Skip empty chunks. */
    hashmap_uf_chunk_t *chunk = old_chunks[cid];
    if (!chunk) continue;

    /* Loop over the masks in this chunk. */
    for (int mid = 0; mid < HASHMAP_UF_MASKS_PER_CHUNK; mid++) {
      /* Skip empty masks. */
      if (chunk->masks[mid] == 0) continue;

      /* Loop over the mask entries. */
      for (int eid = 0; eid < HASHMAP_UF_BITS_PER_MASK; eid++) {
        hashmap_uf_mask_t element_mask = ((hashmap_uf_mask_t)1) << eid;
        if (chunk->masks[mid] & element_mask) {
          ...
          /* If copying failed, then we have an overflow in the new hashmap_uf.
             If this happens, store the offending element in the strays buffer
             for now. */
          else {
            /* (Re)allocate strays buffer? */
            if (strays_count == strays_size) {
              hashmap_uf_element_t *temp_buff;
              strays_size = strays_size ? strays_size * 2 : 10;
              if ((temp_buff = (hashmap_uf_element_t *)swift_malloc(
                       "hashmap_uf_strays",
                       sizeof(hashmap_uf_element_t) * strays_size)) == NULL)
                error("Failed to (re)allocate strays buffer.");
              memcpy(temp_buff, strays,
                     sizeof(hashmap_uf_element_t) * strays_count);
              swift_free("hashmap_uf_strays", strays);
              strays = temp_buff;
            }
    ...
    hashmap_uf_release_chunk(m, chunk);
  }
...
hashmap_uf_value_t *hashmap_uf_get(hashmap_uf_t *m, hashmap_uf_key_t key) {
...
void hashmap_uf_iterate(hashmap_uf_t *m, hashmap_uf_mapper_t f, void *data) {
  /* Loop over the chunks. */
  for (size_t cid = 0; cid < m->nr_chunks; cid++) {
    hashmap_uf_chunk_t *chunk = m->chunks[cid];
    if (!chunk) continue;

    /* Loop over the masks. */
    for (int mid = 0; mid < HASHMAP_UF_MASKS_PER_CHUNK; mid++) {
      hashmap_uf_mask_t mask = chunk->masks[mid];
      if (!mask) continue;

      /* Loop over each element in the mask. */
      for (int eid = 0;
           eid < HASHMAP_UF_BITS_PER_MASK &&
           mid * HASHMAP_UF_BITS_PER_MASK + eid < HASHMAP_UF_ELEMENTS_PER_CHUNK;
           eid++) {
        /* If the element exists, call the function on it. */
        if (mask & (((hashmap_uf_mask_t)1) << eid)) {
          hashmap_uf_element_t *element =
              &chunk->data[mid * HASHMAP_UF_BITS_PER_MASK + eid];
...
void hashmap_uf_free(hashmap_uf_t *m) {
  ...
  for (size_t k = 0; k < m->allocs_count; k++) {
    swift_free("hashmap_uf", m->allocs[k]);
  }
  swift_free("hashmap_uf", m->allocs);
}

size_t hashmap_uf_size(hashmap_uf_t *m) {
  if (m != NULL)
    return m->size;
  else
    return 0;
...
void hashmap_uf_print_stats(hashmap_uf_t *m) {
  ...
  for (hashmap_uf_chunk_t *finger = m->graveyard; finger != NULL;
       finger = (hashmap_uf_chunk_t *)finger->next) {
    graveyard_counter += 1;
  }
  message(
      "populated chunks: %i (%zu kb), graveyard chunks: %i (%zu kb), allocs: "
      "%zu",
      chunk_counter, sizeof(hashmap_uf_chunk_t) * chunk_counter / 1024,
      graveyard_counter, sizeof(hashmap_uf_chunk_t) * graveyard_counter / 1024,
      m->allocs_count);
  ...
  for (int k = 0; k < HASHMAP_UF_MAX_CHAIN_LENGTH; k++) {
    m->chain_length_counts[k] = 0;
  }
  hashmap_uf_iterate(m, hashmap_uf_count_chain_lengths, m);
  ...
  for (size_t k = 0; k < m->nr_chunks; k++) {
    hashmap_uf_chunk_t *chunk = m->chunks[k];
    if (!chunk) {
  ...
  for (int k = 0; k < 11; k++) {
    message("  %3i%% - %3i%%: %zu (%.2f%%)",
            (int)(k * 10 * HASHMAP_UF_MAX_CHUNK_FILL_RATIO),
            (int)((k + 1) * 10 * HASHMAP_UF_MAX_CHUNK_FILL_RATIO),
            chunk_fill_ratio_counts[k],
            (100.0 * chunk_fill_ratio_counts[k]) / m->nr_chunks);
  }

  /* Print struct sizes. */
  message("sizeof(hashmap_uf_element_t): %zu", sizeof(hashmap_uf_element_t));
  message("sizeof(hashmap_uf_chunk_t): %zu", sizeof(hashmap_uf_chunk_t));
  message("HASHMAP_UF_TARGET_CHUNK_BYTES: %i", HASHMAP_UF_TARGET_CHUNK_BYTES);
  message("HASHMAP_UF_BITS_PER_ELEMENT: %i", HASHMAP_UF_BITS_PER_ELEMENT);
  message("HASHMAP_UF_ELEMENTS_PER_CHUNK: %i", HASHMAP_UF_ELEMENTS_PER_CHUNK);
  message("HASHMAP_UF_MASKS_PER_CHUNK: %i", HASHMAP_UF_MASKS_PER_CHUNK);
}
