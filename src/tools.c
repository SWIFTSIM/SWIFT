  ...
  for (int i = 0; i < ci->hydro.count; ++i) {
    ...
    for (int j = i + 1; j < ci->hydro.count; ++j) {
      ...
      if (r2 < hjg2 && part_is_active(pj, e) && !part_is_inhibited(pi, e)) {
        ...
        /* Interact */
        runner_iact_nonsym_density(r2, dx, hj, hi, pj, pi, mu_0, a, H);
  ...
  for (int i = 0; i < ci->hydro.count; ++i) {
    ...
    for (int j = i + 1; j < ci->hydro.count; ++j) {
      ...
      if (part_is_active(pj, e) && !part_is_inhibited(pi, e)) {
        ...
        if (r2 < hig2 || r2 < hjg2) {
          /* Interact */
          runner_iact_nonsym_gradient(r2, dx, hj, hi, pj, pi, mu_0, a, H);
        }
#else
        if (r2 < hjg2) {
          /* Interact */
  ...
  for (int i = 0; i < ci->hydro.count; ++i) {
    ...
    for (int j = i + 1; j < ci->hydro.count; ++j) {
      ...
      if (r2 < hig2 || r2 < hjg2) {
        ...
        /* Interact */
        runner_iact_force(r2, dx, hi, hj, pi, pj, mu_0, a, H);
      }
    }
  }
}
