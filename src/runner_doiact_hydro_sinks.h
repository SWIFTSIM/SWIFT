...
/* Before including this file, define FUNCTION (e.g. "formation"), which
   expands to the full set of interaction function names used by the gas
   loop for sink particle formation:

     runner_dopair1_branch_hydro_sinks_FUNCTION
     runner_dopair1_hydro_sinks_FUNCTION
     runner_dopair1_naive_hydro_sinks_FUNCTION
     runner_doself1_branch_hydro_sinks_FUNCTION
     runner_doself1_hydro_sinks_FUNCTION
     runner_doself1_naive_hydro_sinks_FUNCTION
     runner_dosub_self1_hydro_sinks_FUNCTION
     runner_dosub_pair1_hydro_sinks_FUNCTION

   Only the non-symmetric _1_ variants exist.  The formation loop uses a
   pure gather pattern (each active particle accumulates from neighbours),
   so no symmetric _2_ variants or subset-reiteration functions are needed. */

#define PASTE(x, y) x##_##y

#define _DOPAIR1_BRANCH_HYDRO_SINKS(f) \
  PASTE(runner_dopair1_branch_hydro_sinks, f)
...
#define _DOSELF1_BRANCH_HYDRO_SINKS(f) \
  PASTE(runner_doself1_branch_hydro_sinks, f)
