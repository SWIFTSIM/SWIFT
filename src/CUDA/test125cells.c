...
struct cell *make_cell(size_t n, const double offset[3], double size, double h,
                       ...
                       enum velocity_field vel, enum pressure_field press, struct part* parts) {
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  allocate_cells((void**) &parts, particles, 125);
    ...
    for (int j = 0; j < 5; ++j) {
      for (int k = 0; k < 5; ++k) {
        ...
        cells[i * 25 + j * 5 + k] = make_cell(
            particles, offset, size, h, rho, &partId, perturbation, vel, press, &parts[part3*(i*25 + j* 5 + k )]);
...
/* Do the force calculation */
 ...
 /* Run the GPU code */
    /* Zero the fields */
    for (int j = 0; j < 125; ++j) reset_particles(cells[j], &space.hs, vel, press, size, rho);
    ...
    /* Call GPU code */
    test_125_cells(cells, main_cell, parts, &engine);
    /* Finally, give a gentle kick */
    runner_do_end_force(&runner, main_cell, 0);

    /* Dump if necessary */
//    if (i % 50 == 0) {
      sprintf(outputFileName, "swift_dopair_125_gpu_%s.dat",
              outputFileNameExtension);
      dump_particle_fields(outputFileName, main_cell, solution, 0);
//    }
