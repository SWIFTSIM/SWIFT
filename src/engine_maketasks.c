...
void engine_make_hierarchical_tasks_gravity(struct engine *e, struct cell *c) {
  ...
  if (c->grav.super == c) {
    ...
    if (c->nodeID == e->nodeID) {
      ...
      if (is_self_gravity) {
        ...
        /* Long-range gravity forces once again omitting zoom cells (not the
         * mesh ones!) */
        if (c->type != cell_type_zoom) {
  ...
  else if ((c->grav.super != NULL || c->void_parent != NULL) &&
...
#ifdef EXTRA_STAR_LOOPS
        c->stars.prep1_ghost = scheduler_addtask(s, task_type_stars_prep_ghost1,
                                                 task_subtype_none, 0,
                                                 /* implicit = */ 1, c, NULL);

        c->hydro.prep1_ghost = scheduler_addtask(s, task_type_hydro_prep_ghost1,
                                                 task_subtype_none, 0,
                                                 /* implicit = */ 1, c, NULL);

        c->stars.prep2_ghost = scheduler_addtask(s, task_type_stars_prep_ghost2,
                                                 task_subtype_none, 0,
                                                 /* implicit = */ 1, c, NULL);
#endif
  ...
  for (int ii = i - delta_m; ii <= i + delta_p; ii++) {
    ...
    for (int jj = j - delta_m; jj <= j + delta_p; jj++) {
      ...
      for (int kk = k - delta_m; kk <= k + delta_p; kk++) {
        ...
        /* Avoid duplicate and same cell pairs. */
        if (cid >= cjd) continue;
