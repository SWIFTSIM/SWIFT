...
void engine_addtasks_send_gravity(struct engine *e, struct cell *ci,
  ...
  if (t_grav_counts == NULL && with_star_formation && ci->hydro.count > 0) {
    ...
    if (ci->depth != 0)
      error("Attaching a grav_count task at a non-top level c->depth=%d "
            "c->count=%d",
...
#ifdef EXTRA_STAR_LOOPS
    if (with_feedback)
      engine_addlink(e, &ci->mpi.send, t_prep1);
#endif
...
#ifdef EXTRA_STAR_LOOPS
    if (with_feedback)
      engine_addlink(e, &c->mpi.recv, t_prep1);
#endif
...
int engine_gravity_need_cell_pair_task(struct engine *e, struct cell *ci,
  ...
  if (use_mesh && min_radius2 > max_distance2)
    return 0;
  ...
  for (int ii = i - delta_m; ii <= i + delta_p; ii++) {
    ...
    for (int jj = j - delta_m; jj <= j + delta_p; jj++) {
      ...
      for (int kk = k - delta_m; kk <= k + delta_p; kk++) {
        ...
        if (ci->type != cj->type) {
          error("Cell %d and cell %d are not the same cell type! "
                "(ci->type=%s, cj->type=%s)",
        ...
        if (cj->grav.count == 0 && cj->subtype != cell_subtype_void)
          continue;
...
void engine_make_extra_hydroloop_tasks_mapper(void *map_data, int num_elements,
  ...
  for (int ind = 0; ind < num_elements; ind++) {
    ...
    if (t->type == task_type_star_formation)
      continue;
    if (t->type == task_type_star_formation_sink)
      continue;
    ...
    for (int ii = -1; ii < 2; ii++) {
      ...
      for (int jj = -1; jj < 2; jj++) {
        ...
        for (int kk = -1; kk < 2; kk++) {
          ...
          if (s->with_zoom_region &&
              (ci->type != cell_type_zoom || cj->type != cell_type_zoom)) {
            error("Trying to make FOF tasks for non-zoom cells while running a "
                  "zoom! (ci->type=%s, cj->type=%s)",
                  cellID_names[ci->type], cellID_names[cj->type]);
          }
#endif
 ...
 * cell_set_super_mapper. */
  ...
  if (e->verbose)
    message("Nr. of links: %zd allocated links: %zd ratio: %f memory use: %zd "
            "MB.",
            e->nr_links, e->size_links,
            (float)e->nr_links / (float)e->size_links,
