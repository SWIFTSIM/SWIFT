...
void engine_make_hierarchical_tasks_gravity(struct engine *e, struct cell *c) {
   ...
   */
  void engine_add_ghosts(struct engine * e, struct cell * c,
  ...
  void engine_make_hierarchical_tasks_hydro(struct engine * e, struct cell * c,
    ...
    if (c->hydro.super == c) {
      ...
      /* Add the sort task. */
      c->hydro.sorts = scheduler_addtask(s, task_type_sort, task_subtype_none,
                                         0, 0, c, NULL);
      ...
      if (c->nodeID == e->nodeID) {
        ...
        c->hydro.end_force = scheduler_addtask(
            s, task_type_end_hydro_force, task_subtype_none, 0, 0, c, NULL);
        ...
        if (with_stars) {
          ...
          if (with_star_formation && c->top->hydro.count > 0)
            scheduler_addunlock(s, c->stars.drift,
...
#ifdef EXTRA_STAR_LOOPS
          c->stars.prep1_ghost = scheduler_addtask(
              s, task_type_stars_prep_ghost1, task_subtype_none, 0,
              /* implicit = */ 1, c, NULL);

          c->hydro.prep1_ghost = scheduler_addtask(
              s, task_type_hydro_prep_ghost1, task_subtype_none, 0,
              /* implicit = */ 1, c, NULL);

          c->stars.prep2_ghost = scheduler_addtask(
              s, task_type_stars_prep_ghost2, task_subtype_none, 0,
              /* implicit = */ 1, c, NULL);
#endif
        ...
        if (with_rt) {
          ...
          c->rt.rt_transport_out = scheduler_addtask(
              s, task_type_rt_transport_out, task_subtype_none, 0,
              /*implicit= */ 1, c, NULL);
        ...
        if (with_black_holes) {
          ...
          c->black_holes.swallow_ghost_2 = scheduler_addtask(
              s, task_type_bh_swallow_ghost2, task_subtype_none, 0,
              /* implicit =*/1, c, NULL);
  ...
  void engine_make_hierarchical_tasks_mapper(void *map_data, int num_elements,
    ...
    for (int ind = 0; ind < num_elements; ind++) {
      ...
      /* Skip void cells, we make the subset of tasks these need ourselves
       * elsewhere (only applicable while running a zoom). */
   ...
   * @param nodeID The nodeID of the current node.
   */
  void engine_make_pair_gravity_task(
      struct engine * e, struct scheduler * sched, struct cell * ci,
      struct cell * cj, const int nodeID, const int cid, const int cjd) {
    /* Add a pair task. */
   ...
   */
  void engine_gravity_make_task_loop(struct engine * e, int cid,
                                     const int cdim[3], struct cell *cells,
                                     const int periodic, const int use_mesh,
                                     const int delta_m, const int delta_p) {
    ...
    for (int ii = i - delta_m; ii <= i + delta_p; ii++) {
      ...
      for (int jj = j - delta_m; jj <= j + delta_p; jj++) {
        ...
        for (int kk = k - delta_m; kk <= k + delta_p; kk++) {
          ...
          if (engine_gravity_need_cell_pair_task(e, ci, cj, periodic,
                                                 use_mesh)) {
...
/* Node ID (if running with MPI) */
  ...
  static inline void engine_make_hydro_loops_dependencies(
      ...
      struct task * force, struct task * limiter, struct cell * c,
      int with_cooling, int with_limiter) {
  ...
  static inline void engine_make_hydro_loops_dependencies(
      ...
      struct task * limiter, struct cell * c, int with_cooling,
      int with_limiter) {
    ...
    for (int ind = 0; ind < num_elements; ind++) {
      ...
      /* Self-interaction? */
        ...
        if (with_black_holes && bcount_i > 0) {
          t_bh_density =
              scheduler_addtask(sched, task_type_self, task_subtype_bh_density,
                                flags, 0, ci, NULL);
          t_bh_swallow =
              scheduler_addtask(sched, task_type_self, task_subtype_bh_swallow,
                                flags, 0, ci, NULL);
          t_do_gas_swallow = scheduler_addtask(sched, task_type_self,
        ...
        t_gradient = scheduler_addtask(
            sched, task_type_self, task_subtype_gradient, flags, 0, ci, NULL);
      ...
      /* Otherwise, pair interaction? */
...
#ifdef EXTRA_STAR_LOOPS
          t_star_prep1 =
              scheduler_addtask(sched, task_type_pair, task_subtype_stars_prep1,
                                flags, 0, ci, cj);
          t_star_prep2 =
              scheduler_addtask(sched, task_type_pair, task_subtype_stars_prep2,
                                flags, 0, ci, cj);
#endif
        }
        ...
        if (with_black_holes && (bcount_i > 0 || bcount_j > 0)) {
          ...
          t_bh_feedback =
              scheduler_addtask(sched, task_type_pair, task_subtype_bh_feedback,
                                flags, 0, ci, cj);
        }

        if (with_rt) {
          t_rt_gradient =
              scheduler_addtask(sched, task_type_pair, task_subtype_rt_gradient,
                                flags, 0, ci, cj);
          t_rt_transport =
              scheduler_addtask(sched, task_type_pair,
        ...
        if (cj->nodeID == nodeID) {
          ...
          if (ci->hydro.super != cj->hydro.super) {
            ...
            scheduler_addunlock(sched, t_force,
                                cj->hydro.super->hydro.end_force);
      ...
      /* Otherwise, sub-self interaction? */
        ...
        t_force = scheduler_addtask(sched, task_type_sub_self,
                                    task_subtype_force, flags, 0, ci, NULL);
        ...
        if (with_sink) {
          t_sink_swallow =
              scheduler_addtask(sched, task_type_sub_self,
                                task_subtype_sink_swallow, flags, 0, ci, NULL);
          t_sink_do_sink_swallow = scheduler_addtask(
              sched, task_type_sub_self, task_subtype_sink_do_sink_swallow,
              flags, 0, ci, NULL);
          t_sink_do_gas_swallow = scheduler_addtask(
              sched, task_type_sub_self, task_subtype_sink_do_gas_swallow,
              flags, 0, ci, NULL);
        }
        ...
        if (with_black_holes && bcount_i > 0) {
          ...
          t_do_gas_swallow = scheduler_addtask(sched, task_type_sub_self,
                                               task_subtype_do_gas_swallow,
                                               flags, 0, ci, NULL);
      ...
      /* Otherwise, sub-pair interaction? */
        ...
        /* New task for the force */
        t_force = scheduler_addtask(sched, task_type_sub_pair,
                                    task_subtype_force, flags, 0, ci, cj);
        ...
        if (with_sink) {
          t_sink_swallow =
              scheduler_addtask(sched, task_type_sub_pair,
                                task_subtype_sink_swallow, flags, 0, ci, cj);
          t_sink_do_sink_swallow = scheduler_addtask(
              sched, task_type_sub_pair, task_subtype_sink_do_sink_swallow,
              flags, 0, ci, cj);
          t_sink_do_gas_swallow = scheduler_addtask(
              sched, task_type_sub_pair, task_subtype_sink_do_gas_swallow,
              flags, 0, ci, cj);
        }
        ...
        for (int jj = -1; jj < 2; jj++) {
          ...
          for (int kk = -1; kk < 2; kk++) {
            ...
            if (s->with_zoom_region &&
              ...
              error(
                  "Trying to make hydro tasks for non-zoom cells while running "
                  "a "
            ...
            scheduler_addtask(sched, task_type_pair, task_subtype_density, sid,
                              0, ci, cj);
   ...
   * @param e the engine
   */
  void engine_addunlock_rt_advance_cell_time_tend(
      struct cell * c, struct task * tend, struct engine * e) {
  ...
  void engine_addtasks_send_mapper(void *map_data, int num_elements,
    ...
    for (int k = 0; k < num_elements; k++) {
      ...
      if (!cell_is_empty(ci)) {
        ...
        struct task *tend =
            scheduler_addtask(&e->sched, task_type_send, task_subtype_tend,
                              ci->mpi.tag, 0, ci, cj);
  ...
  void engine_make_fofloop_tasks_mapper(void *map_data, int num_elements,
    ...
    for (int ind = 0; ind < num_elements; ind++) {
      ...
      if (ci->nodeID == nodeID) {
        scheduler_addtask(sched, task_type_fof_self, task_subtype_none, 0, 0,
                          ci, NULL);
        scheduler_addtask(sched, task_type_fof_attach_self, task_subtype_none,
                          0, 0, ci, NULL);
      }
 ...
 * cell_set_super_mapper. */
    ...
    if (e->policy & engine_policy_hydro) {
       ...
       * in the pool. */
      engine_make_extra_hydroloop_tasks_mapper(sched->tasks, sched->nr_tasks,
                                               e);
