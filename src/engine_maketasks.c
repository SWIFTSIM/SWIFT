...
void engine_make_hierarchical_tasks_mapper(void *map_data, int num_elements,
  ...
  for (int ind = 0; ind < num_elements; ind++) {
    ...
    if (c->subtype == void_cell || c->subtype == empty) continue;
  ...
  for (int k = 0; k < num_elements; k++) {
    ...
    if ((e->policy & engine_policy_self_gravity) &&
      ...
      if (ci->type != zoom) {
        ...
        message("ci->type=%d void_c->type=%d void_c->subtype=%d void_c->depth=%d",
                ci->type, void_c->type, void_c->subtype, void_c->depth);
  ...
  if (e->policy & engine_policy_fof) {
    ...
    if (s->with_zoom_region) {
      threadpool_map(&e->threadpool, engine_make_fofloop_tasks_mapper_with_zoom,
                     NULL, s->zoom_props->bkg_cell_offset, 1,
...
/* #ifdef SWIFT_DEBUG_CHECKS */
/*         if (p->cells_out[k]->subtype == void_cell || */
/*             p->cells_out[k]->subtype == empty) */
