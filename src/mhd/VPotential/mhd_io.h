...
INLINE static void calculate_OW_trigger(const struct engine* e, const struct part* p,
  ...
  const float Adv_A[3] = {p->mhd_data.Adv_A_source[0], p->mhd_data.Adv_A_source[1],p->mhd_data.Adv_A_source[2]};

  const float Abs_Adv_A = sqrtf(Adv_A[0]*Adv_A[0]+Adv_A[1]*Adv_A[1]+Adv_A[2]*Adv_A[2]);

  const float Diff_A[3] = {p->mhd_data.Diff_A_source[0], p->mhd_data.Diff_A_source[1],p->mhd_data.Diff_A_source[2]};

  const float Abs_Diff_A = sqrtf(Diff_A[0]*Diff_A[0]+Diff_A[1]*Diff_A[1]+Diff_A[2]*Diff_A[2]);
 ...
 /* Accounting for advection direction*/
  ...
  const float Cos_Adv_Diff = (Diff_A[0]*Adv_A[0]+Diff_A[1]*Adv_A[1]+Diff_A[2]*Adv_A[2])/(Abs_Adv_A*Abs_Diff_A+FLT_MIN);

  const float sign_prefactor = 0.5*(1-Cos_Adv_Diff);

  /* Calculating ratio of local laplacian to largest resolvable laplacian*/

  const float Delta_A[3] = {p->mhd_data.Delta_A[0], p->mhd_data.Delta_A[1],
                      p->mhd_data.Delta_A[2]};

  const float Abs_Delta_A = sqrtf(Delta_A[0]*Delta_A[0]+Delta_A[1]*Delta_A[1]+Delta_A[2]*Delta_A[2]);

  const float A[3] = {p->mhd_data.APred[0], p->mhd_data.APred[1],
                      p->mhd_data.APred[2]};
  const float Aabs = sqrtf(A[0]*A[0]+A[1]*A[1]+A[2]*A[2]);

  const float Max_Delta_A = 2*Aabs/(p->h*p->h+FLT_MIN);

  const float Laplace_ratio = Abs_Delta_A/(Max_Delta_A+FLT_MIN);
...
INLINE static void calculate_effective_resistivity(const struct engine* e, const struct part* p,
                                const struct xpart* xp, float* ret) {
  ...
  const float effective_resistivity = Abs_Diff_A/(Abs_Delta_A+FLT_MIN);

  ret[0] = effective_resistivity;
}

INLINE static void calculate_Rm_local(const struct engine* e, const struct part* p,
                                const struct xpart* xp, float* ret) {
