...
INLINE static int mhd_write_particles(const struct part* parts,
  ...
  // SET CORRECT UNITS
  list[2] = io_make_output_field("VecPot", FLOAT, 3, UNIT_CONV_NO_UNITS,
                                 -3.f / 2.f * (hydro_gamma - 1.f) + 1.f, parts,
                                 mhd_data.APred,
  ...
  list[3] = io_make_output_field("Gau", FLOAT, 1, UNIT_CONV_NO_UNITS,
                                 -3.f / 2.f * (hydro_gamma - 1.f) + 2.f, parts,
                                 mhd_data.Gau, "Co-coving gauge scalar field");

  list[4] = io_make_output_field("divA", FLOAT, 1, UNIT_CONV_NO_UNITS,
                                 -3.f / 2.f * (hydro_gamma - 1.f), parts,
