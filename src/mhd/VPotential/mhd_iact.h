...
__attribute__((always_inline)) INLINE static void runner_iact_mhd_gradient(
  ...
  const float r_inv = r ? 1.0f / r : 0.0f;
  const float faci = pj->mass * wi_dx * r_inv;  // pi->rho;
  const float facj = pi->mass * wj_dx * r_inv;  // pj->rho;
  // const float faci = mj * wi_dx * r_inv;
...
__attribute__((always_inline)) INLINE static void runner_iact_mhd_force(
  ...
  for (int i = 0; i < 3; i++) {
    pi->mhd_data.dAdt[i] += mj * 0.5 * pi->mhd_data.Deta * mag_Disi * dA[i];
    pj->mhd_data.dAdt[i] -= mi * 0.5 * pj->mhd_data.Deta * mag_Disj * dA[i];
  }
  return;
}
