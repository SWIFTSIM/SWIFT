...
INLINE static int mhd_read_particles(struct part* parts,
  ...
  list[0] =
      io_make_input_field("MagneticFluxDensities", FLOAT, 3, COMPULSORY, UNIT_CONV_MAGNETIC_FIELD, parts, mhd_data.B);
...
INLINE static void convert_B(const struct engine* e, const struct part* p,
  ...
  ret[0] = p->mhd_data.B[0];
  ret[1] = p->mhd_data.B[1];
  ret[2] = p->mhd_data.B[2];
...
INLINE static int mhd_write_particles(const struct part* parts,
  ...
  list[1] = io_make_output_field(
      ...
      -1.5f * hydro_gamma - 1.f, parts, mhd_data.div_B,
