...
__attribute__((always_inline)) INLINE static void runner_iact_mhd_force(
  ...
  for (int k = 0; k < 3; k++) {
    sph_acc_term_mul_i += sph_acc_term_i[k]*Bj[k]/B2j;
    sph_acc_term_mul_j += sph_acc_term_j[k]*Bi[k]/B2i; 
  }
  for (int k = 0; k < 3; k++) {
    sph_acc_term_i[k] -= Bj[k]*sph_acc_term_mul_i; 
    sph_acc_term_j[k] -= Bi[k]*sph_acc_term_mul_j; 
  }*/
