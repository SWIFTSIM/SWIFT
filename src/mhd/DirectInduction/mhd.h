...
__attribute__((always_inline)) INLINE static float mhd_compute_timestep(
  ...
  const float dt_eta = p->mhd_data.resistive_eta+p->mhd_data.eta_OWAR != 0.f
                           ? hydro_properties->CFL_condition * cosmo->a *
                                 cosmo->a * p->h * p->h /
                                 (p->mhd_data.resistive_eta+0.01f*p->mhd_data.eta_OWAR)
                           : FLT_MAX;
...
__attribute__((always_inline)) INLINE static void mhd_end_gradient(
  ...
  p->mhd_data.eta_OWAR += 1.0f/OW * ( 0.5f * p->h * p->h / (absB+FLT_MIN)) * 0.5f * fabsf(1.0f-Cos_Ind_Diff) * Abs_Adv_B_source*(p->mass * kernel_root*ker_hdnv);
