...
#ifdef SWIFT_DEBUG_CHECKS
  if (ta == NULL)
    error("Unlocking task is NULL. (%s/%s)", taskID_names[tb->type],
          subtaskID_names[tb->subtype]);
  if (tb == NULL)
    error("Unlocked task is NULL. (%s/%s)", taskID_names[tb->type],
          subtaskID_names[tb->subtype]);
#endif
...
static void zoom_scheduler_splittask_gravity_void_pair(struct task *t,
  ...
  /* Pair interaction? */
    ...
    if (ci->subtype != cell_subtype_void && cj->subtype != cell_subtype_void &&
      ...
      /* Nothing to do if we got the zoom level and found a completely
       * foreign pair. */
      t->type = task_type_none;
      t->subtype = task_subtype_none;
      t->ci = NULL;
