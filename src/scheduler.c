...
void scheduler_addunlock(struct scheduler *s, struct task *ta,
  ...
  if (ta == NULL) {
    fprintf(stderr,
            "Unlocking task a is NULL. unlocked task t-type %s t-subtype %s",
            taskID_names[tb->type], subtaskID_names[tb->subtype]);
    error("Unlocking task is NULL. t-type %s t-subtype %s",
          taskID_names[tb->type], subtaskID_names[tb->subtype]);
    exit(0);
  }
  if (tb == NULL) {
    fprintf(stderr,
            "Unlocked task b is NULL. unlocking task t-type %s t-subtype %s",
            taskID_names[ta->type], subtaskID_names[ta->subtype]);
    error("Unlocked task is NULL. t-type %s t-subtype %s",
          taskID_names[ta->type], subtaskID_names[ta->subtype]);
    exit(0);
  }
#endif
...
void scheduler_write_dependencies(struct scheduler *s, int verbose) {
  ...
  if (s->nodeID == 0) {
    ...
    if (f == NULL)
      error("Error opening dependency graph file.");
    ...
    fprintf(
        ...
        "super,"
        "task_in_is_grav_super,task_out_is_top,task_out_is_hydro_super,"
        ...
        if (scheduler_dosub && (ci->hydro.count < space_subsize_self_hydro) &&
          ...
          for (int k = first_child + 1; k < 8; k++) {
            ...
            if (ci->progeny[k] != NULL &&
              ...
              scheduler_splittask_hydro(scheduler_addtask(s, task_type_self,
                                                          t->subtype, 0, 0,
                                                          ci->progeny[k], NULL),
                                        s);
            }
      ...
      if (cell_can_split_self_fof_task(ci)) {
        ...
        for (int k = first_child + 1; k < 8; k++)
          if (ci->progeny[k] != NULL && ci->progeny[k]->grav.count)
            scheduler_splittask_fof(scheduler_addtask(s, task_type_fof_self,
                                                      t->subtype, 0, 0,
                                                      ci->progeny[k], NULL),
                                    s);
...
void scheduler_splittasks_mapper(void *map_data, int num_elements,
  ...
  for (int ind = 0; ind < num_elements; ind++) {
    ...
    // if task is gpu task do not split
...
struct task *scheduler_addtask(struct scheduler *s, enum task_types type,
  ...
  /* Overflow? */
  if (ind >= s->size)
    error("Task list overflow (%d). Need to increase "
  ...
  if (ci != NULL)
    cell_set_flag(ci, cell_flag_has_tasks);
  if (cj != NULL)
    cell_set_flag(cj, cell_flag_has_tasks);
  //#ifdef WITH_CUDA
...
void scheduler_set_unlocks(struct scheduler *s) {
  ...
  for (int k = 0; k < s->nr_unlocks; k++) {
    //    fprintf(stderr,"in iteration %i of loop\n", k);
  ...
  for (int k = 0; k < s->nr_tasks; k++) {
    ...
    /* Check that we are not overflowing */
    if (offsets[k + 1] < 0)
      error("Task unlock offset array overflowing");
#endif
  }
  ...
  for (int k = nr_tasks - 1; k >= 0; k--) {
    ...
    case task_type_self:
      if (t->subtype == task_subtype_grav) {
        ...
        error("Untreated sub-type for selfs: %s", subtaskID_names[t->subtype]);
      break;
    case task_type_pair:
      if (t->subtype == task_subtype_grav) {
        ...
        // Abouzied: Think about good cost (for rainy days)
      } else if (t->subtype == task_subtype_gpu_pack) {
        ...
        error("Untreated sub-type for pairs: %s", subtaskID_names[t->subtype]);
      }
      break;
...
void scheduler_start(struct scheduler *s) {
  ...
  //  fprintf(stderr, "re-waiting done\n");
  ...
  for (int i = 0; i < s->nr_queues; i++)
    message("n tasks in queue %i are %i tasks_waiting %i", i,
            s->queues[i].count, s->waiting);
}
...
void scheduler_enqueue(struct scheduler *s, struct task *t) {
  ...
  /* Otherwise, look for a suitable queue. */
...
#ifdef WITH_MPI
    {
      ...
      snprintf(file_name_recv, sizeof(file_name_recv), "recv_log_r%d",
               s->nodeID);
      FILE *recv_logs;
      recv_logs = fopen(file_name_recv, "a");
      ...
      mpiuse_log_allocation(t->type, t->subtype, &t->req, 1, size,
                            t->ci->nodeID, t->flags);
      //#ifdef SWIFT_DEBUG_CHECKS
      //      fprintf(recv_logs, "%i, %i, %i, %i, %i, %i\n", t->ci->nodeID,
      //      t->cj->nodeID, size, t->ci->cellID, t->type, t->subtype);
      //#else
      fprintf(recv_logs, "%i,%i,%i,%s\n", t->ci->nodeID, size, t->ci->cellID,
              subtaskID_names[t->subtype]);
      //#endif
      fflush(recv_logs);
      fclose(recv_logs);
...
#ifdef WITH_MPI
    {
      char file_name_send[20];
      snprintf(file_name_send, sizeof(file_name_send), "send_log_r%d",
               s->nodeID);
      FILE *send_logs;
      send_logs = fopen(file_name_send, "a");
      size_t size = 0;              /* Size in bytes. */
      ...
      mpiuse_log_allocation(t->type, t->subtype, &t->req, 1, size,
                            t->cj->nodeID, t->flags);
      //#ifdef SWIFT_DEBUG_CHECKS
      //      fprintf(send_logs, "%i, %i, %i, %i, %i, %i\n", t->ci->nodeID,
      //      t->cj->nodeID, size, t->ci->cellID, t->type, t->subtype);
      //#else

      fprintf(send_logs, "%i,%i,%i,%s\n", t->ci->nodeID, size, t->ci->cellID,
              subtaskID_names[t->subtype]);
      //#endif
      fflush(send_logs);
      fclose(send_logs);
    ...
    //    if(t->subtype == task_subtype_gpu_pack)fprintf(stderr,"queue %i
    //    n_packs_is %i\n", qid, s->queues[qid].n_packs_left);
...
/**
 * @brief Take care of a tasks dependencies without unlocking cells
 * Not use at the moment -> only using signal_sleeping_runners and
 * enqueue dependencies separately
 * @param s The #scheduler.
 * @param t The finished #task.
 *
 * @return A pointer to the next task, if a suitable one has
