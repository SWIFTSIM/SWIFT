...
void scheduler_splittasks(struct scheduler *s, const int fof_tasks,
  ...
  if (verbose) {
    ...
    message("space_subsize_self_sidm= %d", space_subsize_self_sidm);
    message("space_subsize_pair_sidm= %d", space_subsize_pair_sidm);
  }
    ...
    switch (t->type) {
      ...
      case task_type_self:
        if (t->subtype == task_subtype_grav) {
          ...
          cost = 1.f * (wscale * sicount_i) * count_i;
      ...
      case task_type_pair:
        ...
        if (t->subtype == task_subtype_grav) {
          ...
          if (t->ci->nodeID != nodeID || t->cj->nodeID != nodeID) {
            cost = 3.f * (wscale * sicount_i) * sicount_j * sid_scale[t->flags];
          } else {
            cost = 2.f * (wscale * sicount_i) * sicount_j * sid_scale[t->flags];
          }
        } else if (t->subtype == task_subtype_do_gas_swallow) {
