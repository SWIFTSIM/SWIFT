...
struct gravity_cache {
   ...
   * since cell_remove_* overwrites the raw field to the particle's own id
   * on removal. */
  long long *restrict populated_id SWIFT_CACHE_ALIGN;

  /*! Per-index type as read by the last foreign populate call, since the
   * raw field is overwritten to swift_type_dark_matter on removal. */
...
INLINE static void gravity_cache_populate_foreign(
  ...
  /* Grow the cache if this foreign cell outgrew it. */
  if (c->count < gcount_padded) gravity_cache_init(c, gcount_padded + VEC_SIZE);
