...
/**
 * @brief Sort the sidm particles in the given cell along all cardinal
 * directions.
 *
 * @param r The #runner.
 * @param c The #cell.
 ...
 *      for recursive calls.
 */
void runner_do_sidm_sort(struct runner *r, struct cell *c, int flags,
                         int cleanup, int clock) {
...
#ifdef SWIFT_DEBUG_CHECKS
  if (c->sidm.super == NULL) error("Task called above the super level!!!");
#endif

  /* We need to do the local sorts plus whatever was requested further up. */
  flags |= c->sidm.do_sort;
  if (cleanup) {
    c->sidm.sorted = 0;
  } else {
    flags &= ~c->sidm.sorted;
  }
  if (flags == 0 && !cell_get_flag(c, cell_flag_do_sidm_sub_sort)) return;

  /* Check that the particles have been moved to the current time */
  if (flags && !cell_are_sipart_drifted(c, r->e)) {
    error("Sorting un-drifted cell c->nodeID=%d", c->nodeID);
  }

#ifdef SWIFT_DEBUG_CHECKS
  /* Make sure the sort flags are consistent (downward). */
  runner_check_sorts_sidm(c, c->sidm.sorted);

  /* Make sure the sort flags are consistent (upward). */
  for (struct cell *finger = c->parent; finger != NULL;
       finger = finger->parent) {
    if (finger->sidm.sorted & ~c->sidm.sorted)
      error("Inconsistent sort flags (upward).");
  }

  /* Update the sort timer which represents the last time the sorts
     were re-set. */
  if (c->sidm.sorted == 0) c->sidm.ti_sort = r->e->ti_current;
#endif

  /* start by allocating the entry arrays in the requested dimensions. */
  cell_malloc_sidm_sorts(c, flags);
  ...
  if (c->split) {
    ...
    for (int k = 0; k < 8; k++) {
      if (c->progeny[k] != NULL) {
        ...
        if (c->progeny[k]->sidm.count > 0) {

          /* Only propagate cleanup if the progeny is stale. */
          const int cleanup_prog =
              cleanup && (c->progeny[k]->sidm.dx_max_sort_old >
                          space_maxreldx * c->progeny[k]->dmin);
          runner_do_sidm_sort(r, c->progeny[k], flags, cleanup_prog, 0);
          dx_max_sort = max(dx_max_sort, c->progeny[k]->sidm.dx_max_sort);
          dx_max_sort_old =
              max(dx_max_sort_old, c->progeny[k]->sidm.dx_max_sort_old);
        } else {

          /* We need to clean up the unused flags that were in case the
             number of particles in the cell would change */
          cell_clear_sidm_sort_flags(c->progeny[k], /*clear_unused_flags=*/1);
        }
      }
    }
    c->sidm.dx_max_sort = dx_max_sort;
    c->sidm.dx_max_sort_old = dx_max_sort_old;
    ...
    for (int j = 0; j < 13; j++) {
      ...
      for (int k = 1; k < 8; k++)
        if (c->progeny[k - 1] != NULL)
          off[k] = off[k - 1] + c->progeny[k - 1]->sidm.count;
        else
          off[k] = off[k - 1];
      ...
      for (int k = 0; k < 8; k++) {
        ...
        if (c->progeny[k] != NULL && c->progeny[k]->sidm.count > 0) {
          fingers[k] = cell_get_sidm_sorts(c->progeny[k], j);
          buff[k] = fingers[k]->d;
          off[k] = off[k];
        } else
          buff[k] = FLT_MAX;
      ...
      /* For each entry in the new sort list. */
      struct sort_entry *finger = cell_get_sidm_sorts(c, j);
      for (int ind = 0; ind < count; ind++) {
      ...
      /* Add a sentinel. */
      struct sort_entry *entries = cell_get_sidm_sorts(c, j);
      entries[count].d = FLT_MAX;
      entries[count].i = 0;

      /* Mark as sorted. */
      atomic_or(&c->sidm.sorted, 1 << j);
  ...
  /* Otherwise, just sort. */
    ...
    if (c->sidm.sorted == 0) {
      ...
      /* And the individual sort distances if we are a local cell */
      for (int k = 0; k < count; k++) {
        siparts[k].x_diff_sort[0] = 0.0f;
        siparts[k].x_diff_sort[1] = 0.0f;
        siparts[k].x_diff_sort[2] = 0.0f;
      }
      c->sidm.dx_max_sort_old = 0.f;
      c->sidm.dx_max_sort = 0.f;
    }

    /* Fill the sort array. */
    for (int k = 0; k < count; k++) {
      const double px[3] = {siparts[k].x[0], siparts[k].x[1], siparts[k].x[2]};
      for (int j = 0; j < 13; j++)
        if (flags & (1 << j)) {
          struct sort_entry *entries = cell_get_sidm_sorts(c, j);
          entries[k].i = k;
  ...
  /* Make sure the sort flags are consistent (downward). */
  runner_check_sorts_sidm(c, flags);
  ...
  if (clock) TIMER_TOC(timer_do_sidm_sort);
}
