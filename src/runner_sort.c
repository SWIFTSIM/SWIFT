...
/**
 * @brief Sorts again all the sinks in a given cell hierarchy.
 *
 * This is intended to be used after the star formation task has been run
 * to get the cells back into a state where self/pair star tasks can be run.
 *
...
void runner_do_sink_resort(struct runner *r, struct cell *c, const int timer) {
  ...
  /* Did we demand a recalculation of the sinks'sorts? */
  if (cell_get_flag(c, cell_flag_do_sink_resort)) {
    runner_do_all_sink_sort(r, c);
    cell_clear_flag(c, cell_flag_do_sink_resort);
  }

  if (timer) TIMER_TOC(timer_do_sink_resort);
}
...
/**
 * @brief Sort the sinks particles in the given cell along all cardinal
 * directions.
 *
 * @param r The #runner.
 * @param c The #cell.
 ...
 *      for recursive calls.
 */
void runner_do_sink_sort(struct runner *r, struct cell *c, int flags,
                         int cleanup, int clock) {
  ...
  /* We need to do the local sorts plus whatever was requested further up. */
  flags |= c->sinks.do_sort;
  if (cleanup) {
    c->sinks.sorted = 0;
  } else {
    flags &= ~c->sinks.sorted;
  }
...
#ifdef SWIFT_DEBUG_CHECKS
  /* Make sure the sort flags are consistent (downward). */
  runner_check_sorts_sinks(c, c->sinks.sorted);

  /* Make sure the sort flags are consistent (upward). */
  for (struct cell *finger = c->parent; finger != NULL;
       finger = finger->parent) {
    if (finger->sinks.sorted & ~c->sinks.sorted)
      error("Inconsistent sort flags (upward).");
  }

  /* Update the sort timer which represents the last time the sorts
     were re-set. */
  if (c->sinks.sorted == 0) c->sinks.ti_sort = r->e->ti_current;
#endif

  /* start by allocating the entry arrays in the requested dimensions. */
  cell_malloc_sinks_sorts(c, flags);
  ...
  if (c->split) {
    ...
    for (int k = 0; k < 8; k++) {
      if (c->progeny[k] != NULL) {
        ...
        if (c->progeny[k]->sinks.count > 0) {

          /* Only propagate cleanup if the progeny is stale. */
          const int cleanup_prog =
              cleanup && (c->progeny[k]->sinks.dx_max_sort_old >
                          space_maxreldx * c->progeny[k]->dmin);
          runner_do_sink_sort(r, c->progeny[k], flags, cleanup_prog, 0);
          dx_max_sort = max(dx_max_sort, c->progeny[k]->sinks.dx_max_sort);
          dx_max_sort_old =
              max(dx_max_sort_old, c->progeny[k]->sinks.dx_max_sort_old);
        } else {

          /* We need to clean up the unused flags that were in case the
             number of particles in the cell would change */
          cell_clear_sinks_sort_flags(c->progeny[k], /*clear_unused_flags=*/1);
        }
      }
    }
    c->sinks.dx_max_sort = dx_max_sort;
    c->sinks.dx_max_sort_old = dx_max_sort_old;
    ...
    for (int j = 0; j < 13; j++) {
      ...
      for (int k = 1; k < 8; k++)
        if (c->progeny[k - 1] != NULL)
          off[k] = off[k - 1] + c->progeny[k - 1]->sinks.count;
        else
          off[k] = off[k - 1];
      ...
      for (int k = 0; k < 8; k++) {
        ...
        if (c->progeny[k] != NULL && c->progeny[k]->sinks.count > 0) {
          fingers[k] = cell_get_sinks_sorts(c->progeny[k], j);
          buff[k] = fingers[k]->d;
          off[k] = off[k];
        } else
          buff[k] = FLT_MAX;
      ...
      /* For each entry in the new sort list. */
      struct sort_entry *finger = cell_get_sinks_sorts(c, j);
      for (int ind = 0; ind < count; ind++) {
      ...
      /* Add a sentinel. */
      struct sort_entry *entries = cell_get_sinks_sorts(c, j);
      entries[count].d = FLT_MAX;
      entries[count].i = 0;

      /* Mark as sorted. */
      atomic_or(&c->sinks.sorted, 1 << j);
  ...
  /* Otherwise, just sort. */
    ...
    if (c->sinks.sorted == 0) {
      ...
      /* And the individual sort distances if we are a local cell */
      for (int k = 0; k < count; k++) {
        sinks[k].x_diff_sort[0] = 0.0f;
        sinks[k].x_diff_sort[1] = 0.0f;
        sinks[k].x_diff_sort[2] = 0.0f;
      }
      c->sinks.dx_max_sort_old = 0.f;
      c->sinks.dx_max_sort = 0.f;
    }

    /* Fill the sort array. */
    for (int k = 0; k < count; k++) {
      const double px[3] = {sinks[k].x[0], sinks[k].x[1], sinks[k].x[2]};
      for (int j = 0; j < 13; j++)
        if (flags & (1 << j)) {
          struct sort_entry *entries = cell_get_sinks_sorts(c, j);
          entries[k].i = k;
  ...
  /* Make sure the sort flags are consistent (downward). */
  runner_check_sorts_sinks(c, flags);
  ...
  if (clock) TIMER_TOC(timer_do_sinks_sort);
}
...
void runner_do_all_sink_sort(struct runner *r, struct cell *c) {
  ...
  if (!cell_is_active_sinks(c, r->e) && !cell_is_active_hydro(c, r->e)) return;

  /* Shall we sort at this level? */
  if (c->hydro.super == c) {

    /* Sort everything */
    runner_do_sink_sort(r, c, 0x1FFF, /*cleanup=*/0, /*timer=*/0);
    ...
    if (c->split) {
      for (int k = 0; k < 8; ++k) {
        if (c->progeny[k] != NULL) runner_do_all_sink_sort(r, c->progeny[k]);
      }
    } else {
#ifdef SWIFT_DEBUG_CHECKS
      error("Reached a leaf without encountering a hydro super cell!");
