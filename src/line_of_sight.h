 ...
 *
 * @param x 1D coordinates of particles.
...
void do_line_of_sight(struct engine *e) {
  ...
  for (int j = 0; j < LOS_params.num_tot; j++) {
          ...
          if (dy <= hsml) {
            r2 = dx * dx + dy * dy;
              ...
              if (r2 <= hsml * hsml) {
                ...
                /* We've found one, tag it. */
                LOS_list[j].particles_in_los_local += 1;
                s->parts[i].gpart->type = swift_type_los;
              }
          }
        }
      }
    ...
    for (size_t i = 0; i < nr_parts; i++) {
      ...
      /* If the gas particle is tagged in this LOS. */
      if (s->parts[i].gpart->type == swift_type_los) {
        ...
        /* Untag gas particle. */
        s->parts[i].gpart->type = swift_type_gas;
      ...
      if (e->nodeID == 0) {
        if (k == 0) {
          ...
          if (LOS_counts[k] > 0) {
            ...
            MPI_Recv(LOS_particles,
                 sizeof(struct line_of_sight_particles) * LOS_counts[k],
