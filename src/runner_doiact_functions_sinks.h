...
void DO_NONSYM_PAIR1_SINKS_NAIVE(struct runner *r, struct cell *restrict ci,
  ...
  for (int sid = 0; sid < scount_i; sid++) {
    ...
    for (int sjd = 0; sjd < scount_j; sjd++) {
      ...
      /* Check that particles have been drifted to the current time */
      if (si->ti_drift != e->ti_current) {
	warning("1 %lld ci->sinks.density = %p %i", ci->cellID, ci->sinks.density, ci->nodeID);
	warning("1 %lld ci.hydro->super->sinks.density = %p, %i", ci->hydro.super->cellID, ci->hydro.super->sinks.density, ci->hydro.super->nodeID);
	warning("1 %lld ci.hydro->super->sinks.density = %p %i", ci->top->cellID, ci->top->sinks.density, ci->top->nodeID);

	warning("1 %lld cj->sinks.density = %p, %i", cj->cellID, cj->sinks.density, cj->nodeID);
	warning("1 %lld cj.hydro->super->sinks.density = %p, %i", cj->hydro.super->cellID, cj->hydro.super->sinks.density, cj->hydro.super->nodeID);
	warning("1 %lld, cj.hydro->super->sinks.density = %p, %i", cj->top->cellID, cj->top->sinks.density, cj->top->nodeID);

	warning("1.1 --------------------");
	if (ci->hydro.super->sinks.density != NULL) {
	  for (struct link *l = ci->hydro.super->sinks.density; l != NULL; l = l->next) {
	    struct task *t = l->t;
	    struct cell *ck = t->ci;
	    struct cell *cl = t->cj;
	    const int ck_active =
	      cell_is_active_sinks(ck, e) || cell_is_active_hydro(ck, e);
	    const int cl_active = (cl != NULL) && (cell_is_active_sinks(cl, e) ||
						   cell_is_active_hydro(cl, e));
	    if (cl != NULL) {
	      message("[1, %lld, %lld, %d, %d] Check existence of sinks.density, t = %p, t->flags = %lld, type = %d, subtype = %d, ck_active = %d, ck_active = %d, t->skip = %d",
		      ck->cellID, cl->cellID, ck->nodeID, cl->nodeID, t, t->flags, t->type, t->subtype, ck_active, cl_active, t->skip);
	    } else {
	      message("[1, %lld, %d] Check existence of sinks.density, t = %p, t->flags = %lld, type = %d, subtype = %d, ck_active = %d, t->skip = %d",
		      ck->cellID, ck->nodeID, t, t->flags, t->type, t->subtype, ck_active, t->skip);
	    }
	  }
	}
	warning("1.2 --------------------");
	...
	if (cj->hydro.super->sinks.density != NULL) {
	  for (struct link *l = cj->hydro.super->sinks.density; l != NULL; l = l->next) {
	    struct task *t = l->t;
	    struct cell *ck = t->ci;
	    struct cell *cl = t->cj;
	    const int ck_active =
	...
	sleep(10);
        error("Particle si not drifted to current time. si->id = %lld, sj->id = %lld | i: hydro super = %lld, grav super = %lld,"
	      " top = %lld, c = %lld, nodeID = %d | j hydro super = %lld, grav super = %lld,"
      ...
      if (sj->ti_drift != e->ti_current) {
	warning("2 %lld ci->sinks.density = %p %i", ci->cellID, ci->sinks.density, ci->nodeID);
	warning("2 %lld ci.hydro->super->sinks.density = %p, %i", ci->hydro.super->cellID, ci->hydro.super->sinks.density, ci->hydro.super->nodeID);
	warning("2 %lld ci.hydro->super->sinks.density = %p %i", ci->top->cellID, ci->top->sinks.density, ci->top->nodeID);

	warning("2 %lld cj->sinks.density = %p, %i", cj->cellID, cj->sinks.density, cj->nodeID);
	warning("2 %lld cj.hydro->super->sinks.density = %p, %i", cj->hydro.super->cellID, cj->hydro.super->sinks.density, cj->hydro.super->nodeID);
	warning("2 %lld, cj.hydro->super->sinks.density = %p, %i", cj->top->cellID, cj->top->sinks.density, cj->top->nodeID);
	...
	if (ci->hydro.super->sinks.density != NULL) {
	  for (struct link *l = ci->hydro.super->sinks.density; l != NULL; l = l->next) {
	    ...
	    const int cl_active = (cl != NULL) && (cell_is_active_sinks(cl, e) ||
						   cell_is_active_hydro(cl, e));
	    if (cl != NULL) {
	      warning("[2, %lld, %lld, %d, %d] Check existence of sinks.density, t = %p, t->flags = %lld, type = %d, subtype = %d, ck_active = %d, ck_active = %d, t->skip = %d",
		      ck->cellID, cl->cellID, ck->nodeID, cl->nodeID, t, t->flags, t->type, t->subtype, ck_active, cl_active, t->skip);
	    } else {
	      warning("[2, %lld, %d] Check existence of sinks.density, t = %p, t->flags = %lld, type = %d, subtype = %d, ck_active = %d, t->skip = %d",
		      ck->cellID, ck->nodeID, t, t->flags, t->type, t->subtype, ck_active, t->skip);
	    }
	  }
	}
