 ...
 *
 ******************************************************************************/
#ifndef SWIFT_GEAR_BH_IACT_H
#define SWIFT_GEAR_BH_IACT_H
...
runner_iact_nonsym_bh_gas_density(
  ...
  switch (bh_props->feedback_model) {
    case AGN_isotropic_model: {
      ...
      /* Loop over rays */
      for (int i = 0; i < gear_blackhole_number_of_rays; i++) {
    ...
    case AGN_minimum_distance_model: {
       ...
       * number of rays. */
      const int arr_size = min(bi->num_ngbs, gear_blackhole_number_of_rays);
