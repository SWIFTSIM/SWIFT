 ...
 *
 ******************************************************************************/
#ifndef SWIFT_GEAR_BLACK_HOLES_PROPERTIES_H
#define SWIFT_GEAR_BLACK_HOLES_PROPERTIES_H
...
enum BH_merger_thresholds {
  BH_mergers_circular_velocity,        /*< v_circ at separation, as in GEAR */
...
INLINE static void black_holes_props_init(struct black_holes_props *bp,
  ...
  bp->subgrid_seed_mass =
      parser_get_param_float(params, "GEARAGN:subgrid_seed_mass_Msun");

  /* Convert to internal units */
  bp->subgrid_seed_mass *= phys_const->const_solar_mass;

  bp->use_subgrid_mass_from_ics =
      parser_get_opt_param_int(params, "GEARAGN:use_subgrid_mass_from_ics", 1);
  if (bp->use_subgrid_mass_from_ics)
    bp->with_subgrid_mass_check =
        parser_get_opt_param_int(params, "GEARAGN:with_subgrid_mass_check", 1);
  ...
  if (bp->with_angmom_limiter)
    bp->alpha_visc = parser_get_param_float(params, "GEARAGN:viscous_alpha");

  bp->epsilon_r =
      parser_get_param_float(params, "GEARAGN:radiative_efficiency");
  if (bp->epsilon_r > 1.f)
    error("GEARAGN:radiative_efficiency must be <= 1, not %f.", bp->epsilon_r);

  bp->f_Edd = parser_get_param_float(params, "GEARAGN:max_eddington_fraction");
  bp->f_Edd_recording = parser_get_param_float(
      params, "GEARAGN:eddington_fraction_for_recording");

  /*  Booth & Schaye (2009) Parameters */
  bp->with_boost_factor =
      parser_get_param_int(params, "GEARAGN:with_boost_factor");

  if (bp->with_boost_factor) {
    bp->boost_alpha_only =
        parser_get_param_int(params, "GEARAGN:boost_alpha_only");
    bp->boost_alpha = parser_get_param_float(params, "GEARAGN:boost_alpha");

    if (!bp->boost_alpha_only) {
      bp->boost_beta = parser_get_param_float(params, "GEARAGN:boost_beta");

      /* Load the density in cgs and convert to internal units */
      bp->boost_n_h_star =
          parser_get_param_float(params, "GEARAGN:boost_n_h_star_H_p_cm3") /
          units_cgs_conversion_factor(us, UNIT_CONV_NUMBER_DENSITY);
    }
  }

  bp->use_nibbling = parser_get_param_int(params, "GEARAGN:use_nibbling");
  if (bp->use_nibbling) {
    bp->min_gas_mass_for_nibbling = parser_get_param_float(
        params, "GEARAGN:min_gas_mass_for_nibbling_Msun");
  ...
  bp->with_fixed_T_near_EoS =
      parser_get_param_int(params, "GEARAGN:with_fixed_T_near_EoS");
  if (bp->with_fixed_T_near_EoS) {
    bp->fixed_T_above_EoS_factor =
        exp10(parser_get_param_float(params, "GEARAGN:fixed_T_above_EoS_dex"));
    bp->fixed_u_for_soundspeed =
        parser_get_param_float(params, "GEARAGN:fixed_T_near_EoS_K") /
  ...
  char temp[40];
  parser_get_param_string(params, "GEARAGN:AGN_feedback_model", temp);
  ...
  bp->AGN_delta_T_desired =
      parser_get_param_float(params, "GEARAGN:AGN_delta_T_K");

  /* Read the properties of the variable heating temperature model */
  bp->use_variable_delta_T =
      parser_get_param_int(params, "GEARAGN:use_variable_delta_T");
  if (bp->use_variable_delta_T) {
    bp->AGN_delta_T_mass_norm =
        parser_get_param_float(params, "GEARAGN:AGN_delta_T_mass_norm") *
        T_K_to_int;
    bp->AGN_delta_T_mass_reference =
        parser_get_param_float(params, "GEARAGN:AGN_delta_T_mass_reference") *
        phys_const->const_solar_mass;
    bp->AGN_delta_T_mass_exponent =
        parser_get_param_float(params, "GEARAGN:AGN_delta_T_mass_exponent");

    bp->AGN_with_locally_adaptive_delta_T = parser_get_param_int(
        params, "GEARAGN:AGN_with_locally_adaptive_delta_T");
    if (bp->AGN_with_locally_adaptive_delta_T) {
      bp->AGN_delta_T_crit_factor =
          parser_get_param_float(params, "GEARAGN:AGN_delta_T_crit_factor");
      bp->AGN_delta_T_background_factor = parser_get_param_float(
          params, "GEARAGN:AGN_delta_T_background_factor");
    }

    bp->AGN_delta_T_max =
        parser_get_param_float(params, "GEARAGN:AGN_delta_T_max") * T_K_to_int;
    bp->AGN_delta_T_min =
        parser_get_param_float(params, "GEARAGN:AGN_delta_T_min") * T_K_to_int;
    bp->AGN_use_nheat_with_fixed_dT =
        parser_get_param_int(params, "GEARAGN:AGN_use_nheat_with_fixed_dT");
  }
  ...
  bp->num_ngbs_to_heat =
      parser_get_param_float(params, "GEARAGN:AGN_num_ngb_to_heat");

  /* Reposition parameters --------------------------------- */

  bp->max_reposition_mass =
      parser_get_param_float(params, "GEARAGN:max_reposition_mass") *
      phys_const->const_solar_mass;
  bp->max_reposition_distance_ratio =
      parser_get_param_float(params, "GEARAGN:max_reposition_distance_ratio");
  ...
  if (bp->with_reposition_velocity_threshold) {
    ...
    bp->min_reposition_velocity_threshold = parser_get_param_float(
        params, "GEARAGN:min_reposition_velocity_threshold");
  ...
  bp->set_reposition_speed =
      parser_get_param_int(params, "GEARAGN:set_reposition_speed");

  if (bp->set_reposition_speed) {
    bp->reposition_coefficient_upsilon = parser_get_param_float(
        params, "GEARAGN:reposition_coefficient_upsilon");
    ...
    bp->reposition_reference_mass =
        parser_get_param_float(params, "GEARAGN:reposition_reference_mass") *
        phys_const->const_solar_mass;
    bp->reposition_exponent_mass = parser_get_opt_param_float(
        params, "GEARAGN:reposition_exponent_mass", 2.0);
    bp->reposition_reference_n_H =
        parser_get_param_float(params, "GEARAGN:reposition_reference_n_H");
    bp->reposition_exponent_n_H = parser_get_opt_param_float(
        params, "GEARAGN:reposition_exponent_n_H", 1.0);
  }

  bp->correct_bh_potential_for_repositioning =
      parser_get_param_int(params, "GEARAGN:with_potential_correction");

  /* Merger parameters ------------------------------------- */

  bp->minor_merger_threshold =
      parser_get_param_float(params, "GEARAGN:threshold_minor_merger");

  bp->major_merger_threshold =
      parser_get_param_float(params, "GEARAGN:threshold_major_merger");

  char temp2[40];
  parser_get_param_string(params, "GEARAGN:merger_threshold_type", temp2);
  ...
  bp->max_merging_distance_ratio =
      parser_get_param_float(params, "GEARAGN:merger_max_distance_ratio");
  ...
  const double time_step_min_Myr = parser_get_opt_param_float(
      params, "GEARAGN:minimum_timestep_Myr", FLT_MAX);
...
#endif /* SWIFT_GEAR_BLACK_HOLES_PROPERTIES_H */
