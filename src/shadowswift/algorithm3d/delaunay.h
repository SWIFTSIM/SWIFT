...
inline static int delaunay_find_tetrahedra_containing_vertex(
  ...
  while (int_lifo_queue_is_empty(&d->tetrahedra_containing_vertex)) {
    ...
    int test_adbe = geometry3d_orient_adaptive(&d->geometry, al, dl, bl, el, ad,
                                               dd, bd, ed);
    if (test_adbe > 0) {
      tests |= 1 << 2;
    }
    int test_abce = geometry3d_orient_adaptive(&d->geometry, al, bl, cl, el, ad,
                                               bd, cd, ed);
    if (test_abce > 0) {
      tests |= 1 << 3;
    }
    switch (tests) {
      ...
      case 0:
        ...
        break;
      default:
        error("Impossible tetrahedron test result: %n", &tests);
    }
...
inline static void delaunay_get_search_radii(struct delaunay* restrict d,
  ...
  for (int i = 0; i < count; i++) {
    int gen_idx_in_d = pid[i] + d->vertex_start;
