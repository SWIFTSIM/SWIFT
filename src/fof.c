...
void fof_init(struct space *s, long long Ngas, long long Ngparts, long long Nstars) {
  ...
  const int default_id = parser_get_opt_param_int(e->parameter_file, "FOF:group_id_default", 2147483647);

  /* Make sure default group ID is positive. */
  if(default_id < 0) error("The default group ID set: %d, has to be positive.", default_id);
...
__attribute__((always_inline)) INLINE static size_t fof_find_global(const size_t i,
                                                          size_t *group_index) {
...
__attribute__((always_inline)) INLINE static size_t fof_find(const size_t i,
                                                          size_t *group_index) {
...
static void rec_fof_search_pair_foreign(struct cell *ci, struct cell *cj, struct space *s,
                           const double *dim,
                           const double search_r2, int *link_count, struct fof_mpi **group_links, int *group_links_size) {
  ...
  if (ci->split && cj->split) {
    for (int k = 0; k < 8; k++) {
      if (ci->progeny[k] != NULL) {
        ...
        for (int l = 0; l < 8; l++)
          if (cj->progeny[l] != NULL)
            rec_fof_search_pair_foreign(ci->progeny[k], cj->progeny[l], s, dim, search_r2, link_count, group_links, group_links_size);
      }
    }
  }
  /* Perform FOF search between pairs of cells that are within the linking
...
void fof_search_pair_cells_foreign(struct space *s, struct cell *ci, struct cell *cj, int *link_count, struct fof_mpi **group_links, int *group_links_size) {
  ...
  if(ci_local) {
    ...
    for (size_t i = 0; i < count_i; i++) {
      ...
      /* Find the root of pi. */
      const size_t root_i = fof_find_global(offset_i[i] - node_offset, group_index);
          ...
          if(!found) {
            ...
            if(*link_count + 1 > *group_links_size) {
              ...
              *group_links_size = new_size;
              ...
              message("Re-allocating local group links from %d to %d elements.", *link_count, new_size);
            }
...
void fof_find_foreign_links_mapper(void *map_data, int num_elements,
  ...
  struct fof_mpi *local_group_links = (struct fof_mpi*)calloc(sizeof(struct fof_mpi), local_group_links_size);
  ...
  for (int ind = 0; ind < num_elements; ind++) {
    ...
    rec_fof_search_pair_foreign(local_cell, foreign_cell, s, dim, search_r2, &local_link_count, &local_group_links, &local_group_links_size);
  ...
  if (lock_lock(&s->lock) == 0) {
    ...
    if(*group_link_count + local_link_count > *group_links_size) {
      ...
      *group_links_size = new_size;
      ...
      message("Re-allocating global group links from %d to %d elements.", *group_link_count, new_size);
    }
...
void fof_search_tree(struct space *s) {
  ...
  if(nr_nodes == 1) {
    ...
    for(size_t i=0; i<num_groups_local; i++) gparts[high_group_sizes[i].index].group_id = group_id_offset + i;
