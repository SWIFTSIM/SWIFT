...
/**
 * @brief Recurse down foreign cells until reaching one with dark matter
 * tasks; then trigger the linking of the #dmpart array from that
 * level.
 *
 * @param c The #cell.
 * @param dmparts The #dmpart array.
...
void cell_sanitize(struct cell *c, int treated) {
  ...
  if (c->split) {
    ...
    for (int i = 0; i < dmcount; ++i) dark_matter_h_max_active = max(dark_matter_h_max_active, dmparts[i].h);
  }
...
/**
 * @brief Checks that the #dmpart in a cell are at the
 * current point in time
 *
 * Calls error() if the cell is not at the current time.
 *
 ...
 *
 * @param c The top-level #cell to play with.
 * @param super_dark_matter Pointer to the deepest cell with tasks in this part of
 * the tree.
 */
...
/**
 * @brief Resets all the sorting properties for the dark_matter in a given cell
 * hierarchy.
 *
 * The clear_unused_flags argument can be used to additionally clean up all
 * the flags demanding a sort for the given cell. This should be used with
 ...
 * @param clear_unused_flags Do we also clean the flags demanding a sort?
 */
void cell_clear_dark_matter_sort_flags(struct cell *c, const int clear_unused_flags) {
    ...
    if (c->split) {
        for (int k = 0; k < 8; k++)
            if (c->progeny[k] != NULL)
                cell_clear_dark_matter_sort_flags(c->progeny[k], clear_unused_flags);
    }
}
...
void cell_check_timesteps(const struct cell *c, const integertime_t ti_current,
    ...
    if (c->hydro.ti_end_min == 0 && c->grav.ti_end_min == 0 &&
      ...
      c->sinks.ti_end_min == 0 && c->rt.ti_rt_end_min == 0 &&
      c->dark_matter.ti_end_min == 0 && c->nr_tasks > 0)
...
void cell_check_sort_flags(const struct cell *c) {
  ...
  const int do_dark_matter_sub_sort = cell_get_flag(c, cell_flag_do_dark_matter_sub_sort);
  ...
  if (do_dark_matter_sub_sort)
    error(
        "cell %lld has a dmparts sub_sort flag set. Node=%d depth=%d maxdepth=%d",
        c->cellID, c->nodeID, c->depth, c->maxdepth);
