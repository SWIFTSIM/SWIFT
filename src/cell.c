 ...
 * are slelf-complete, the Voronoi grid of that cell can completely be
 * constructed using only particles of this cell and its direct neigbours,
  ...
  if (cj == NULL) {
    ...
    if (ci_local) {
      atomic_and(&ci->grid.complete,
                 !cell_pair_invalidates_completeness(ci, cj));
    }
    if (cj_local) {
      atomic_and(&cj->grid.complete,
                 !cell_pair_invalidates_completeness(cj, ci));
    }
  }
}
 ...
 *
 * A cell is consisdered complete if it and all its neighbours are
