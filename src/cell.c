...
/**
 * @brief Construct the void cell multipole.
 *
...
void cell_make_void_multipole(struct space *s, struct cell *c, integertime_t ti_current,
  ...
  if (s->nr_gparts > 0) {
    ...
    if (swift_memalign("void_parts", (void **)&void_parts, gpart_align,
      ...
      error("Failed to allocate array for void cell particles.");
    size_t nr_void_parts = 0;
    ...
    for (int k = 0; k < s->zoom_props->nr_zoom_cells; ++k) {
      ...
      for (int i = 0; i < c_zoom->grav.count; ++i) {
        struct gpart void_gpart = c_zoom->grav.parts[i];
        void_parts[nr_void_parts] = void_gpart;
        nr_void_parts++;

      }
    }

    /* Interact particles with multipole */
    gravity_P2M(c->grav.multipole, void_parts, nr_void_parts, grav_props);
...
//  /* Compute CoM from all zoom cells */
//  double CoM[3] = {0., 0., 0.};
//  double vel[3] = {0., 0., 0.};
//  float max_delta_vel[3] = {0.f, 0.f, 0.f};
//  float min_delta_vel[3] = {0.f, 0.f, 0.f};
...
//  for (int k = 0; k < s->zoom_props->nr_zoom_cells; ++k) {
//    const struct cell *c_zoom = &cells[k];
//    const struct multipole *m = &c_zoom->grav.multipole->m_pole;
//
//    /* Contribution to multipole */
//    gravity_M2M(&temp, m, c->grav.multipole->CoM, c_zoom->grav.multipole->CoM);
//    gravity_multipole_add(&c->grav.multipole->m_pole, &temp);
//
//    /* Upper limit of max CoM<->gpart distance */
//    const double dx =
//        c->grav.multipole->CoM[0] - c_zoom->grav.multipole->CoM[0];
//    const double dy =
//        c->grav.multipole->CoM[1] - c_zoom->grav.multipole->CoM[1];
//    const double dz =
//        c->grav.multipole->CoM[2] - c_zoom->grav.multipole->CoM[2];
//    const double r2 = dx * dx + dy * dy + dz * dz;
