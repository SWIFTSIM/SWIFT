 ...
 *
 ******************************************************************************/
#ifndef SWIFT_DEFAULT_SIDM_IO_H
#define SWIFT_DEFAULT_SIDM_IO_H

#include "io_properties.h"
#include "sidm_part.h"

/**
 * @brief Specifies which si-particle fields to read from a dataset
 *
...
INLINE static void convert_sipart_pos(const struct engine* e,
  ...
  if (s->periodic) {
    ...
    ret[2] = box_wrap(sip->x[2], 0.0, s->dim[2]);
  } else {
    ret[0] = sip->x[0];
    ret[1] = sip->x[1];
    ret[2] = sip->x[2];
...
/**
 * @brief Specifies which si-particle fields to write to a dataset
 *
 * @param siparts The si-particle array.
