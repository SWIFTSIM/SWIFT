...
__attribute__((always_inline)) INLINE static void rt_finalise_transport(
  ...
  for (int g = 0; g < RT_NGROUPS; g++) {
    ...
    //rtd->radiation[g].energy_density -= rtd->radiation[g].energy_density * 3.f * cosmo->a_dot * dt * powl(cosmo->a,2); // dilution

    rtd->radiation[g].flux[0] += rtd->flux[g].flux[0] * Vinv;
    //rtd->radiation[g].flux[0] -= rtd->radiation[g].flux[0] * 3.f * cosmo->a_dot * dt * powl(cosmo->a,2); // dilution
    
    rtd->radiation[g].flux[1] += rtd->flux[g].flux[1] * Vinv;
    //rtd->radiation[g].flux[1] -= rtd->radiation[g].flux[1] * 3.f * cosmo->a_dot * dt * powl(cosmo->a,2); // dilution
    
    rtd->radiation[g].flux[2] += rtd->flux[g].flux[2] * Vinv;
    //rtd->radiation[g].flux[2] -= rtd->radiation[g].flux[2] * 3.f * cosmo->a_dot * dt * powl(cosmo->a,2); // dilution 
