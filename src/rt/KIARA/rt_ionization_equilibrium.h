...
/**
 * @file src/rt/KIARA/rt_ionization_equilibrium.h
...
rt_ion_equil_mass_fractions_from_T(double T, float X, float Y, float* XHI,
  ...
  if (X <= RT_KIARA_TINY_MASS_FRACTION) {
    nH = RT_KIARA_TINY_MASS_FRACTION;
    nHe = 1.;
  } else {
    nH = X;
    nHe = 0.25 * Y;
  ...
  *XHI = max(*XHI, RT_KIARA_TINY_MASS_FRACTION);
  *XHII = (float)(mHII * mtot_inv);
  *XHII = max(*XHII, RT_KIARA_TINY_MASS_FRACTION);
  *XHeI = (float)(mHeI * mtot_inv);
  *XHeI = max(*XHeI, RT_KIARA_TINY_MASS_FRACTION);
  *XHeII = (float)(mHeII * mtot_inv);
  *XHeII = max(*XHeII, RT_KIARA_TINY_MASS_FRACTION);
  *XHeIII = (float)(mHeIII * mtot_inv);
  *XHeIII = max(*XHeIII, RT_KIARA_TINY_MASS_FRACTION);
}
