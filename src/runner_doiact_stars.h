...
void DOSELF1_STARS(struct runner *r, struct cell *c, int timer) {
  ...
  for (int sid = 0; sid < scount; sid++) {
    ...
    for (int pjd = 0; pjd < count; pjd++) {
      ...
      if (r2 > 0.f && r2 < hig2) {
        IACT_STARS(r2, dx, hi, hj, si, pj, a, H, cosmo, stars_properties, xpj, e->ti_current);
      }
...
void DO_NONSYM_PAIR1_STARS(struct runner *r, struct cell *restrict ci,
  ...
  for (int sid = 0; sid < scount_i; sid++) {
    ...
    for (int pjd = 0; pjd < count_j; pjd++) {
      ...
      if (r2 < hig2) IACT_STARS(r2, dx, hi, hj, si, pj, a, H, cosmo, stars_properties, xpj, e->ti_current);
...
void DOPAIR1_SUBSET_STARS(struct runner *r, struct cell *restrict ci,
  ...
  for (int pid = 0; pid < scount; pid++) {
    ...
    for (int pjd = 0; pjd < count_j; pjd++) {
      ...
      /* Hit or miss? */
      if (r2 < hig2) {
        IACT_STARS(r2, dx, hi, pj->h, spi, pj, a, H, cosmo, stars_properties, xpj, e->ti_current);
      }
    } /* loop over the parts in cj. */
  }   /* loop over the parts in ci. */
}
...
void DOSELF1_SUBSET_STARS(struct runner *r, struct cell *restrict ci,
  ...
  for (int spid = 0; spid < scount; spid++) {
    ...
    for (int pjd = 0; pjd < count_i; pjd++) {
      ...
      /* Hit or miss? */
      if (r2 > 0.f && r2 < hig2) {
        IACT_STARS(r2, dx, hi, hj, spi, pj, a, H, cosmo, stars_properties, xpj, e->ti_current);
      }
    } /* loop over the parts in cj. */
  }   /* loop over the parts in ci. */
}
