...
/**
 * @brief Calculate the number density of dmparts around the #dmpart
 *
 * @param r runner task
 * @param c cell
 * @param timer 1 if the time is to be recorded.
 */
void DOSELF1_DM(struct runner *r, struct cell *c, int timer) {
  ...
  struct dmpart *restrict dmparts = c->dark_matter.parts;
    ...
    for (int dmid = 0; dmid < count; dmid++) {

      /* Get a hold of the ith bpart in ci. */
      struct dmpart *restrict dmi = &dmparts[bid];

      /* Skip inactive particles */
      if (!dmpart_is_active(dmi, e)) continue;

      const float hi = dmi->h;
      const float hig2 = hi * hi * kernel_gamma2;
      const float bix[3] = {(float)(dmi->x[0] - c->loc[0]),
                            (float)(dmi->x[1] - c->loc[1]),
                            (float)(dmi->x[2] - c->loc[2])};

      /* Loop over the parts in cj. */
      for (int dmjd = 0; dmjd < count; dmjd++) {

        /* Get a pointer to the jth particle. */
        struct dmpart *restrict dmj = &dmparts[dmjd];
        const float hj = dmj->h;

        /* Early abort? */
        if (dmpart_is_inhibited(dmj, e)) continue;

        /* Compute the pairwise distance. */
        const float dmpjx[3] = {(float)(dmj->x[0] - c->loc[0]),
                              (float)(dmj->x[1] - c->loc[1]),
                              (float)(dmj->x[2] - c->loc[2])};
        float dx[3] = {bix[0] - dmjx[0], bix[1] - dmjx[1], bix[2] - dmjx[2]};
        const float r2 = dx[0] * dx[0] + dx[1] * dx[1] + dx[2] * dx[2];

#ifdef SWIFT_DEBUG_CHECKS
        /* Check that particles have been drifted to the current time */
        if (dmi->ti_drift != e->ti_current)
          error("Particle bi not drifted to current time");
        if (dmj->ti_drift != e->ti_current)
          error("Particle pj not drifted to current time");
#endif

        if (r2 < hig2) {
          IACT_DM(r2, dx, hi, hj, dmi, dmj, a, H);
        }
      } /* loop over the dmparts in ci. */
    }   /* loop over the dmparts in ci. */
    
  TIMER_TOC(TIMER_DOSELF_DM);
}

/**
 * @brief Calculate the number density of cj #dmpart around the ci #dmpart
 *
 * @param r runner task
 * @param ci The first #cell
 * @param cj The second #cell
 */
void DO_NONSYM_PAIR1_DM_NAIVE(struct runner *r, struct cell *restrict ci,
  ...
  for (int k = 0; k < 3; k++) {
    ...
    for (int dmid = 0; dmid < count_i; dmid++) {

      /* Get a hold of the ith bpart in ci. */
      struct dmpart *restrict dmi = &dmparts_i[dmid];
      ...
      const float hi = dmi->h;
      const float hig2 = hi * hi * kernel_gamma2;
      const float dmix[3] = {(float)(dmi->x[0] - (cj->loc[0] + shift[0])),
                            (float)(dmi->x[1] - (cj->loc[1] + shift[1])),
                            (float)(dmi->x[2] - (cj->loc[2] + shift[2]))};

      /* Loop over the parts in cj. */
      for (int dmjd = 0; dmjd < count_j; dmjd++) {

        /* Get a pointer to the jth particle. */
        struct dmpart *restrict dmj = &dmparts_j[dmjd];
        const float hj = dmj->h;
        ...
        /* Compute the pairwise distance. */
        const float dmjx[3] = {(float)(dmj->x[0] - cj->loc[0]),
                              (float)(dmj->x[1] - cj->loc[1]),
                              (float)(dmj->x[2] - cj->loc[2])};
        float dx[3] = {dmix[0] - dmjx[0], dmix[1] - dmjx[1], dmix[2] - dmjx[2]};
        const float r2 = dx[0] * dx[0] + dx[1] * dx[1] + dx[2] * dx[2];
