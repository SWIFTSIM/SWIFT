...
/**
 * @file src/cooling/QLA_EAGLE/cooling_tables.c
 * @brief Functions to read QLA_EAGLE tables
 */
...
static const size_t num_elements_temperature = qla_eagle_cooling_N_He_frac *
...
/**
 * @brief Reads in QLA_EAGLE table of redshift values
 *
...
void get_cooling_redshifts(struct cooling_function_data *cooling) {
  ...
  if (!feof(infile)) {
    ...
    /* Check value */
    if (N_Redshifts != qla_eagle_cooling_N_redshifts)
...
/**
 * @brief Reads in QLA_EAGLE cooling table header. Consists of tables
...
void read_cooling_header(const char *fname,
  ...
  /* Check value */
  if (N_Temp != qla_eagle_cooling_N_temperature)
  ...
  /* Check value */
  if (N_He != qla_eagle_cooling_N_He_frac)
  ...
  /* Check value */
  if (N_SolarAbundances != qla_eagle_cooling_N_abundances)
...
void get_redshift_invariant_table(
  ...
  char filename[qla_eagle_table_path_name_length + 21];
  if (photodis) {
  ...
  for (int specs = 0; specs < qla_eagle_cooling_N_metal; specs++) {
    ...
    for (int j = 0; j < qla_eagle_cooling_N_temperature; j++) {
      for (int k = 0; k < qla_eagle_cooling_N_density; k++) {

        /* Index in the HDF5 table */
        const int hdf5_index = row_major_index_2d(
            j, k, qla_eagle_cooling_N_temperature, qla_eagle_cooling_N_density);

        /* Index in the internal table */
        const int internal_index = row_major_index_3d(
            specs, k, j, qla_eagle_cooling_N_metal, qla_eagle_cooling_N_density,
            qla_eagle_cooling_N_temperature);
   ...
   * varying index is on right. Tables contain cooling rates but we
   * want rate of change of internal energy, hence minus sign. */
  for (int i = 0; i < qla_eagle_cooling_N_He_frac; i++) {
    for (int j = 0; j < qla_eagle_cooling_N_temperature; j++) {
      for (int k = 0; k < qla_eagle_cooling_N_density; k++) {
        ...
        /* Index in the internal table */
        const int internal_index = row_major_index_3d(
            k, i, j, qla_eagle_cooling_N_density, qla_eagle_cooling_N_He_frac,
            qla_eagle_cooling_N_temperature);
  ...
  for (int i = 0; i < qla_eagle_cooling_N_temperature; i++) {
    for (int j = 0; j < qla_eagle_cooling_N_density; j++) {

      /* Index in the HDF5 table */
      const int hdf5_index = row_major_index_2d(
          i, j, qla_eagle_cooling_N_temperature, qla_eagle_cooling_N_density);

      /* Index in the internal table */
      const int internal_index = row_major_index_2d(
          j, i, qla_eagle_cooling_N_density, qla_eagle_cooling_N_temperature);
...
void get_cooling_table(struct cooling_function_data *restrict cooling,
  ...
  for (int z_index = low_z_index; z_index <= high_z_index; z_index++) {
...
#ifdef SWIFT_DEBUG_CHECKS
    if (local_z_index >= qla_eagle_cooling_N_loaded_redshifts)
    ...
    for (int specs = 0; specs < qla_eagle_cooling_N_metal; specs++) {
       ...
       * varying index is on right. Tables contain cooling rates but we
       * want rate of change of internal energy, hence minus sign. */
      for (int i = 0; i < qla_eagle_cooling_N_density; i++) {
        for (int j = 0; j < qla_eagle_cooling_N_temperature; j++) {
          ...
          const int internal_index = row_major_index_4d(
              specs, local_z_index, i, j, qla_eagle_cooling_N_metal,
              qla_eagle_cooling_N_loaded_redshifts, qla_eagle_cooling_N_density,
              qla_eagle_cooling_N_temperature);
    ...
    for (int i = 0; i < qla_eagle_cooling_N_He_frac; i++) {
      for (int j = 0; j < qla_eagle_cooling_N_temperature; j++) {
        for (int k = 0; k < qla_eagle_cooling_N_density; k++) {
          ...
          /* Index in the internal table */
          const int internal_index = row_major_index_4d(
              local_z_index, k, i, j, qla_eagle_cooling_N_loaded_redshifts,
              qla_eagle_cooling_N_density, qla_eagle_cooling_N_He_frac,
              qla_eagle_cooling_N_temperature);
    ...
    for (int i = 0; i < qla_eagle_cooling_N_temperature; i++) {
      for (int j = 0; j < qla_eagle_cooling_N_density; j++) {
        ...
        /* Index in the internal table */
        const int internal_index = row_major_index_3d(
            local_z_index, j, i, qla_eagle_cooling_N_loaded_redshifts,
            qla_eagle_cooling_N_density, qla_eagle_cooling_N_temperature);
