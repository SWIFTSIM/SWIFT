...
// static double deltaT_MY_COOLING;         /*!< log10 of temperature spacing in
// the interpolation tables */

// additional definitions from somewhere else
#define MAXITER \
  300000 /*! Maximum number of iterations before process is terminated */
...
void MakeRateTable(struct cooling_function_data *cooling) {
  ...
  cooling->gs.ethmin =
      pow(10.0, T_MIN_MY_COOLING) * (1. + cooling->gs.yhelium) /
      ((1. + 4. * cooling->gs.yhelium) * cooling->gs.mhboltz * GAMMA_MINUS1);
  /* minimum internal energy for neutral gas */

  for (i = 0; i <= NCOOLTAB; i++) {
    cooling->RateT[i].BetaH0 = cooling->RateT[i].BetaHep =
        cooling->RateT[i].Betaff = cooling->RateT[i].AlphaHp =
            cooling->RateT[i].AlphaHep = cooling->RateT[i].AlphaHepp =
                cooling->RateT[i].Alphad = cooling->RateT[i].GammaeH0 =
                    cooling->RateT[i].GammaeHe0 = cooling->RateT[i].GammaeHep =
                        0;
    ...
    if (473638 / T < 70)
      cooling->RateT[i].BetaHep =
          5.54e-17 * pow(T, -0.397) * exp(-473638 / T) * Tfact;

    /* free-free */
    cooling->RateT[i].Betaff =
        1.43e-27 * sqrt(T) *
        (1.1 + 0.34 * exp(-(5.5 - log10(T)) * (5.5 - log10(T)) / 3));

    /* recombination */
    /* Cen 1992 */
    /* Hydrogen II */
    cooling->RateT[i].AlphaHp =
        8.4e-11 * pow(T / 1000, -0.2) / (1. + pow(T / 1.0e6, 0.7)) / sqrt(T);
    /* Helium II */
    ...
    if (470000 / T < 70)
      cooling->RateT[i].Alphad = 1.9e-3 * pow(T, -1.5) * exp(-470000 / T) *
                                 (1. + 0.3 * exp(-94000 / T));
    ...
    /* Hydrogen */
    if (157809.1 / T < 70)
      cooling->RateT[i].GammaeH0 =
          5.85e-11 * sqrt(T) * exp(-157809.1 / T) * Tfact;
    /* Helium */
    if (285335.4 / T < 70)
      cooling->RateT[i].GammaeHe0 =
          2.38e-11 * sqrt(T) * exp(-285335.4 / T) * Tfact;
    /* Hellium II */
    if (631515.0 / T < 70)
      cooling->RateT[i].GammaeHep =
          5.68e-12 * sqrt(T) * exp(-631515.0 / T) * Tfact;
  }
}
...
void ReadIonizeParams(struct cooling_function_data *cooling) {
  ...
  for (iter = 0, i = 0; iter < 2; iter++) {
    ...
    if (iter == 0)
      while (fscanf(fdcool, "%g %g %g %g %g %g %g", &dummy, &dummy, &dummy,
                    &dummy, &dummy, &dummy, &dummy) == 7)
        cooling->NheattabUVB++;
    if (iter == 1)
      while (fscanf(fdcool, "%g %g %g %g %g %g %g",
                    &cooling->PhotoTUVB[i].variable, &cooling->PhotoTUVB[i].gH0,
                    &cooling->PhotoTUVB[i].gHe, &cooling->PhotoTUVB[i].gHep,
                    &cooling->PhotoTUVB[i].eH0, &cooling->PhotoTUVB[i].eHe,
                    &cooling->PhotoTUVB[i].eHep) == 7)
        i++;
    fclose(fdcool);
  ...
  for (i = 0; i < cooling->NheattabUVB; ++i)
    if (cooling->PhotoTUVB[i].gH0 == 0.0) break;
...
void IonizeParamsUVB(double redz, struct cooling_function_data *cooling) {
  ...
  if (cooling->NheattabUVB == 0 ||
      logz > cooling->PhotoTUVB[cooling->NheattabUVB - 1].variable ||
      cooling->PhotoTUVB[ilow].gH0 == 0 ||
      cooling->PhotoTUVB[ilow + 1].gH0 == 0) {
  ...
  cooling->pc.gJH0 =
      pow(10., (dzhi * log10(cooling->PhotoTUVB[ilow].gH0) +
                dzlow * log10(cooling->PhotoTUVB[ilow + 1].gH0)) /
                   (dzlow + dzhi));
  cooling->pc.gJHe0 =
      pow(10., (dzhi * log10(cooling->PhotoTUVB[ilow].gHe) +
                dzlow * log10(cooling->PhotoTUVB[ilow + 1].gHe)) /
                   (dzlow + dzhi));
  cooling->pc.gJHep =
      pow(10., (dzhi * log10(cooling->PhotoTUVB[ilow].gHep) +
                dzlow * log10(cooling->PhotoTUVB[ilow + 1].gHep)) /
                   (dzlow + dzhi));
  cooling->pc.epsH0 =
      pow(10., (dzhi * log10(cooling->PhotoTUVB[ilow].eH0) +
                dzlow * log10(cooling->PhotoTUVB[ilow + 1].eH0)) /
                   (dzlow + dzhi));
  cooling->pc.epsHe0 =
      pow(10., (dzhi * log10(cooling->PhotoTUVB[ilow].eHe) +
                dzlow * log10(cooling->PhotoTUVB[ilow + 1].eHe)) /
                   (dzlow + dzhi));
  cooling->pc.epsHep =
      pow(10., (dzhi * log10(cooling->PhotoTUVB[ilow].eHep) +
                dzlow * log10(cooling->PhotoTUVB[ilow + 1].eHep)) /
                   (dzlow + dzhi));
