...
void zoom_link_void_buffer_leaves(struct space *s, struct cell *c) {
  ...
  /* If we are above regular buffer cells we need to ensure this "void leaf",
   * i.e. void cell progeny that isn't above the zoom cells and is instead
  ...
  for (int k = 0; k < 8; k++) {
    ...
    /* Establish the location of the progeny cell. */
    double loc[3] = {c->loc[0] + (c->width[0] / 4),
                     c->loc[1] + (c->width[1] / 4),
                     c->loc[2] + (c->width[2] / 4)};
    if (k & 4) loc[0] += c->width[0] / 2;
