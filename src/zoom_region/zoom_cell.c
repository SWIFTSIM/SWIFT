...
void zoom_find_void_cells(struct space *s, const int verbose) {
  ...
  void zoom_verify_cell_type(struct space * s) {
    ...
    if (s->zoom_props->with_buffer_cells) {
      ...
      /* Did we find the boundaries?. */
      if (!found_bkg_zoomi_low || !found_bkg_zoomj_low ||
          !found_bkg_zoomk_low || !found_bkg_zoomi_up || !found_bkg_zoomj_up ||
          !found_bkg_zoomk_up)
   ...
   * constructs all zoom, background and buffer cells (if required) and sets
   * their initial values.
   *
   * Zoom cells occupy the first cells in the cells array, followed by
   * background cells and then buffer cells (if required).
   *
   * @param s The space.
  ...
  void zoom_construct_tl_cells(struct space * s, const integertime_t ti_current,
    ...
    for (int i = 0; i < s->cdim[0]; i++) {
      for (int j = 0; j < s->cdim[1]; j++) {
        for (int k = 0; k < s->cdim[2]; k++) {
          const size_t cid =
              cell_getid_offset(s->cdim, bkg_cell_offset, i, j, k);
