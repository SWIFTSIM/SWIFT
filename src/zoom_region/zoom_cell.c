...
/**
 * @brief Find what background cells contain the zoom region.
 *
...
void zoom_find_void_cells(struct space *s, const int verbose) {
  ...
  /* Work out how many void cells we should have. (We multiply by 1.0001 to
   * avoid rounding errors). */
  const int void_cdim = fmax(zoom_props->dim[0] * s->iwidth[0] * 1.0001, 1);
  const int target_void_count = void_cdim * void_cdim * void_cdim;
...
void zoom_find_neighbouring_cells(struct space *s, const int verbose) {
  ...
  /* Loop over background cells. We'll walk out delta_cells cells from
   * any void cells. */
