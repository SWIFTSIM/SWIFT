...
void zoom_parse_params(struct swift_params *params,
  ...
  if (props->region_buffer_ratio == 0) { /* i.e. no buffer cells. */
    if (!cdim_is_power_of_2(props->cdim[0], region_buffer_ratio)) {
      error(
  ...
  void zoom_region_init(struct swift_params * params, struct space * s,
     ...
     * cells. */
      ...
      if (zoom_get_cell_props_with_buffer_cells(s, grav_props, &max_dim,
                                                ini_dim))
        error(
