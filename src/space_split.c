...
/**
 * @brief Collect the values from the progeny and store them in the parent.
 *
 * @param c The #cell to update.
 * @param s The #space.
 */
static void space_split_update_parent(struct cell *c, struct space *s) {
  ...
  /* Reset the star formation history so we don't accumulate twice. */
 ...
 * @param s The #space.
 */
static void space_split_check_done(struct cell *c, struct space *s) {

  if (c == NULL) return;

  /* Increment and check if we are the last child (8th). */
  if (atomic_inc(&c->split_buffers.children_completed) + 1 == 8) {
    /* We are the last one. Update parent. */
    if (lock_lock(&c->hydro.lock) != 0) error("Failed to lock parent");
    space_split_update_parent(c, s);
    if (lock_unlock(&c->hydro.lock) != 0)
      error("Failed to unlock parent");
    
    /* Propagate up. */
    space_split_check_done(c->parent, s);
  }
}
...
void space_split_mapper(void *map_data, int num_cells, void *extra_data) {
  ...
  for (int ind = 0; ind < num_cells; ind++) {
    ...
    c->split_buffers.children_completed = 0;
