...
void runner_do_long_range_zoom_periodic(struct runner *r, struct cell *ci,
  ...
  /* Because neighbour cells are by definition the background cells within the
   * mesh distance criterion we only need to check them for long range
   * interactions. */
...
void runner_count_mesh_interactions(struct runner *r, struct cell *ci,
  ...
  /* We need to treat zoom cells and background cells differently
