...
/**
 * @brief Compute the new (integer) time-step of a given #sipart
 *
 * @param sip The #sipart.
 * @param e The #engine (used to get some constants).
 */
__attribute__((always_inline)) INLINE static integertime_t get_sipart_timestep(
    const struct sipart *restrict sip, const struct engine *restrict e) {

  /* SIDM time-step */
  float new_dt_sidm = sidm_compute_timestep(
      sip, e->sidm_properties, (e->policy & engine_policy_cosmology),
      e->cosmology, e->time, e->time_base);
  ...
  if (e->policy & engine_policy_external_gravity)
    new_dt_ext = external_gravity_timestep(e->time, e->external_potential,
                                           e->physical_constants, sip->gpart);

  const float a_hydro[3] = {0.f, 0.f, 0.f};
  if (e->policy & engine_policy_self_gravity)
    new_dt_self = gravity_compute_timestep_self(
        sip->gpart, a_hydro, e->gravity_properties, e->cosmology);

  /* Take the minimum of all */
  float new_dt = min3(new_dt_sidm, new_dt_self, new_dt_ext);
  ...
  new_dt = min(new_dt, e->dt_max);
  if (new_dt < e->dt_min) {
    error("sipart (id=%lld) wants a time-step (%e) below dt_min (%e)", sip->id,
          new_dt, e->dt_min);
  }

  /* Convert to integer time */
  const integertime_t new_dti = make_integer_timestep(
      new_dt, sip->time_bin, num_time_bins, e->ti_current, e->time_base_inv);
