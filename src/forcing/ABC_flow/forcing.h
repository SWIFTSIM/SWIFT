 ...
 *
 * Based on David Galloway (2012) ABC flows then and now, Geophysical & Astrophysical Fluid Dynamics, 106:4-5, 450-467 
 * This version differs from the paper by imposing normalized velocity
 *
...
__attribute__((always_inline)) INLINE static void forcing_terms_apply(
  ...
  const float C = trems->C;
  const float Norm = 1/sqrt(A*A + B*B + C*C)
  const float Vz_factor = terms->Vz_factor;
  const double k0 = (2. * M_PI / L) * terms->kv;
  double v_ABC[3];

  /* Eq. 2 of David Galloway (2012) ABC flows then and now, Geophysical & Astrophysical Fluid Dynamics, 106:4-5, 450-467 */
...
static INLINE void forcing_terms_print(const struct forcing_terms* terms) {
  ...
  message("Forcing terms is 'ABC flow'. U0: %.5f / Vz factor: %.5f.",
          terms->u0, terms->Vz_factor);
  message("Run using ABC parameters: A = %.5f, B = %.5f, C = %.5f",
