...
__attribute__((always_inline)) INLINE static void forcing_hydro_terms_apply(
  ...
  if ((time >= terms->times[t_index]) && (terms->final_injection == 0)) {
    terms->counter++;

    const double SN_loc[3] = {terms->x_SN[t_index], terms->y_SN[t_index],
                              terms->z_SN[t_index]};
    ...
    if (distance <= terms->r_inj) {
      message("applying injection to particle %lld at t: %f", p->id, time);

      /* store old (physical) specific energy and velocity */
      const float u_old =
          hydro_get_physical_internal_energy(p, xp, s->e->cosmology);
      const float v_old =
      ...
      switch (injection_model) {
        ...
        case forcing_balsara_kim_mechanism_set_u:
          ...
          hydro_set_drifted_physical_internal_energy(
              p, s->e->cosmology, s->e->pressure_floor_props, u_new);
...
static INLINE void forcing_terms_print(const struct forcing_terms *terms) {
  ...
  switch (injection_model) {
    ...
    case forcing_balsara_kim_mechanism_set_u:
      ...
      message(
          "Balsara-Kim density dependent specific energy injection with E_inj: "
          "%f",
          terms->E_inj);
      break;
...
static INLINE void forcing_terms_init(struct swift_params *parameter_file,
  ...
  while (getline(&line, &len, file) != -1) {
    ...
    read_successfully =
        sscanf(line, "%lf,%lf,%lf,%lf", time, x_SN, y_SN, z_SN) == 4;
  ...
  for (size_t i = 1; i < terms->num_supernovae; ++i) {
    ...
    if (terms->times[i] <= terms->times[i - 1])
      error(
          "injection coordinate list not having monotonically increasing "
          "times.");
  }
  ...
  double r_inj_pc =
      parser_get_param_double(parameter_file, "BalsaraKimForcing:r_inj_pc");
  ...
  if (strcmp(injection_model, "set_u") == 0) {
    ...
    E_inj_cgs =
        parser_get_param_double(parameter_file, "BalsaraKimForcing:E_inj_cgs");
  } else if (strcmp(injection_model, "set_const_u") == 0) {
    terms->injection_model = forcing_balsara_kim_mechanism_set_const_u;

    u_inj_cgs =
        parser_get_param_double(parameter_file, "BalsaraKimForcing:u_inj_cgs");
    ...
    vel_inj_cgs =
        parser_get_param_double(parameter_file, "BalsaraKimForcing:v_inj_kms") *
        1.e5;
  } else {
