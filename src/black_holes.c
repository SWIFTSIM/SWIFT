...
/**
 * @brief Mapper function for the exact black holes checks.
 *
 * @brief map_data The #bpart's.
 * @brief nr_bparts The number of black hole particles.
...
#else
  error("Black holes checking function called without the corresponding flag.");
#endif
}

/**
 * @brief Compute the exact interactions for a selection of black hole particles
...
void black_holes_exact_density_check(struct space *s, const struct engine *e,
  ...
  const double N_ngb_max =
      N_ngb_target + 2. * e->black_holes_properties->delta_neighbours;
  ...
  fprintf(file_swift, "# Black holes accuracy test - SWIFT DENSITIES\n");
  fprintf(file_swift, "# N= %d\n", SWIFT_BLACK_HOLES_DENSITY_CHECKS);
  ...
  fprintf(file_exact, "# Black holes accuracy test - EXACT DENSITIES\n");
  ...
  if (wrong_rho)
    error(
        ...
        "black hole particles! (out of %d particles)",
        wrong_rho, counter);
  else
    message("Verified %d black hole particles", counter);
