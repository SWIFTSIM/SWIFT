...
/**
 * @brief Return the argument to the power three adiabatic index minus seven over
 * two.
 *
 * Computes \f$x^{(3\gamma - 7)/2}\f$.
 *
 * @param x Argument
 */
__attribute__((always_inline, const)) INLINE static float
pow_three_gamma_minus_seven_over_two(float x) {
...
/**
 * @brief Return minus three times the adiabatic index plus 3
 *
 * Computes \f$x^{-3\gamma+3}\f$.
 *
 * @param x Argument
 * @return Inverse argument to the power the adiabatic index plus one divided by
 * two times the adiabatic index
 */
__attribute__((always_inline, const)) INLINE static float
pow_minus_three_gamma_plus_three(float x) {
