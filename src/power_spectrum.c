...
void power_spectrum_stripped(const struct space *s, struct threadpool *tp, const double *rho, const int Ngrid, const int verbose) {
  const enum power_type type1 = pow_type_matter;
  const enum power_type type2 = pow_type_matter;
  ...
  fftw_plan fftplanpow = s->e->power_data->fftplanpow;
  int shift_centre_small_k_bins = s->e->power_data->shift_centre_small_k_bins;
  ...
  double *powgrid = fftw_alloc_real(Ngrid2 * (Ngrid + 2));
  memuse_log_allocation("fftw_grid.grid", pow_data->powgrid, 1,
                        sizeof(double) * Ngrid2 * (Ngrid + 2));
  fftw_complex* powgridft = (fftw_complex*)powgrid;
  double *powgrid2 = powgrid;
  fftw_complex* powgridft2 = powgridft;
  ...
  double invcellmean = Ngrid3 / volume * conv_EV;
  double invcellmean2 = invcellmean2 = Ngrid3 / volume * conv_EV;
  ...
  densdata.dens = powgrid;
  densdata.N = Ngrid;
  densdata.type = type1;
  densdata.windoworder = windoworder;
  densdata.e = s->e;
  densdata.nu_model = &nu_model;

  /* Calculate mass terms for shot noise */
  double shot = 0.;
  if (verbose) message("Going to compute the shot noise");
  ...
  powmapdata.powgridft = powgridft;
  powmapdata.powgridft2 = powgridft2;
  powmapdata.Ngrid = Ngrid;
  powmapdata.windoworder = windoworder;
  ...
  sprintf(outputfileBase, "power_MG_test128%s", get_powtype_filename(type1));

  if (verbose) message("Got to the loop part");
  /* From here this is instead of looping over foldings */
  ...
  double* kout = (double*)malloc(Nhalf * sizeof(double));
  double* pout = (double*)malloc(Nhalf * sizeof(double));

  /* Empty the grid(s) */
  bzero(powgrid, Ngrid2 * (Ngrid + 2) * sizeof(double));
  if (type1 != type2)
    bzero(powgrid2, Ngrid2 * (Ngrid + 2) * sizeof(double));

  /* Fill out the grid (?) */
  if (verbose) message("Filling out the grid");
  ...
  if (e->nodeID == 0) {
    ...
    convdata.grid = powgrid;
    convdata.invcellmean = invcellmean;
    if (Ngrid < 32) {
      mass_to_contrast_mapper(powgrid, Ngrid, &convdata);
    } else {
      threadpool_map(tp, mass_to_contrast_mapper, powgrid, Ngrid,
                      sizeof(double), threadpool_auto_chunk_size, &convdata);
    }

    /* Perform FFT(s) */
    if (verbose) message("Performing FFT");
    fftw_execute_dft_r2c(fftplanpow, powgrid,
                        powgridft);
    ...
    if (verbose) message("Calculating mode contributions");
    if (Ngrid < 32) {
      pow_from_grid_mapper(powgridft, Ngrid, &powmapdata);
    } else {
      threadpool_map(tp, pow_from_grid_mapper, powgridft, Ngrid,
  ...
  for (int j = 1; j <= Nhalf; ++j) {
      kout[j-1] = j * kfac;
      message("Powersum entry is %lf", powersum[j]);
      pout[j-1] = powersum[j] / modecounts[j] * volfac;
  }
  if (e->nodeID == 0) {

    /* Output attempt at combined measurement */
    if (verbose) message("Attempting to output");
    ...
    for (int j = 0; j < Nhalf; ++j) {
      ...
      if (shift_centre_small_k_bins && j < number_of_corrected_bins) {
  ...
  powgrid2 = NULL;
  powgridft2 = NULL;
  memuse_log_allocation("fftw_grid.grid", powgrid, 0, 0);
  fftw_free(powgrid);
  powgrid = NULL;
  powgridft = NULL;
}
