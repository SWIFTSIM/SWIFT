 ...
 * a new one such that the particle becomes active again in the next time-step.
 *
 * @param sip The #sipart to update.
 * @param e The #engine (to extract time-line information).
 *
 * @return The updated integer end-of-step of the particle.
 */
__attribute__((always_inline)) INLINE static integertime_t
timestep_limit_sipart(struct sipart *restrict sip, const struct engine *e) {
  ...
  if (sipart_is_active(sip, e)) {
    ...
    /* New time-bin of this particle */
    sip->time_bin = -sip->limiter_data.wakeup + 2;

    /* Mark the particle as being rady to be time integrated */
    sip->limiter_data.wakeup = time_bin_not_awake;

    /* Return the new end-of-step for this particle */
    return ti_current + get_integer_timestep(sip->time_bin);
    ...
    /* The timebins to play with */
    const timebin_t old_bin = sip->time_bin;
    const timebin_t new_bin = -sip->limiter_data.wakeup + 2;
    ...
    /* Note that there is no need to change the mesh integration as we
     * can't go back more than one global step */
    kick_sipart(sip, dt_kick_grav, ti_end_half_old, ti_beg_old,
                /*dt_kick_mesh_grav=*/0.,
                /*ti_start_mesh=*/-1, /*ti_end_mesh=*/-1);
    ...
    /* Note that there is no need to change the mesh integration as we
     * can't go back more than one global step */
    kick_sipart(sip, dt_kick_grav, ti_beg_old, ti_beg_new,
                /*dt_kick_mesh_grav=*/0.,
                /*ti_start_mesh=*/-1, /*ti_end_mesh=*/-1);
    ...
    if (new_bin > e->max_active_bin) {
      ...
      /* Note that there is no need to change the mesh integration as we
       * can't go back more than one global step */
      kick_sipart(sip, dt_kick_grav, ti_beg_new, ti_end_half_new,
                  /*dt_kick_mesh_grav=*/0.,
                  /*ti_start_mesh=*/-1, /*ti_end_mesh=*/-1);
