...
/**
 * @brief Mapper function to drift *all* the #dmpart to the current time.
 *
 * @param map_data An array of #cell%s.
 * @param num_elements Chunk size.
 * @param extra_data Pointer to an #engine.
 */
void engine_do_drift_all_dmpart_mapper(void *map_data, int num_elements,
                                      void *extra_data) {
    ...
    for (int ind = 0; ind < num_elements; ind++) {
        ...
        if (c->nodeID == e->nodeID) {
            ...
            /* Drift all the particles */
            cell_drift_dmpart(c, e, /* force the drift=*/1, NULL);
        }
    }
}
