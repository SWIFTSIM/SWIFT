...
#define HASHMAP_GP_GROWTH_FACTOR (2)
#define HASHMAP_GP_MAX_FILL_RATIO (1.0)
#define HASHMAP_GP_MAX_CHUNK_FILL_RATIO (0.5)

/**
 * @brief Pre-allocate a number of chunks for the graveyard.
 */
void hashmap_gp_allocate_chunks(hashmap_gp_t *m, int num_chunks) {
  /* Allocate a fresh set of chunks. */
  hashmap_gp_chunk_t *alloc;
  if ((alloc = (hashmap_gp_chunk_t *)swift_calloc(
           "hashmap_gp", num_chunks, sizeof(hashmap_gp_chunk_t))) == NULL) {
...
void hashmap_gp_init(hashmap_gp_t *m) {
  ...
  if ((m->chunks = (hashmap_gp_chunk_t **)swift_calloc(
           "hashmap_gp", m->nr_chunks, sizeof(hashmap_gp_chunk_t *))) == NULL) {
    error("Unable to allocate hashmap_gp chunks.");
  }
  ...
  m->allocs_size = HASHMAP_GP_ALLOCS_INITIAL_SIZE;
  if ((m->allocs = (void **)swift_malloc(
           "hashmap_gp", sizeof(void *) * m->allocs_size)) == NULL) {
    error("Unable to allocate allocs pointer array.");
  }
  m->allocs_count = 0;

  /* Set initial sizes. */
  m->table_size = m->nr_chunks * HASHMAP_GP_ELEMENTS_PER_CHUNK;
  m->size = 0;
  ...
  if (HASHMAP_GP_DEBUG_OUTPUT) {
    message(
        ...
        "%zu empty chunks.",
        m->table_size * sizeof(hashmap_gp_element_t), sizeof(hashmap_gp_element_t),
        m->nr_chunks);
  }
}
 ...
 */
void hashmap_gp_release_chunk(hashmap_gp_t *m, hashmap_gp_chunk_t *chunk) {
  /* Clear all the chunk's data. */
  memset(chunk, 0, sizeof(hashmap_gp_chunk_t));
 ...
 */
hashmap_gp_chunk_t *hashmap_gp_get_chunk(hashmap_gp_t *m) {
  int num_chunks = m->nr_chunks * HASHMAP_GP_ALLOC_SIZE_FRACTION;
  if (!num_chunks) num_chunks = 1;
  if (m->graveyard == NULL) {
    hashmap_gp_allocate_chunks(m, num_chunks);
  }

  hashmap_gp_chunk_t *res = (hashmap_gp_chunk_t *)m->graveyard;
  m->graveyard = (hashmap_gp_chunk_t *)res->next;
  res->next = NULL;
 ...
 * element.
 *
 * The returned element is either the one that already existed in the hashmap_gp,
 * or a newly-reseverd element initialized to zero.
 *
 * If the hashmap_gp is full, NULL is returned.
 *
 ...
 * is repeated for at most HASHMAP_GP_MAX_CHAIN_LENGTH steps, at which point
 * insertion fails.
 */
hashmap_gp_element_t *hashmap_gp_find(hashmap_gp_t *m, hashmap_gp_key_t key, int create_new,
                                int *chain_length, int *created_new_element) {
  /* If full, return immediately */
  if (create_new && m->size > m->table_size * HASHMAP_GP_MAX_FILL_RATIO) {
    if (HASHMAP_GP_DEBUG_OUTPUT) {
      message("hashmap_gp is too full (%zu of %zu elements used), re-hashing.",
              m->size, m->table_size);
    }
    return NULL;
  }
  ...
  const size_t chunk_offset = offset / HASHMAP_GP_ELEMENTS_PER_CHUNK;
  size_t offset_in_chunk = offset - chunk_offset * HASHMAP_GP_ELEMENTS_PER_CHUNK;
  ...
  if (m->chunks[chunk_offset] == NULL) {
    ...
    m->chunks[chunk_offset] = hashmap_gp_get_chunk(m);
  }
  hashmap_gp_chunk_t *chunk = m->chunks[chunk_offset];

  /* Count the number of free elements in this chunk and bail if it is too low.
   */
  int chunk_count = 0;
  for (int k = 0; k < HASHMAP_GP_MASKS_PER_CHUNK; k++) {
    chunk_count += __builtin_popcountll(chunk->masks[k]);
  }
  if (create_new &&
      chunk_count > HASHMAP_GP_ELEMENTS_PER_CHUNK * HASHMAP_GP_MAX_CHUNK_FILL_RATIO) {
    if (HASHMAP_GP_DEBUG_OUTPUT) {
      message(
          "chunk %zu is too full (%i of %i elements used, fill ratio is "
          "%.2f%%), re-hashing.",
          chunk_offset, chunk_count, HASHMAP_GP_ELEMENTS_PER_CHUNK,
  ...
  for (int i = 0; i < HASHMAP_GP_MAX_CHAIN_LENGTH; i++) {
    /* Record the chain_length, if not NULL. */
    if (chain_length) *chain_length = i;

    /* Compute the offsets within the masks of this chunk. */
    const int mask_offset = offset_in_chunk / HASHMAP_GP_BITS_PER_MASK;
    const int offset_in_mask =
        offset_in_chunk - mask_offset * HASHMAP_GP_BITS_PER_MASK;

    /* Is the offset empty? */
    hashmap_gp_mask_t search_mask = ((hashmap_gp_mask_t)1) << offset_in_mask;
    ...
    /* None of the above, so this is a collision. Re-hash, but within the same
      ...
      offset_in_chunk = rand_r(&curr) % HASHMAP_GP_ELEMENTS_PER_CHUNK;
    }
  }
...
/**
 * @brief Grows the hashmap_gp and rehashes all the elements
 */
void hashmap_gp_grow(hashmap_gp_t *m, size_t new_size) {
  /* Hold on to the old data. */
  const size_t old_table_size = m->table_size;
  hashmap_gp_chunk_t **old_chunks = m->chunks;

  /* Re-allocate the chunk array. */
  if (new_size == 0) new_size = m->table_size * HASHMAP_GP_GROWTH_FACTOR;
  m->nr_chunks =
      (new_size + HASHMAP_GP_ELEMENTS_PER_CHUNK - 1) / HASHMAP_GP_ELEMENTS_PER_CHUNK;
  m->table_size = m->nr_chunks * HASHMAP_GP_ELEMENTS_PER_CHUNK;

  if (HASHMAP_GP_DEBUG_OUTPUT) {
    message("Increasing hash table size from %zu (%zu kb) to %zu (%zu kb).",
            old_table_size, old_table_size * sizeof(hashmap_gp_element_t) / 1024,
            m->table_size, m->table_size * sizeof(hashmap_gp_element_t) / 1024);
  }
  ...
  hashmap_gp_element_t *strays = NULL;
  size_t strays_count = 0;
  size_t strays_size = 0;

  /* Iterate over the chunks and add their entries to the new table. */
  for (size_t cid = 0; cid < old_table_size / HASHMAP_GP_ELEMENTS_PER_CHUNK;
       cid++) {
    /* Skip empty chunks. */
    hashmap_gp_chunk_t *chunk = old_chunks[cid];
    if (!chunk) continue;

    /* Loop over the masks in this chunk. */
    for (int mid = 0; mid < HASHMAP_GP_MASKS_PER_CHUNK; mid++) {
      /* Skip empty masks. */
      if (chunk->masks[mid] == 0) continue;

      /* Loop over the mask entries. */
      for (int eid = 0; eid < HASHMAP_GP_BITS_PER_MASK; eid++) {
        hashmap_gp_mask_t element_mask = ((hashmap_gp_mask_t)1) << eid;
        if (chunk->masks[mid] & element_mask) {
          hashmap_gp_element_t *element =
              &chunk->data[mid * HASHMAP_GP_BITS_PER_MASK + eid];

          /* Copy the element over to the new hashmap_gp. */
          hashmap_gp_element_t *new_element =
              hashmap_gp_find(m, element->key, /*create_new=*/1,
                           /*chain_length=*/NULL, /*created_new_element=*/NULL);

          if (new_element) {
            new_element->value = element->value;
          }
          /* If copying failed, then we have an overflow in the new hashmap_gp.
             If this happens, store the offending element in the strays buffer
             for now. */
          else {
            /* (Re)allocate strays buffer? */
            if (strays_count == strays_size) {
              hashmap_gp_element_t *temp_buff;
              strays_size = strays_size ? strays_size * 2 : 10;
              if ((temp_buff = (hashmap_gp_element_t *)swift_malloc(
                       "hashmap_gp_strays",
                       sizeof(hashmap_gp_element_t) * strays_size)) == NULL)
                error("Failed to (re)allocate strays buffer.");
              memcpy(temp_buff, strays,
                     sizeof(hashmap_gp_element_t) * strays_count);
              swift_free("hashmap_gp_strays", strays);
              strays = temp_buff;
            }
    ...
    hashmap_gp_release_chunk(m, chunk);
  }

  /* Free the old list of chunks. */
  swift_free("hashmap_gp", old_chunks);

  /* If we have any strays, add them back to the hashmap_gp. This will inevitably
     trigger a rehashing, but that's not our problem. */
  if (strays_count) {
    for (size_t k = 0; k < strays_count; k++) {
      hashmap_gp_put(m, strays[k].key, strays[k].value);
    }
    swift_free("hashmap_gp_strays", strays);
  }
}

void hashmap_gp_put(hashmap_gp_t *m, hashmap_gp_key_t key, hashmap_gp_value_t value) {
  /* Try to find an element for the given key. */
  hashmap_gp_element_t *element =
      hashmap_gp_find(m, key, /*create_new=*/1, /*chain_length=*/NULL,
                   /*created_new_element=*/NULL);

  /* Loop around, trying to find our place in the world. */
  while (!element) {
    hashmap_gp_grow(m, 0);
    element = hashmap_gp_find(m, key, /*create_new=*/1, /*chain_length=*/NULL,
                           /*created_new_element=*/NULL);
  }
...
hashmap_gp_value_t *hashmap_gp_get(hashmap_gp_t *m, hashmap_gp_key_t key) {
...
hashmap_gp_value_t *hashmap_gp_get_new(hashmap_gp_t *m, hashmap_gp_key_t key,
                                 int *created_new_element) {
  /* Look for the given key. */
  hashmap_gp_element_t *element = hashmap_gp_find(
...
hashmap_gp_value_t *hashmap_gp_lookup(hashmap_gp_t *m, hashmap_gp_key_t key) {
  hashmap_gp_element_t *element =
      hashmap_gp_find(m, key, /*create_new=*/0, /*chain_length=*/NULL,
                   /*created_new_element=*/NULL);
  return element ? &element->value : NULL;
}

void hashmap_gp_iterate(hashmap_gp_t *m, hashmap_gp_mapper_t f, void *data) {
  /* Loop over the chunks. */
  for (size_t cid = 0; cid < m->nr_chunks; cid++) {
    hashmap_gp_chunk_t *chunk = m->chunks[cid];
    if (!chunk) continue;

    /* Loop over the masks. */
    for (int mid = 0; mid < HASHMAP_GP_MASKS_PER_CHUNK; mid++) {
      hashmap_gp_mask_t mask = chunk->masks[mid];
      if (!mask) continue;

      /* Loop over each element in the mask. */
      for (int eid = 0;
           eid < HASHMAP_GP_BITS_PER_MASK &&
           mid * HASHMAP_GP_BITS_PER_MASK + eid < HASHMAP_GP_ELEMENTS_PER_CHUNK;
           eid++) {
        /* If the element exists, call the function on it. */
        if (mask & (((hashmap_gp_mask_t)1) << eid)) {
          hashmap_gp_element_t *element =
              &chunk->data[mid * HASHMAP_GP_BITS_PER_MASK + eid];
...
void hashmap_gp_free(hashmap_gp_t *m) {
  ...
  for (size_t k = 0; k < m->allocs_count; k++) {
    swift_free("hashmap_gp", m->allocs[k]);
  }
  swift_free("hashmap_gp", m->allocs);
}

size_t hashmap_gp_size(hashmap_gp_t *m) {
  if (m != NULL)
    return m->size;
  else
    return 0;
...
#if HASHMAP_GP_DEBUG_OUTPUT
void hashmap_gp_count_chain_lengths(hashmap_gp_key_t key, hashmap_gp_value_t *value,
                                 void *data) {
  hashmap_gp_t *m = (hashmap_gp_t *)data;
  int count = 0;
  hashmap_gp_find(m, key, /*create_entry=*/0, &count,
               /*created_new_element=*/NULL);
...
void hashmap_gp_print_stats(hashmap_gp_t *m) {
  ...
  for (hashmap_gp_chunk_t *finger = m->graveyard; finger != NULL;
       finger = (hashmap_gp_chunk_t *)finger->next) {
    graveyard_counter += 1;
  }
  message(
      "populated chunks: %i (%zu kb), graveyard chunks: %i (%zu kb), allocs: "
      "%zu",
      chunk_counter, sizeof(hashmap_gp_chunk_t) * chunk_counter / 1024,
      graveyard_counter, sizeof(hashmap_gp_chunk_t) * graveyard_counter / 1024,
      m->allocs_count);
  ...
  for (int k = 0; k < HASHMAP_GP_MAX_CHAIN_LENGTH; k++) {
    m->chain_length_counts[k] = 0;
  }
  hashmap_gp_iterate(m, hashmap_gp_count_chain_lengths, m);
  ...
  for (size_t k = 0; k < m->nr_chunks; k++) {
    hashmap_gp_chunk_t *chunk = m->chunks[k];
    if (!chunk) {
      chunk_fill_ratio_counts[0] += 1;
    } else {
      int count = 0;
      for (int j = 0; j < HASHMAP_GP_MASKS_PER_CHUNK; j++) {
        count += __builtin_popcountll(chunk->masks[j]);
      }
      chunk_fill_ratio_counts[(int)(10 * count / HASHMAP_GP_MAX_CHUNK_FILL_RATIO /
                                    HASHMAP_GP_ELEMENTS_PER_CHUNK)] += 1;
    }
  }
  message("chunk fill ratios:");
  for (int k = 0; k < 11; k++) {
    message("  %3i%% - %3i%%: %zu (%.2f%%)",
            (int)(k * 10 * HASHMAP_GP_MAX_CHUNK_FILL_RATIO),
            (int)((k + 1) * 10 * HASHMAP_GP_MAX_CHUNK_FILL_RATIO),
            chunk_fill_ratio_counts[k],
            (100.0 * chunk_fill_ratio_counts[k]) / m->nr_chunks);
  }

  /* Print struct sizes. */
  message("sizeof(hashmap_gp_element_t): %zu", sizeof(hashmap_gp_element_t));
  message("sizeof(hashmap_gp_chunk_t): %zu", sizeof(hashmap_gp_chunk_t));
  message("HASHMAP_GP_TARGET_CHUNK_BYTES: %i", HASHMAP_GP_TARGET_CHUNK_BYTES);
  message("HASHMAP_GP_BITS_PER_ELEMENT: %i", HASHMAP_GP_BITS_PER_ELEMENT);
  message("HASHMAP_GP_ELEMENTS_PER_CHUNK: %i", HASHMAP_GP_ELEMENTS_PER_CHUNK);
  message("HASHMAP_GP_MASKS_PER_CHUNK: %i", HASHMAP_GP_MASKS_PER_CHUNK);
}
