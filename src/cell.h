...
enum cell_flags {
  ...
  cell_flag_do_sink_sub_sort = (1UL << 13),
  cell_flag_do_sink_drift = (1UL << 14),
  cell_flag_do_sink_sub_drift = (1UL << 15),
  cell_flag_do_stars_resort = (1UL << 16),
  cell_flag_do_sink_resort = (1UL << 17),
  cell_flag_has_tasks = (1UL << 18),
  cell_flag_do_hydro_sync = (1UL << 19),
  cell_flag_do_hydro_sub_sync = (1UL << 20),
  cell_flag_unskip_self_grav_processed = (1UL << 21),
  cell_flag_unskip_pair_grav_processed = (1UL << 22),
  cell_flag_skip_rt_sort = (1UL << 23),    /* skip rt_sort after a RT recv? */
  cell_flag_do_rt_sub_sort = (1UL << 24),  /* same as hydro_sub_sort for RT */
  cell_flag_rt_requests_sort = (1UL << 25) /* was this sort requested by RT? */
};
...
void cell_clear_sinks_sort_flags(struct cell *c, const int unused_flags);
...
/**
 * @brief Allocate sinks sort memory for cell.
 *
 * @param c The #cell that will require sorting.
 * @param flags Cell flags.
 */
__attribute__((always_inline)) INLINE static void cell_malloc_sinks_sorts(
    struct cell *c, const int flags) {

  const int count = c->sinks.count;

  /* Have we already allocated something? */
  if (c->sinks.sort != NULL) {

    /* Start by counting how many dimensions we need
       and how many we already have */
    const int num_arrays_wanted =
        intrinsics_popcount(c->sinks.sort_allocated | flags);
    const int num_already_allocated =
        intrinsics_popcount(c->sinks.sort_allocated);
    ...
    struct sort_entry *new_array = NULL;
    if ((new_array = (struct sort_entry *)swift_malloc(
             "sinks.sort", sizeof(struct sort_entry) * num_arrays_wanted *
                               (count + 1))) == NULL)
      error("Failed to allocate sort memory.");
    ...
    int to = 0;
    for (int j = 0; j < 13; j++) {
      if (c->sinks.sort_allocated & (1 << j)) {
        memcpy(&new_array[to * (count + 1)], &c->sinks.sort[from * (count + 1)],
               sizeof(struct sort_entry) * (count + 1));
        ++from;
        ++to;
      } else if (flags & (1 << j)) {
        ++to;
        c->sinks.sort_allocated |= (1 << j);
      }
    }

    /* Swap the pointers */
    swift_free("sinks.sort", c->sinks.sort);
    c->sinks.sort = new_array;

  } else {

    c->sinks.sort_allocated = flags;
    ...
    /* If there is anything, allocate enough memory */
    if (num_arrays) {
      if ((c->sinks.sort = (struct sort_entry *)swift_malloc(
               "sinks.sort",
               sizeof(struct sort_entry) * num_arrays * (count + 1))) == NULL)
        error("Failed to allocate sort memory.");
    }
...
/**
 * @brief Free sinks sort memory for cell.
 *
 * @param c The #cell.
 */
__attribute__((always_inline)) INLINE static void cell_free_sinks_sorts(
    struct cell *c) {
 ...
 */
__attribute__((always_inline)) INLINE static struct sort_entry *
cell_get_sinks_sorts(const struct cell *c, const int sid) {

#ifdef SWIFT_DEBUG_CHECKS
  if (sid >= 13 || sid < 0) error("Invalid sid!");

  if (!(c->sinks.sort_allocated & (1 << sid)))
    error("Sort not allocated along direction %d", sid);
#endif
  ...
  /* We need to find at what position in the meta-array of
     ...
     the number of bits that are in the results to obtain the position
     of the correspondin sid in the meta-array */
  const int j = intrinsics_popcount(c->sinks.sort_allocated & ((1 << sid) - 1));

  /* Return the corresponding array */
  return &c->sinks.sort[j * (c->sinks.count + 1)];
}
