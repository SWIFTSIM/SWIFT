...
__attribute__((always_inline)) INLINE static void runner_iact_force(
 ...
 if (!pi->is_h_max && !pj->is_h_max) {   
      if (pi->mat_id == pj->mat_id){  
          ...
          float alpha_u = 0.1f + mean_balsara * 0.9f;//0.05f;//0.5f;//1.f;//0.1f;//1.f 
          ...
          float alpha_rho = 0.1f + mean_balsara * 0.9f;//0.05f;//0.5f;//1.f;//0.1f;//1.f 
...
__attribute__((always_inline)) INLINE static void runner_iact_nonsym_force(
  ...
  if (!pi->is_h_max && !pj->is_h_max) {  
      if (pi->mat_id == pj->mat_id){  
          ...
          float alpha_rho = 0.1f + mean_balsara * 0.9f;//0.05f;//0.5f;//1.f;//0.1f; 
          float drho_dt_cond_i = -alpha_rho * mj * (pi->rho / pj->rho) * v_sig_cond * (rhotilde_i - rhotilde_j) * mean_G / mean_rho; 
