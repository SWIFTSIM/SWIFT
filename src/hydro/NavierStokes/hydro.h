...
__attribute__((always_inline)) INLINE static void hydro_prepare_force(
   ...
   if(p->dvx_xx != p->dvx_xx) error("dvx_xx is NaN");
   p->tau_xx = 2.0 * p->viscosity * (eta_xx - 0.3333333333f * p->div_v);
   p->tau_xy = 2.0 * p->viscosity * eta_xy;
   p->tau_yx = p->tau_xy;
   p->tau_xz = 2.0 * p->viscosity * eta_xz;
   p->tau_zx = p->tau_xz;
   p->tau_yy = 2.0 * p->viscosity * (eta_yy - 0.3333333333f * p->div_v);
   p->tau_yz = 2.0 * p->viscosity * eta_yz;
   p->tau_zy = p->tau_yz;
   p->tau_zz = 2.0 * p->viscosity * (eta_zz - 0.3333333333f * p->div_v);
}
...
__attribute__((always_inline)) INLINE static void hydro_end_force(
  ...
  if(p->is_boundary){
    ...
    p->a_hydro[1] = 0.0;
    p->a_hydro[2] = 0.0;
  }/*else if( p->id == 5000000){
    printf("Particle a_hydro[1] = %f, a_hydro[0] = %.15f, a_constant = %f\n",p->a_hydro[1],p->a_hydro[0], p->a_constant[0]);
  }*/
