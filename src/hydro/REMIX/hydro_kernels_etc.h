...
hydro_runner_iact_density_extra_kernel(struct part *restrict pi,
  ...
  for (int i = 0; i < 3; i++) {
    pi->grad_m0_no_mean_kernel[i] +=
        (pj->mass / pj->rho_evolved) * dx[i] * wi_dx * r_inv;
    pj->grad_m0_no_mean_kernel[i] +=
        (pi->mass / pi->rho_evolved) * (-dx[i] * wj_dx * r_inv);
  }
}
...
hydro_runner_iact_gradient_extra_kernel(struct part *restrict pi,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    pi->gradient.grad_m1_term1_gradhterm[i] +=
        dx[i] * wi_dx_gradhterm * volume_j;
    pj->gradient.grad_m1_term1_gradhterm[i] +=
  ...
  pi->gradient.grad_m2_term1_gradhterm.xx +=
      dx[0] * dx[0] * wi_dx_gradhterm * volume_j;
  pi->gradient.grad_m2_term1_gradhterm.yy +=
      dx[1] * dx[1] * wi_dx_gradhterm * volume_j;
  pi->gradient.grad_m2_term1_gradhterm.zz +=
      dx[2] * dx[2] * wi_dx_gradhterm * volume_j;
  pi->gradient.grad_m2_term1_gradhterm.xy +=
      dx[0] * dx[1] * wi_dx_gradhterm * volume_j;
  pi->gradient.grad_m2_term1_gradhterm.xz +=
      dx[0] * dx[2] * wi_dx_gradhterm * volume_j;
  pi->gradient.grad_m2_term1_gradhterm.yz +=
      dx[1] * dx[2] * wi_dx_gradhterm * volume_j;

  pj->gradient.grad_m2_term1_gradhterm.xx +=
      dx[0] * dx[0] * wj_dx_gradhterm * volume_i;
  pj->gradient.grad_m2_term1_gradhterm.yy +=
      dx[1] * dx[1] * wj_dx_gradhterm * volume_i;
  pj->gradient.grad_m2_term1_gradhterm.zz +=
      dx[2] * dx[2] * wj_dx_gradhterm * volume_i;
  pj->gradient.grad_m2_term1_gradhterm.xy +=
      dx[0] * dx[1] * wj_dx_gradhterm * volume_i;
  pj->gradient.grad_m2_term1_gradhterm.xz +=
      dx[0] * dx[2] * wj_dx_gradhterm * volume_i;
  pj->gradient.grad_m2_term1_gradhterm.yz +=
      dx[1] * dx[2] * wj_dx_gradhterm * volume_i;
...
hydro_end_gradient_extra_kernel(struct part *restrict p) {
  ...
  p->gradient.grad_m0_gradhterm -=
      0.5f * volume * hydro_dimension * kernel_root * h_inv_dim_plus_one;

  for (i = 0; i < 3; i++) {
    p->gradient.grad_m0[i] +=
        p->gradient.grad_m0_gradhterm * p->dh_norm_kernel[i];
    for (j = 0; j < 3; j++) {
      p->gradient.grad_m1_term1[i][j] +=
          p->gradient.grad_m1_term1_gradhterm[j] * p->dh_norm_kernel[i];
    }
  }

  p->gradient.grad_m2_term1_x.xx +=
      p->gradient.grad_m2_term1_gradhterm.xx * p->dh_norm_kernel[0];
  p->gradient.grad_m2_term1_x.yy +=
      p->gradient.grad_m2_term1_gradhterm.yy * p->dh_norm_kernel[0];
  p->gradient.grad_m2_term1_x.zz +=
      p->gradient.grad_m2_term1_gradhterm.zz * p->dh_norm_kernel[0];
  p->gradient.grad_m2_term1_x.xy +=
      p->gradient.grad_m2_term1_gradhterm.xy * p->dh_norm_kernel[0];
  p->gradient.grad_m2_term1_x.xz +=
      p->gradient.grad_m2_term1_gradhterm.xz * p->dh_norm_kernel[0];
  p->gradient.grad_m2_term1_x.yz +=
      p->gradient.grad_m2_term1_gradhterm.yz * p->dh_norm_kernel[0];

  p->gradient.grad_m2_term1_y.xx +=
      p->gradient.grad_m2_term1_gradhterm.xx * p->dh_norm_kernel[1];
  p->gradient.grad_m2_term1_y.yy +=
      p->gradient.grad_m2_term1_gradhterm.yy * p->dh_norm_kernel[1];
  p->gradient.grad_m2_term1_y.zz +=
      p->gradient.grad_m2_term1_gradhterm.zz * p->dh_norm_kernel[1];
  p->gradient.grad_m2_term1_y.xy +=
      p->gradient.grad_m2_term1_gradhterm.xy * p->dh_norm_kernel[1];
  p->gradient.grad_m2_term1_y.xz +=
      p->gradient.grad_m2_term1_gradhterm.xz * p->dh_norm_kernel[1];
  p->gradient.grad_m2_term1_y.yz +=
      p->gradient.grad_m2_term1_gradhterm.yz * p->dh_norm_kernel[1];

  p->gradient.grad_m2_term1_z.xx +=
      p->gradient.grad_m2_term1_gradhterm.xx * p->dh_norm_kernel[2];
  p->gradient.grad_m2_term1_z.yy +=
      p->gradient.grad_m2_term1_gradhterm.yy * p->dh_norm_kernel[2];
  p->gradient.grad_m2_term1_z.zz +=
      p->gradient.grad_m2_term1_gradhterm.zz * p->dh_norm_kernel[2];
  p->gradient.grad_m2_term1_z.xy +=
      p->gradient.grad_m2_term1_gradhterm.xy * p->dh_norm_kernel[2];
  p->gradient.grad_m2_term1_z.xz +=
      p->gradient.grad_m2_term1_gradhterm.xz * p->dh_norm_kernel[2];
  p->gradient.grad_m2_term1_z.yz +=
      p->gradient.grad_m2_term1_gradhterm.yz * p->dh_norm_kernel[2];
}
...
hydro_prepare_force_extra_kernel(struct part *restrict p) {
  ...
  if (!p->is_h_max) {
    ...
    float m2_inv_mult_grad_m1_x[3], m2_inv_mult_grad_m1_y[3],
        m2_inv_mult_grad_m1_z[3];
    ...
    sym_matrix_multiplication_ABA(&m2_inv_mult_grad_m2_mult_m2_inv_x, &m2_inv,
                                  &grad_m2_x);
    struct sym_matrix m2_inv_mult_grad_m2_mult_m2_inv_y;
    sym_matrix_multiplication_ABA(&m2_inv_mult_grad_m2_mult_m2_inv_y, &m2_inv,
                                  &grad_m2_y);
    struct sym_matrix m2_inv_mult_grad_m2_mult_m2_inv_z;
    sym_matrix_multiplication_ABA(&m2_inv_mult_grad_m2_mult_m2_inv_z, &m2_inv,
                                  &grad_m2_z);

    float ABA_mult_m1_x[3], ABA_mult_m1_y[3], ABA_mult_m1_z[3];
    sym_matrix_multiply_by_vector(
        ABA_mult_m1_x, &m2_inv_mult_grad_m2_mult_m2_inv_x, p->gradient.m1);
    sym_matrix_multiply_by_vector(
        ABA_mult_m1_y, &m2_inv_mult_grad_m2_mult_m2_inv_y, p->gradient.m1);
    sym_matrix_multiply_by_vector(
        ABA_mult_m1_z, &m2_inv_mult_grad_m2_mult_m2_inv_z, p->gradient.m1);

    float A, B[3], grad_A[3], grad_B[3][3];

    // Calculata A
    A = p->gradient.m0;
    A -= m2_inv_mult_m1[0] * p->gradient.m1[0] +
         m2_inv_mult_m1[1] * p->gradient.m1[1] +
         m2_inv_mult_m1[2] * p->gradient.m1[2];
    A = 1 / A;
    ...
    grad_A[0] += ABA_mult_m1_x[0] * p->gradient.m1[0] +
                 ABA_mult_m1_x[1] * p->gradient.m1[1] +
                 ABA_mult_m1_x[2] * p->gradient.m1[2];
    grad_A[1] += ABA_mult_m1_y[0] * p->gradient.m1[0] +
                 ABA_mult_m1_y[1] * p->gradient.m1[1] +
                 ABA_mult_m1_y[2] * p->gradient.m1[2];
    grad_A[2] += ABA_mult_m1_z[0] * p->gradient.m1[0] +
                 ABA_mult_m1_z[1] * p->gradient.m1[1] +
                 ABA_mult_m1_z[2] * p->gradient.m1[2];
    ...
    if (x > offset) {
      float sigma = 0.2f;
      p->vac_switch =
          expf(-(x - offset) * (x - offset) / (2.f * sigma * sigma));
    }
...
__attribute__((always_inline)) INLINE static void hydro_set_Gi_Gj_forceloop(
  ...
  if (!pi->is_h_max && !pj->is_h_max) {
    ...
    for (i = 0; i < 3; i++) {
      wi_dx_term[i] =
          dx[i] * r_inv * 0.5f * (wi_dx * hid_inv + wj_dx * hjd_inv);
      wj_dx_term[i] =
          -dx[i] * r_inv * 0.5f * (wi_dx * hid_inv + wj_dx * hjd_inv);
    }
    ...
    for (i = 0; i < 3; i++) {
      wi_dx_term[i] += -0.5f * (hydro_dimension * wi + (r / pi->h) * wi_dx) *
                       hid_inv * pi->dh_norm_kernel[i];
      wj_dx_term[i] += -0.5f * (hydro_dimension * wj + (r / pj->h) * wj_dx) *
                       hjd_inv * pj->dh_norm_kernel[i];
    }

    for (i = 0; i < 3; i++) {
      modified_grad_wi[i] = pi->force.A * wi_dx_term[i] +
                            pi->force.grad_A[i] * wi_term +
                            pi->force.A * pi->force.B[i] * wi_term;
      modified_grad_wj[i] = pj->force.A * wj_dx_term[i] +
                            pj->force.grad_A[i] * wj_term +
                            pj->force.A * pj->force.B[i] * wj_term;
    }

    for (i = 0; i < 3; i++) {
      for (j = 0; j < 3; j++) {
        modified_grad_wi[i] +=
            pi->force.A * pi->force.B[j] * dx[j] * wi_dx_term[i];
        modified_grad_wi[i] +=
            pi->force.grad_A[i] * pi->force.B[j] * dx[j] * wi_term;
        modified_grad_wi[i] +=
            pi->force.A * pi->force.grad_B[i][j] * dx[j] * wi_term;

        modified_grad_wj[i] +=
            -pj->force.A * pj->force.B[j] * dx[j] * wj_dx_term[i];
        modified_grad_wj[i] +=
            -pj->force.grad_A[i] * pj->force.B[j] * dx[j] * wj_term;
        modified_grad_wj[i] +=
            -pj->force.A * pj->force.grad_B[i][j] * dx[j] * wj_term;
      }
    }

    float modified_wi = pi->force.A * wi_term;
    float modified_wj = pj->force.A * wj_term;
    modified_wi += pi->force.A * pi->force.B[0] * dx[0] * wi_term +
                   pi->force.A * pi->force.B[1] * dx[1] * wi_term +
                   pi->force.A * pi->force.B[2] * dx[2] * wi_term;
    modified_wj += -(pj->force.A * pj->force.B[0] * dx[0] * wj_term +
                     pj->force.A * pj->force.B[1] * dx[1] * wj_term +
                     pj->force.A * pj->force.B[2] * dx[2] * wj_term);
