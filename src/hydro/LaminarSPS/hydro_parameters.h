...
struct viscosity_global_data {
    float initial_particle_distance;
...
static INLINE void viscosity_init(struct swift_params* params,
  ...
  viscosity->initial_particle_distance = parser_get_param_float(
      params, "SPH:inter_particle");
#ifdef HYDRO_DIMENSION_2D
    const double int_part_distance_dimensional = sqrt(viscosity->initial_particle_distance * viscosity->initial_particle_distance * 2.0) / 2.0;
    viscosity->Smagorinsky_constant =(0.12 * int_part_distance_dimensional);
    viscosity->Smagorinsky_constant = viscosity->Smagorinsky_constant * viscosity->Smagorinsky_constant;
    viscosity->Blin_constant = (float) (2.0/3.0)*0.0066 * int_part_distance_dimensional*int_part_distance_dimensional;

#endif

#ifdef HYDRO_DIMENSION_3D
    const double int_part_distance_dimensional = sqrt(viscosity->initial_particle_distance * viscosity->initial_particle_distance * 3.0) / 3.0;
    viscosity->Smagorinsky_constant = (0.12 * int_part_distance_dimensional);
    viscosity->Smagorinsky_constant = viscosity->Smagorinsky_constant * viscosity->Smagorinsky_constant;
    viscosity->Blin_constant = (float) (2.0/3.0)*0.0066 * int_part_distance_dimensional*int_part_distance_dimensional;
...
static INLINE void viscosity_init_no_hydro(
    struct viscosity_global_data* viscosity) {
  viscosity->initial_particle_distance = 1.0f;
}
...
static INLINE void viscosity_print(
  ...
  message("Artificial viscosity parameters set to initial_particle_distance: %.3f",
          viscosity->initial_particle_distance);
...
static INLINE void viscosity_print_snapshot(
  ...
  io_write_attribute_f(h_grpsph, "initial_particle_distance", viscosity->initial_particle_distance);
}
#endif
