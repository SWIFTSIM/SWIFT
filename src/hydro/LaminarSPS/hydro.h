...
__attribute__((always_inline)) INLINE static float hydro_get_physical_pressure(
  ...
  return pressure_from_density(p->rho, p->rho_base);
#else
...
__attribute__((always_inline)) INLINE static float hydro_compute_timestep(
  ...
  const float a_x = p->a_hydro[0] + p->a_constant[0];
  const float a_y = p->a_hydro[1] + p->a_constant[1];
  const float a_z = p->a_hydro[2] + p->a_constant[2];
  const float a = sqrt(a_x*a_x + a_y*a_y + a_z*a_z);
  float dt_f;
  if(a > 0){
    dt_f = FLT_MAX;
  }else{
...
__attribute__((always_inline)) INLINE static void hydro_prepare_force(
    ...
    const float prr = diagonal_sq+diagonal_sq + p->grad_v_xy*p->grad_v_xy + p->grad_v_xz*p->grad_v_xz + p->grad_v_yz*p->grad_v_yz;

    const float viscosity_sps = hydro_props->viscosity.Smagorinsky_constant * sqrt(prr);
 
    const float div_v = p->grad_v_xx + p->grad_v_yy + p->grad_v_zz;

    const float sps_k = (2.0f/3.0f) * viscosity_sps * div_v;

    const float blin = hydro_props->viscosity.Blin_constant * prr;

    const float sumsps = -(sps_k + blin);

    const float two_viscosity_sps = viscosity_sps + viscosity_sps;
 
    const float inv_rho = 1.0f / p->rho;

    /* Diagonal tau values */
    p->tau_xx = inv_rho * (two_viscosity_sps * p->grad_v_xx + sumsps);
    p->tau_yy = inv_rho * (two_viscosity_sps * p->grad_v_yy + sumsps);
    p->tau_zz = inv_rho * (two_viscosity_sps * p->grad_v_zz + sumsps);
    p->tau_xy = inv_rho * (viscosity_sps * p->grad_v_xy);
    p->tau_xz = inv_rho * (viscosity_sps * p->grad_v_xz);
    p->tau_yz = inv_rho * (viscosity_sps * p->grad_v_yz);
...
__attribute__((always_inline)) INLINE static void hydro_reset_acceleration(
  ...
  reset_diffusive_term(p);  
  reset_shifting_data(p);
}
...
#if defined(EOS_MULTIFLUID_TAIT)
  const float pressure = pressure_from_density(p->rho, p->rho_base);
#else
