...
INLINE static void hydro_read_particles(struct part* parts,
  ...
  *num_fields = 8;
  ...
  list[2] = io_make_input_field("Masses", DOUBLE, 1, COMPULSORY, UNIT_CONV_MASS,
                                parts, mass);
  list[3] = io_make_input_field("SmoothingLength", DOUBLE, 1, COMPULSORY,
                                UNIT_CONV_LENGTH, parts, h);
  list[4] = io_make_input_field("InternalEnergy", DOUBLE, 1, COMPULSORY,
                                UNIT_CONV_ENERGY_PER_UNIT_MASS, parts, u);
  list[5] = io_make_input_field("ParticleIDs", ULONGLONG, 1, COMPULSORY,
                                UNIT_CONV_NO_UNITS, parts, id);
  list[6] = io_make_input_field("Accelerations", DOUBLE, 3, OPTIONAL,
...
INLINE static void convert_part_pos(const struct engine* e,
  ...
  if (e->snapshot_use_delta_from_edge) {
    ret[0] = fmin(ret[0], s->dim[0] - e->snapshot_delta_from_edge);
    ret[1] = fmin(ret[1], s->dim[1] - e->snapshot_delta_from_edge);
    ret[2] = fmin(ret[2], s->dim[2] - e->snapshot_delta_from_edge);
  }
}
...
INLINE static void convert_part_vel(const struct engine* e,
  ...
  const double dt_kick_grav_mesh = e->dt_kick_grav_mesh_for_io;

  const integertime_t ti_beg = get_integer_time_begin(ti_current, p->time_bin);
  const integertime_t ti_end = get_integer_time_end(ti_current, p->time_bin);

  /* Get time-step since the last kick */
  double dt_kick_grav, dt_kick_hydro;
  if (with_cosmology) {
...
INLINE static void hydro_write_particles(const struct part* parts,
  ...
  *num_fields = 16;
  ...
  list[1] = io_make_output_field_convert_part(
      "Velocities", DOUBLE, 3, UNIT_CONV_SPEED, 0.f, parts, xparts,
      convert_part_vel,
  ...
  list[3] = io_make_output_field(
      "SmoothingLengths", DOUBLE, 1, UNIT_CONV_LENGTH, 1.f, parts, h,
      "Co-moving smoothing lengths (FWHM of the kernel) of the particles");

  list[4] = io_make_output_field(
      "InternalEnergies", DOUBLE, 1, UNIT_CONV_ENERGY_PER_UNIT_MASS,
  ...
  list[6] = io_make_output_field("Densities", DOUBLE, 1, UNIT_CONV_DENSITY, -3.f,
                                 parts, rho,
                                 "Co-moving mass densities of the particles");

  list[7] = io_make_output_field_convert_part(
      "Entropies", DOUBLE, 1, UNIT_CONV_ENTROPY_PER_UNIT_MASS, 0.f, parts,
      xparts, convert_S, "Co-moving entropies per unit mass of the particles");

  list[8] = io_make_output_field_convert_part(
      "Pressures", DOUBLE, 1, UNIT_CONV_PRESSURE, -3.f * hydro_gamma, parts,
      xparts, convert_P, "Co-moving pressures of the particles");

  list[9] = io_make_output_field_convert_part(
      "ViscosityParameters", DOUBLE, 1, UNIT_CONV_NO_UNITS, 0.f, parts, xparts,
      convert_viscosity, "Visosity coefficient (alpha_visc) of the particles");

  list[10] = io_make_output_field_convert_part(
      "DiffusionParameters", DOUBLE, 1, UNIT_CONV_NO_UNITS, 0.f, parts, xparts,
      convert_diffusion, "Diffusion coefficient (alpha_diff) of the particles");

  list[11] = io_make_output_field(
      "LaplacianInternalEnergies", DOUBLE, 1, UNIT_CONV_FREQUENCY_SQUARED,
  ...
  list[12] = io_make_output_field(
      "VelocityDivergences", DOUBLE, 1, UNIT_CONV_FREQUENCY, 0.f, parts,
      viscosity.div_v,
  ...
  list[13] = io_make_output_field(
      "VelocityDivergenceTimeDifferentials", DOUBLE, 1,
  ...
  list[14] = io_make_output_field_convert_part(
      "Potentials", DOUBLE, 1, UNIT_CONV_POTENTIAL, -1.f, parts, xparts,
  ...
  list[15] = io_make_output_field_convert_part(
      "Softenings", DOUBLE, 1, UNIT_CONV_LENGTH, 1.f, parts, xparts,
