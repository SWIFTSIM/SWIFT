...
__attribute__((always_inline)) INLINE static void runner_iact_force(
  ...
  if ((pi->mass > 8.5e-5) && (pi->time_bin < 38) && (pi->force.pressure) && (pi->time_bin > 0)){
    ...
    message("  -> neighbour pj: id=%lld mass=%e rho=%e P=%e pos=(%g,%g,%g) r=%e",
            pj->id, pj->mass, pj->rho, pj->force.pressure,
            pj->x[0], pj->x[1], pj->x[2], sqrtf(r2));
  }
  if ((pj->mass > 8.5e-5) && (pj->time_bin < 38) && (pj->force.pressure > 1) && (pj->time_bin > 0)){
    ...
    message("  -> neighbour pi: id=%lld mass=%e rho=%e P=%e pos=(%g,%g,%g) r=%e",
            pi->id, pi->mass, pi->rho, pi->force.pressure,
            pi->x[0], pi->x[1], pi->x[2], sqrtf(r2));
  }
