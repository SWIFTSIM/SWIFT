...
__attribute__((always_inline)) INLINE static void hydro_debug_particle(
  ...
  warning("[PID%lld] decoupled: %d", p->id, p->decoupled);
  ...
  warning("[PID%lld] a=[%.3e,%.3e,%.3e]", p->id, p->a_hydro[0], p->a_hydro[1],
          p->a_hydro[2]);
  warning("[PID%lld] u=%.3e, du/dt=%.3e P=%.3e", p->id, p->u, p->u_dt,
  ...
  warning(
      ...
      "[%.3e,%.3e,%.3e],"
      "[%.3e,%.3e,%.3e]]",
      p->id, p->time_bin, p->rho, p->gradients.velocity_tensor[0][0],
      p->gradients.velocity_tensor[0][1], p->gradients.velocity_tensor[0][2],
      p->gradients.velocity_tensor[1][0], p->gradients.velocity_tensor[1][1],
      p->gradients.velocity_tensor[1][2], p->gradients.velocity_tensor[2][0],
      p->gradients.velocity_tensor[2][1], p->gradients.velocity_tensor[2][2]);
