...
__attribute__((always_inline)) INLINE static void hydro_velocities_set(
  ...
  if (!fix_particle && p->conserved.mass > 0.0f && p->rho > 0.0f) {
    ...
    if (d == 0.0f) {
      error("Distance to centroid is zero! Abort steering");
    }
 ...
 * being off, but SHADOWSWIFT_STEER_MOTION being on*/
    ...
    if (d > 0.9f * etaR || d > p->geometry.min_face_dist) {
      ...
      if (fac_dt < fac) {
        ...
        //
        // float Egrav = p->conserved.mass * sqrtf(g[0] * g[0] + g[1] * g[1] +
        // g[2] * g[2]) *
        ...
        //                                  soundspeed;
        //
        // const float Ekin_cold = 0.5f * p->conserved.mass * (v[0] * v[0] +
        // v[1] * v[1] + v[2] * v[2]);
        ...
        //   // message("here! Thermal = %g, Ekin + thermal = %g, Egrav = %g,
        //   mach_cold = %g, dens = %g",
        //   //   thermal_energy, p->timestepvars.Ekin + thermal_energy, Egrav,
        //   mach_cold, p->rho);
        //
        ...
        //
        //   message("here! Thermal = %g, Ekinmax + thermal = %g, Ekin_cold =
        //   %g, Egrav = %g, mach_cold = %g, dens = %g, mass = %g, massflux =
        //   %g",
        //     thermal_energy, p->timestepvars.Ekin + thermal_energy, Ekin_cold,
        //     Egrav, mach_cold, p->rho, p->conserved.mass, p->flux.mass);
        //
        //
        //
        //   fac = fac_dt;
        // } // Not so sure this works... Egrav term still isnt enough. Plenty
        // of cold unperturbed gas at centres
        ...
        const double T_transition =
            hydro_properties->hydrogen_ionization_temperature;
        ...
        //   %g, Egrav = %g, mach_cold = %g, dens = %g, mass = %g, massflux =
        //   %g",
        //   //   thermal_energy, p->timestepvars.Ekin + thermal_energy,
        //   Ekin_cold, Egrav, mach_cold, p->rho, p->conserved.mass,
        //   p->flux.mass);
        //
        //
        //
        ...
        // ) {
        //
        //   // message("here! Thermal = %g, Ekin_cold = %g, Egrav = %g,
        //   mach_cold = %g, dens = %g, mass = %g, massflux = %g, potential E =
        //   %g",
        //   //   thermal_energy, Ekin_cold, Egrav, mach_cold, p->rho,
        //   p->conserved.mass, p->flux.mass, p->gpart->potential *
        //   p->conserved.mass);
        //
        //   // if (thermal_energy > 1e-5 * (p->timestepvars.Ekin +
        //   thermal_energy)) {
        //   //   message("Ekin_max = %g, p.mach = %g,", p->timestepvars.Ekin,
        //   p->timestepvars.mach_number);
        //   // }
        //
        //   fac = fac_dt;
        //
        // } /* SEEMINGLY LOVES TO APPLY TO EXACT WRONG PARTICLES!, actually in
        // tests it seems fine? IDK why it does that. In restarts from Mach50 it
        // was good? Redo. Mach50 AND bad idea sadly SADLY THIS LEADS TO NaNs
        // and CRASHES WITH EXPLODING SHELLS!*/
