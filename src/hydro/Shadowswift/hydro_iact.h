...
__attribute__((always_inline)) INLINE static void runner_iact_flux_exchange(
  ...
  float v_rel_j[3] = {pj->v[0] - pi->v[0], pj->v[1] - pi->v[1],
                      pj->v[2] - pi->v[2]};
  ...
  if (pj->flux.dt >= 0) {
    float v_rel_i[3] = {pi->v[0] - pj->v[0], pi->v[1] - pj->v[1],
                        pi->v[2] - pj->v[2]};
  ...
  float vij[3];
  hydro_get_interface_velocity(pi->v_part_full, pj->v_part_full, dx, r2,
  ...
  float totflux[6]; // Curently in units X * s^-1, will be * dt later
