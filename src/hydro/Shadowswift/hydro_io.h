 ...
 *               2024 Yolan Uyttenhove (Yolan.Uyttenhove@UGent.be)
 *
 ...
 * @brief ShadowSWIFT hydrodynamics scheme (Moving mesh)
 */
...
/* Set the description of the gradient implementation */
#if defined(SHADOWSWIFT_MESHLESS_GRADIENTS)
#define SHADOWSWIFT_GRADIENT_IMPLEMENTATION "Meshless gradients"
#elif defined(SHADOWSWIFT_GRADIENTS_WLS)
#define SHADOWSWIFT_GRADIENT_IMPLEMENTATION  \
  "WLS gradients using Voronoi neighbours"
#elif defined(SHADOWSWIFT_GRADIENTS)
#define SHADOWSWIFT_GRADIENT_IMPLEMENTATION "Springel (2010) gradients"
#else
#define SHADOWSWIFT_GRADIENT_IMPLEMENTATION "No gradients"
#endif
...
INLINE static void hydro_read_particles(struct part* parts,
  ...
  *num_fields = 8;
  warning("Part Size: %lu", sizeof(struct part));
  ...
  *num_fields = 14;
  ...
  list[13] =
      io_make_output_field("Flux_counts", LONGLONG, 1, UNIT_CONV_NO_UNITS, 0.f,
                           parts, flux.count, "Flux counters of the particles");
#endif
}
...
INLINE static void hydro_write_flavour(hid_t h_grpsph) {
  ...
  io_write_attribute_s(h_grpsph, "Gradient reconstruction model",
                       SHADOWSWIFT_GRADIENT_IMPLEMENTATION);

  /* Slope limiter information (TODO) */
