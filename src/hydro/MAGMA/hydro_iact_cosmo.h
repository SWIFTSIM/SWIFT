...
__attribute__((always_inline)) INLINE static void runner_iact_force(
  ...
  uj_mid = pj->u + F_ji * (uj_fder + 0.5f * uj_sder);
  ...
  const float dvdh_i = (vi_mid[0] - vj_mid[0]) * h_i[0] + (vi_mid[1] - vj_mid[1]) * h_i[1] + (vi_mid[2] - vj_mid[2]) * h_i[2] + a2_Hubble * r2 / hi;
  const float dvdh_j = (vj_mid[0] - vi_mid[0]) * h_j[0] + (vj_mid[1] - vi_mid[1]) * h_j[1] + (vj_mid[2] - vi_mid[2]) * h_j[2] + a2_Hubble * r2 / hj;
  ...
  const float v_diffg = sqrtf((vi_mid[0] - vj_mid[0]) * (vi_mid[0] - vj_mid[0]) + (vi_mid[1] - vj_mid[1]) * (vi_mid[1] - vj_mid[1]) + (vi_mid[2] - vj_mid[2]) * (vi_mid[2] - vj_mid[2])) + H * a * r;
  const float iG = 0.5f;
