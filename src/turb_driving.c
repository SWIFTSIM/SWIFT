...
/***************************************/
const int TurbDriving_Global_DrivingSpectrumKey = 0;             // driving pwr-spec: 0=Ek~const; 1=sharp-peak at kc; 2=Ek~k^(-5/3); 3=Ek~k^-2
const int TurbDriving_Global_DrivingRandomNumberKey = 1;         // random number seed for modes
const double TurbDriving_Global_DrivingScaleKMinVar = 1.0;       // minimum driving-k: should be ~2.*M_PI/All.BoxSize --> TurbDrive_MaxWavelength
const double TurbDriving_Global_DrivingScaleKMaxVar = 2.0;       // maximum driving-k: set to couple times Kmin or more if more cascade desired   // should be < MaxWavelength
const double TurbDriving_Global_AccelerationPowerVariable = 1.0; // energy of driving-scale modes: sets norm of turb --> TurbDrive_ApproxRMSVturb
const double TurbDriving_Global_DecayTime = 0.;                  // decay time for driving-mode phase correlations   --> TurbDrive_CoherenceTime
const double TurbDriving_Global_SolenoidalFraction = 0.5;        // fractional wt of solenoidal modes (wt*curl + (1-wt)*div) --> TurbDrive_SolenoidalFraction
const double TurbDriving_Global_DtTurbUpdates = 1.0;             // time interval for driving updates (set by hand)  --> TurbDrive_TimeBetweenTurbUpdates
/***************************************/
...
double st_return_mode_correlation_time(void)
{
    if(TurbDriving_Global_DecayTime > 0)
      ...
      return st_return_driving_scale() / TurbDriving_Global_AccelerationPowerVariable;
}
...
{
    return TurbDriving_Global_AccelerationPowerVariable / st_return_mode_correlation_time(); // new convention, hoping this is more clear re: meaning of variable
}

/* initialize phase variables */
void st_turbdrive_init_ouseq(void)
{
  for(int i = 0;i<6*StNModes;i++) {StOUPhases[i] = st_turbdrive_get_gaussian_random_variable()*st_return_rms_acceleration();}
}
...
{
    double damping = exp( -st_return_dt_between_updates()/st_return_mode_correlation_time());
    for(int i = 0;i<6*StNModes;i++) {
      StOUPhases[i] = StOUPhases[i] * damping + st_return_rms_acceleration() * sqrt(1.-damping*damping)*st_turbdrive_get_gaussian_random_variable();
    }
}
...
void st_turbdrive_calc_phases(void)
{
    ...
    {
        double ka = 0., kb = 0., kk = 0.; int dim = hydro_dimension;
        for(j = 0; j<dim;j++)
        {
        ...
        for(j = 0; j<dim;j++)
        {
            ...
            StAka[3*i+j] = TurbDriving_Global_SolenoidalFraction*curla+(1. - TurbDriving_Global_SolenoidalFraction)*divb;
            StAkb[3*i+j] = TurbDriving_Global_SolenoidalFraction*curlb+(1. - TurbDriving_Global_SolenoidalFraction)*diva;
        }
    }
}
  ...
  const int count = c->hydro.count;
    ...
    if(delta >= Dt_Update)
    {
	   ...
	   double e_diss_sum=0, e_drive_sum=0, glob_diss_sum=0, glob_drive_sum=0;
           //message(" ..updating fields tracked for following injected energy and dissipation");
	...
	// MATTHIEU: Need to do this only once!!!
        ...
        st_update_ouseq();
        message(" ..calculating coefficients and phases following desired projection");
        st_turbdrive_calc_phases();
        StTPrev = StTPrev + Dt_Update / e->time_base;
        message(" ..updated turbulent stirring field at time %f", StTPrev * e->time_base);
    }
}
...
void init_turb(const struct space *s, const struct engine *e)
{
  ...
  kmax = 2.*M_PI / TurbDriving_Global_DrivingScaleKMaxVar; // convert these from spatial lengths to wavenumbers in the new convention we are using
    ...
    message("Initializing turbulent driving: max integer mode number ikx/iky/ikz = %d %d %d",ikxmax,ikymax,ikzmax);
    ...
    StOUPhases = (double*) swift_malloc("StOUPhases", StNModes * 6 * sizeof(double));
            ...
            for(ikz = 0;ikz <= ikzmax; ikz++)
            {
                ...
                if(k>=kmin && k<=kmax)
                {
                    ...
                    else if(TurbDriving_Global_DrivingSpectrumKey == 2)
                    {
                        ...
                        ampl = pow(k/kmin, 1./3. - 0*0.5*hydro_dimension); // this should scale as the acceleration per eddy, ~v^2/L, crudely
                    }
                    else if(TurbDriving_Global_DrivingSpectrumKey == 3)
                    {
                        ...
                        ampl = pow(k/kmin, 0. - 0*0.5*hydro_dimension); // this should scale as the acceleration per eddy, ~v^2/L, crudely
                    }
                    else
                    {
                    ...
                    message("  Mode: %d, ikx=%d, iky=%d, ikz=%d, kx=%f, ky=%f, kz=%f, ampl=%f",StNModes,ikx,iky,ikz,StMode[3*StNModes+0],StMode[3*StNModes+1],StMode[3*StNModes+2],StAmpl[StNModes]);
    ...
    for(int i=0; i<StNModes; i++) {StAmpl[i] *= sqrt(1./amplitude_integrated_allmodes);} // normalize total driving amplitude across all modes here
    ...
    gsl_rng_set(StRng, TurbDriving_Global_DrivingRandomNumberKey); // initialize seed
    for(int j=0;j<100;j++) {double tmp; tmp=st_turbdrive_get_gaussian_random_variable(); tmp++;} // cycle past initial seed
    ...
    set_turb_ampl(e, &e->s->cells_top[0]); // set initial amplitudes and calculate initial quantities needed for dissipation measures
    StTPrev = e->ti_current; // mark current time as last update of turb driving fields
}
...
{
#if defined(HYDRO_DIMENSION_3D)
    return sqrt(3.0/3.0)*sqrt(3.0)*1.0/sqrt(1.0-2.0*TurbDriving_Global_SolenoidalFraction+3.0*TurbDriving_Global_SolenoidalFraction*TurbDriving_Global_SolenoidalFraction);
#elif defined(HYDRO_DIMENSION_2D)
    return sqrt(3.0/2.0)*sqrt(3.0)*1.0/sqrt(1.0-2.0*TurbDriving_Global_SolenoidalFraction+2.0*TurbDriving_Global_SolenoidalFraction*TurbDriving_Global_SolenoidalFraction);
#elif defined(HYDRO_DIMENSION_1D)
    return sqrt(3.0/1.0)*sqrt(3.0)*1.0/sqrt(1.0-2.0*TurbDriving_Global_SolenoidalFraction+1.0*TurbDriving_Global_SolenoidalFraction*TurbDriving_Global_SolenoidalFraction);
#endif
}
	  ...
	  double fx = 0, fy = 0, fz = 0;
	    ...
	    const float mass = hydro_get_mass(p);
            ...
            if(mass > 0.)
            {
                ...
                for(int j=0; j<3; j++) {p->turbulence.a[j] = acc[j];}
            } else {
...
void do_turb_driving_step_first_half(struct cell *c, const struct engine *e)
{
  ...
  for(int i = 0; i < count; ++i)
	...
	double ekin0 = 0.5 * mass * (p->v[0] * p->v[0] + p->v[1] * p->v[1] + p->v[2] * p->v[2]);
	double vtmp[3];
	double dvel[3];
	for(int j=0;j<3;j++) {
	...
	double ekin1 = 0.5 * mass * (vtmp[0]*vtmp[0]+vtmp[1]*vtmp[1]+vtmp[2]*vtmp[2]);
...
void log_turb_temp(struct engine *e)
{
  ...
  double dudt_drive = 0, dudt_diss = 0, mass = 0, ekin = 0, ethermal = 0;
    ...
    for(int i = 0; i < nr_parts; i++)
    {
      ...
      ekin += 0.5 * m * (p->v[0] * p->v[0] + p->v[1] * p->v[1] + p->v[2] * p->v[2]);
      ethermal += m * p->u;
      mass += m;
    }
    double mach = sqrt(2.*ekin / (hydro_gamma*(hydro_gamma-1)*ethermal));
    
    fprintf(FdTurb, "%g %g %g %g %g %g %g\n", e->time, mach, (ekin + ethermal) / mass, dudt_drive / mass,
	    dudt_diss / mass, TurbInjectedEnergy / mass, TurbDissipatedEnergy / mass);
    fflush(FdTurb);
}
