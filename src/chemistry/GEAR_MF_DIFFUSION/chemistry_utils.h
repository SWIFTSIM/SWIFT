...
// Compute the eigenvector corresponding to the eigenvalue eigenvalue0 of a 3x3
// symmetric matrix A
__attribute__((always_inline)) INLINE  void
...
chemistry_utils_diagonalize_3x3(const double A[3][3], double eigenvalues[3],
  ...
  if (norm > 0.0) {
    ...
    // Extracting matrix A elements for convenience
    ...
    // Compute the eigenvectors so that the set {eigenvector0,
    // eveceigenvector1, eigenvector2} is right handed and orthonormal.
    if (halfDet >= 0.0) {
