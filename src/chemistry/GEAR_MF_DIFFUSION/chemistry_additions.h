 ...
 *
 * This function is called by kick1 and kick2 tasks, effectively performing two
 * kicks.
 *
 * @param p Particle to act upon.
 * @param dt_therm Thermal energy time-step @f$\frac{dt}{a^2}@f$.
...
__attribute__((always_inline)) INLINE static void chemistry_kick_extra(
  ...
  for (int m = 0; m < GEAR_CHEMISTRY_ELEMENT_COUNT; ++m) {
    ...
    chemistry_part_integrate_flux_source_term(p, m, dt_therm_phys, flux_source,
                                              chem_data, cosmo,
                                              chd->diffusion_flux[m]);
  ...
  if (chd->flux.dt > 0.0) {
    ...
    /* something tricky happens at the beginning of the simulation: the flux
       ...
       the actual time step in chemistry_prepare_force(). We have to make sure
       it is properly set to -1 here, so that inactive particles are indeed
       found to be inactive during the flux loop. */
    chd->flux.dt = -1.0f;
  }
