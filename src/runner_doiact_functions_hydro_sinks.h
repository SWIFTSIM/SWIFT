...
void DOPAIR1_NAIVE(struct runner *r, const struct cell *restrict ci,
  ...
  const float r_cut = sink_props->cut_off_radius;
  ...
  /* TODO: Check that r_cut > cell dmin ? */  
  ...
  for (int pid = 0; pid < count_i; pid++) {
    ...
    /* Skip particles not eligible for sink formation */
    /* TODO: Add this function. TODO: Find a better name... */
    if (part_is_eligible_for_sink_formation(pj, e)) continue;
    ...
    for (int pjd = 0; pjd < count_j; pjd++) {
      ...
      const int doi = pi_active && (r2 < r_cut2);
      const int doj = pj_active && (r2 < r_cut2);

      /* Hit or miss? */
      if (doi) {
	IACT_NONSYM_HYDRO_SINKS(r2, dx, r_cut, pi, pj, a, H);
