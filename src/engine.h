...
struct engine {
  ...
  /*! This rank's own share of radiation_hii_merge_collision_count, updated
   * by runner_main.c's atomic_add on every part_hii_tag unpack. Reduced
   * (summed) across ranks into radiation_hii_merge_collision_count each
   * step; never read directly for reporting. */
  long long radiation_hii_merge_collision_count_local;

  /*! Cumulative ionizing photon count forfeited by losing claims in the
   * S3.4 merge, summed across all ranks (same reduction as
   * radiation_hii_merge_collision_count above). S3.4's accept-and-merge
   * design: the losing star's debit already happened on its own rank and
  ...
  /*! This rank's own share of radiation_hii_forfeited_budget; see
   * radiation_hii_merge_collision_count_local. */
  double radiation_hii_forfeited_budget_local;
#endif
