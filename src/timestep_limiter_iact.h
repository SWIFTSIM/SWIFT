 ...
 * @brief Timestep limiter loop
 */
__attribute__((always_inline)) INLINE static void runner_iact_dm_limiter(
    const float r2, const float *dx, const float hi, const float hj,
    struct dmpart *restrict pi, struct dmpart *restrict pj, const float a,
    const float H) {
 ...
 * @brief Timestep limiter loop (non-symmetric version)
 */
__attribute__((always_inline)) INLINE static void runner_iact_nonsym_dm_limiter(
    const float r2, const float *dx, const float hi, const float hj,
    const struct dmpart *restrict pi, struct dmpart *restrict pj, const float a,
    const float H) {
 ...
 * @param H Current Hubble parameter.
 */
__attribute__((always_inline)) INLINE static void runner_iact_dm_timebin(
    const float r2, const float *dx, const float hi, const float hj,
    struct dmpart *restrict pi, struct dmpart *restrict pj, const float a,
    const float H) {
    ...
    /* Update the minimal time-bin */
    if (pj->time_bin > 0)
       pi->limiter_data.min_ngb_time_bin = min(pi->limiter_data.min_ngb_time_bin, pj->time_bin);
    
    if (pi->time_bin > 0)
       pj->limiter_data.min_ngb_time_bin = min(pj->limiter_data.min_ngb_time_bin, pi->time_bin);
}
 ...
 * @param H Current Hubble parameter.
 */
__attribute__((always_inline)) INLINE static void runner_iact_nonsym_dm_timebin(
    const float r2, const float *dx, const float hi, const float hj,
    struct dmpart *restrict pi, const struct dmpart *restrict pj, const float a,
    const float H) {
