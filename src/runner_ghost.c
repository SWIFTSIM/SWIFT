...
/* Import the stars df_from_dm loop functions. */
...
/**
 * @brief Intermediate task after DM density calculation for stars to check smoothing lengths are correct and finish the density calculation
 * 
 * @param r The runner thread.
 * @param c The cell.
 * @param timer Are we timing this ?
...
void runner_do_stars_df_from_dm_ghost(struct runner *r, struct cell *c, int timer) {
  ...
  const struct unit_system *us = e->internal_units;
  const struct phys_const *phys_const = e->physical_constants;
  const int with_df = (e->policy & engine_policy_dynamical_friction);
  const int with_cosmology = (e->policy & engine_policy_cosmology);
  const struct cosmology *cosmo = e->cosmology;
  const float df_h_max = e->stars_properties->df_from_dm.h_max;
  const float df_h_min = e->stars_properties->df_from_dm.h_min;
  const float eps = e->stars_properties->df_from_dm.h_tolerance;
  const float df_eta_dim =
      pow_dimension(e->stars_properties->df_from_dm.eta_neighbours);
  const int max_smoothing_iter = e->stars_properties->df_from_dm.max_smoothing_iterations;
  int redo = 0, scount = 0;
  ...
  if (c->split) {
    for (int k = 0; k < 8; k++) {
      if (c->progeny[k] != NULL) {
        runner_do_stars_df_from_dm_ghost(r, c->progeny[k], 0);
      }
    }
  } else {
    ...
    if ((right = (float *)malloc(sizeof(float) * c->stars.count)) == NULL)
      error("Can't allocate memory for right.");
    for (int k = 0; k < c->stars.count; k++)
      if ((spart_is_active(&sparts[k], e)) && (&sparts[k].df_data.apply_df)) {
        sid[scount] = k;
        h_0[scount] = sparts[k].df_data.h_dm;
        left[scount] = 0.f;
        right[scount] = df_h_max;
        ++scount;
      }
    ...
    for (int num_reruns = 0; scount > 0 && num_reruns < max_smoothing_iter;
      ...
      for (int i = 0; i < scount; i++) {
        ...
        /* Get some useful values */
        const float h_init = h_0[i];
        const float h_old = sp->df_data.h_dm;
        const float h_old_dim = pow_dimension(h_old);
        const float h_old_dim_minus_one = pow_dimension_minus_one(h_old);
        ...
        if (sp->df_data.density_dm.wcount < 1.e-5 * kernel_root) { /* No neighbours case */
          ...
          const float n_target = df_eta_dim;
          const float f = n_sum - n_target;
          const float f_prime =
              sp->df_data.density_dm.wcount_dh * h_old_dim +
              hydro_dimension * sp->df_data.density_dm.wcount * h_old_dim_minus_one;
           ...
           */
          /* Same if we are below h_min */
          if (((sp->df_data.h_dm >= df_h_max) && (f < 0.f)) ||
              ((sp->df_data.h_dm <= df_h_min) && (f > 0.f))) {
        ...
        if (fabsf(h_new - h_old) > eps * h_old) {
          ...
          if ((h_new == left[i] && h_old == right[i]) ||
            ...
            /* Bisect the remaining interval */
            sp->df_data.h_dm = pow_inv_dimension(
                0.5f * (pow_dimension(left[i]) + pow_dimension(right[i])));

          } else {

            /* Normal case */
            sp->df_data.h_dm = h_new;
          }

          /* If below the absolute maximum, try again */
          if (sp->df_data.h_dm < df_h_max && sp->df_data.h_dm > df_h_min) {
          ...
          } else if (sp->df_data.h_dm <= df_h_min) {

            /* Ok, this particle is a lost cause... */
            sp->df_data.h_dm = df_h_min;

          } else if (sp->df_data.h_dm >= df_h_max) {

            /* Ok, this particle is a lost cause... */
            sp->df_data.h_dm = df_h_max;
      ...
      if (scount > 0) {
        ...
        for (struct cell *finger = c; finger != NULL; finger = finger->parent) {
          ...
          /* Run through this cell's density interactions. */
          for (struct link *l = finger->stars.df_from_dm; l != NULL; l = l->next) {
            ...
            /* Self interaction? */
            if (l->t->type == task_type_self) {
              runner_dosub_self_subset_stars_df_from_dm(r, finger, sparts, sid,
                                                     scount, 1);
            }
            ...
            /* Otherwise, pair interaction? */
              ...
              /* Left or right? */
              if (l->t->ci == finger)
                runner_dosub_pair_subset_stars_df_from_dm(r, finger, sparts, sid,
                                                       scount, l->t->cj, 1);
              else
                runner_dosub_pair_subset_stars_df_from_dm(r, finger, sparts, sid,
                                                       scount, l->t->ci, 1);
    ...
    if (scount) {
      ...
      for (int i = 0; i < scount; i++) {
        struct spart *sp = &sparts[sid[i]];
        warning("ID: %lld, h: %g, wcount: %g", sp->id, sp->df_data.h_dm,
                sp->df_data.density_dm.wcount);
      }
...
/**
 * @brief Intermediate task after stars density calculation for stars to check smoothing lengths are correct and finish the density calculation
 * 
 * Will have to copy lots of the above code into here.
 *
 * @param r The runner thread.
 * @param c The cell.
 * @param timer Are we timing this ?
...
void runner_do_stars_df_from_stars_ghost(struct runner *r, struct cell *c, int timer) {
  ...
  const int with_cosmology = (e->policy & engine_policy_cosmology);
  const struct cosmology *cosmo = e->cosmology;
  const float df_h_max = e->stars_properties->df_from_stars.h_max;
  const float df_h_min = e->stars_properties->df_from_stars.h_min;
  const float eps = e->stars_properties->df_from_stars.h_tolerance;
  const float df_eta_dim =
      pow_dimension(e->stars_properties->df_from_stars.eta_neighbours);
  const int max_smoothing_iter = e->stars_properties->df_from_stars.max_smoothing_iterations;
  int redo = 0, scount = 0;
  ...
  if (c->split) {
    for (int k = 0; k < 8; k++) {
      if (c->progeny[k] != NULL) {
        runner_do_stars_df_from_stars_ghost(r, c->progeny[k], 0);
      }
    }
  } else {
    ...
    for (int k = 0; k < c->stars.count; k++)
      if ((spart_is_active(&sparts[k], e)) && (&sparts[k].df_data.apply_df)) {
        sid[scount] = k;
        h_0[scount] = sparts[k].df_data.h_stars;
        left[scount] = 0.f;
        right[scount] = df_h_max;
        ++scount;
      }
    ...
    for (int num_reruns = 0; scount > 0 && num_reruns < max_smoothing_iter;
      ...
      for (int i = 0; i < scount; i++) {
        ...
        /* Get some useful values */
        const float h_init = h_0[i];
        const float h_old = sp->df_data.h_stars;
        const float h_old_dim = pow_dimension(h_old);
        const float h_old_dim_minus_one = pow_dimension_minus_one(h_old);
        ...
        if (sp->df_data.density_stars.wcount < 1.e-5 * kernel_root) { /* No neighbours case */
          ...
          const float f = n_sum - n_target;
          const float f_prime =
              sp->df_data.density_stars.wcount_dh * h_old_dim +
              hydro_dimension * sp->df_data.density_stars.wcount * h_old_dim_minus_one;
           ...
           */
          /* Same if we are below h_min */
          if (((sp->df_data.h_stars >= df_h_max) && (f < 0.f)) ||
              ((sp->df_data.h_stars <= df_h_min) && (f > 0.f))) {
        ...
        if (fabsf(h_new - h_old) > eps * h_old) {
          ...
          if ((h_new == left[i] && h_old == right[i]) ||
            ...
            /* Bisect the remaining interval */
            sp->df_data.h_stars = pow_inv_dimension(
                0.5f * (pow_dimension(left[i]) + pow_dimension(right[i])));

          } else {

            /* Normal case */
            sp->df_data.h_stars = h_new;
          }

          /* If below the absolute maximum, try again */
          if (sp->df_data.h_stars < df_h_max && sp->df_data.h_stars > df_h_min) {
          ...
          } else if (sp->df_data.h_stars <= df_h_min) {

            /* Ok, this particle is a lost cause... */
            sp->df_data.h_stars = df_h_min;

          } else if (sp->df_data.h_stars >= df_h_max) {

            /* Ok, this particle is a lost cause... */
            sp->df_data.h_stars = df_h_max;
        ...
        /* We now have a particle whose smoothing length has converged */
        message("Successfully converged to h=%e, n=%e, n_target=%e", sp->df_data.h_stars, sp->df_data.density_stars.wcount, df_eta_dim/pow_dimension(sp->df_data.h_stars));
      }
      ...
      if (scount > 0) {
        ...
        for (struct cell *finger = c; finger != NULL; finger = finger->parent) {
          ...
          /* Run through this cell's density interactions. */
          for (struct link *l = finger->stars.df_from_stars; l != NULL; l = l->next) {
            ...
            /* Self interaction? */
            if (l->t->type == task_type_self) {
              runner_dosub_self_subset_stars_df_from_stars(r, finger, sparts, sid,
                                                     scount, 1);
            }
            ...
            /* Otherwise, pair interaction? */
              ...
              /* Left or right? */
              if (l->t->ci == finger)
                runner_dosub_pair_subset_stars_df_from_stars(r, finger, sparts, sid,
                                                       scount, l->t->cj, 1);
              else
                runner_dosub_pair_subset_stars_df_from_stars(r, finger, sparts, sid,
                                                       scount, l->t->ci, 1);
    ...
    if (scount) {
      ...
      for (int i = 0; i < scount; i++) {
        struct spart *sp = &sparts[sid[i]];
        warning("ID: %lld, h: %g, wcount: %g", sp->id, sp->df_data.h_stars,
                sp->df_data.density_stars.wcount);
      }
