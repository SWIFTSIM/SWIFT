 ...
 * Copyright (c) 2025 Jonathan Davies (j.j.davies@ljmu.ac.uk)
 *
...
void sidm_exact_density_compute_mapper(void *map_data, int nr_siparts,
  ...
  for (int i = 0; i < nr_siparts; ++i) {

    struct sipart *sipi = &siparts[i];
    const long long id = sipi->id;
    ...
    if (id % SWIFT_SIDM_DENSITY_CHECKS == 0 && sipart_is_starting(sipi, e)) {
      ...
      const double pix[3] = {sipi->x[0], sipi->x[1], sipi->x[2]};
      const double hi = sipi->h;
      ...
      /* Interact it with all other sidm particles in the space.*/
      for (int j = 0; j < (int)s->nr_siparts; ++j) {

        const struct sipart *sipj = &s->siparts[j];

        /* Compute the pairwise distance. */
        double dx = sipj->x[0] - pix[0];
        double dy = sipj->x[1] - pix[1];
        double dz = sipj->x[2] - pix[2];
        ...
        if (r2 < hig2) {
          ...
          if (sipart_is_inhibited(sipj, e)) {
            sipi->inhibited_exact = 1;
          } else {
      ...
      sipi->N_density_exact = N_density_exact;
      sipi->rho_exact = rho_exact * pow_dimension(hi_inv);
      sipi->n_exact = n_exact * pow_dimension(hi_inv);
...
#else
  error("SIDM checking function called without the corresponding flag.");
#endif
}
...
void sidm_exact_density_compute(struct space *s, const struct engine *e) {
  ...
  threadpool_map(&s->e->threadpool, sidm_exact_density_compute_mapper,
                 s->siparts, s->nr_siparts, sizeof(struct sipart), 0, &data);
...
void sidm_exact_density_check(struct space *s, const struct engine *e,
  ...
  const struct sipart *siparts = s->siparts;
  const size_t nr_siparts = s->nr_siparts;

  const double eta = e->sidm_properties->eta_neighbours;
  const double N_ngb_target =
      (4. / 3.) * M_PI * pow_dimension(kernel_gamma * eta);
  const double N_ngb_max =
      N_ngb_target + 2. * e->sidm_properties->delta_neighbours;
  const double N_ngb_min =
      N_ngb_target - 2. * e->sidm_properties->delta_neighbours;

  /* File name */
  char file_name_swift[100];
  sprintf(file_name_swift, "sidm_checks_swift_step%.4d.dat", e->step);
  ...
  for (size_t i = 0; i < nr_siparts; ++i) {

    const struct sipart *sipi = &siparts[i];
    const long long id = sipi->id;

    const double N_ngb = (4. / 3.) * M_PI * kernel_gamma * kernel_gamma *
                         kernel_gamma * sipi->h * sipi->h * sipi->h * sipi->n;
    ...
    if (id % SWIFT_SIDM_DENSITY_CHECKS == 0 && sipart_is_starting(sipi, e)) {
      ...
      fprintf(
          ...
          id, sipi->x[0], sipi->x[1], sipi->x[2], sipi->h, sipi->N_density, 0,
          sipi->rho, sipi->n, N_ngb);
    }
  }
  ...
  sprintf(file_name_exact, "sidm_checks_exact_step%.4d.dat", e->step);

  /* Creare files and write header */
  FILE *file_exact = fopen(file_name_exact, "w");
  if (file_exact == NULL) error("Could not create file '%s'.", file_name_exact);
  fprintf(file_exact, "# SIDM accuracy test - EXACT DENSITIES\n");
  ...
  for (size_t i = 0; i < nr_siparts; ++i) {
    ...
    if (id % SWIFT_SIDM_DENSITY_CHECKS == 0 && sipart_is_starting(sipi, e)) {
       ...
       * that step or not. */
      if (!found_inhibited && sipi->N_density_exact != sipi->N_density &&
      ...
      if (!found_inhibited && (N_ngb > N_ngb_max || N_ngb < N_ngb_min)) {
        ...
        message("N_NGB: id=%lld exact=%f N_true=%d N_swift=%d", id, N_ngb,
                sipi->N_density_exact, sipi->N_density);
