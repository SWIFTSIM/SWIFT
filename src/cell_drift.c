  ...
  if (c->split && (force || cell_get_flag(c, cell_flag_do_sink_sub_drift))) {
    ...
    for (int k = 0; k < 8; k++) {
      if (c->progeny[k] != NULL) {
        ...
        cell_r_max = max(cell_r_max, cp->sinks.h_max);
        cell_r_max_active = max(cell_r_max_active, cp->sinks.h_max_active);
      }
    }

    /* Store the values */
    c->sinks.h_max = cell_r_max;
    c->sinks.h_max_active = cell_r_max_active;
    ...
    for (size_t k = 0; k < nr_sinks; k++) {
      ...
      /* Maximal smoothing length */
      cell_r_max = max(cell_r_max, sink->h);
      ...
      if (sink_is_active(sink, e)) {
        ...
        cell_r_max_active = max(cell_r_max_active, sink->h);
      }
    }
