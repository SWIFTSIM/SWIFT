...
/* /\** */
/*  * @brief Sort entry for radial distance. */
/*  *\/ */
/* struct radial_sort_entry { */
/*   float r2; /\* Squared distance to star *\/ */
/*   int index; /\* Index of the part in the cell *\/ */
/* }; */

/* /\** */
/*  * @brief Local QuickSort for radial distances. */
/*  *\/ */
/* void runner_do_radial_sort(struct radial_sort_entry *sort, int N) { */
/*   /\* Use your provided sort stack logic here *\/ */
/*   struct { short int lo, hi; } qstack[256];  */
/*   int qpos = 0, i, j, lo, hi, imin; */
/*   struct radial_sort_entry temp; */
/*   float pivot; */

/*   qstack[0].lo = 0; */
/*   qstack[0].hi = N - 1; */
/*   while (qpos >= 0) { */
/*     lo = qstack[qpos].lo; hi = qstack[qpos].hi; qpos--; */
/*     if (hi - lo < 15) { */
/*       for (i = lo; i < hi; i++) { */
/*         imin = i; */
/*         for (j = i + 1; j <= hi; j++)  */
/*           if (sort[j].r2 < sort[imin].r2) imin = j; */
/*         if (imin != i) { temp = sort[imin]; sort[imin] = sort[i]; sort[i] =
 * temp; } */
/*       } */
/*     } else { */
/*       pivot = sort[(lo + hi) / 2].r2; */
/*       i = lo; j = hi; */
/*       while (i <= j) { */
/*         while (sort[i].r2 < pivot) i++; */
/*         while (sort[j].r2 > pivot) j--; */
/*         if (i <= j) { */
/*           if (i < j) { temp = sort[i]; sort[i] = sort[j]; sort[j] = temp; }
 */
/*           i++; j--; */
/*         } */
/*       } */
/*       if (i < hi) { qpos++; qstack[qpos].lo = i; qstack[qpos].hi = hi; } */
/*       if (lo < j) { qpos++; qstack[qpos].lo = lo; qstack[qpos].hi = j; } */
/*     } */
/*   } */
/* } */

/* /\** */
/*  * @brief Check if a cell overlaps with a sphere, respecting PBC. */
/*  * */
/*  * @param c The cell. */
/*  * @param pos The center of the sphere (star position). */
/*  * @param r The radius of the sphere. */
/*  * @param dim The box dimensions (for PBC). */
/*  *\/ */
/* int cell_overlaps_sphere(const struct cell *c, const double pos[3], float r,
 * const double dim[3]) { */
...
/*   for (int k = 0; k < 3; k++) { */
/*     /\* Distance from sphere center to the closest point on the cell box *\/
 */
/*     double p_min = c->loc[k]; */
/*     double p_max = c->loc[k] + c->width[k]; */

/*     /\* Handle Periodic Boundary Conditions *\/ */
/*     double d_pos = pos[k]; */
/*     // This is the standard SWIFT nearest-image projection */
/*     if (d_pos < p_min - 0.5 * dim[k]) d_pos += dim[k]; */
/*     if (d_pos > p_max + 0.5 * dim[k]) d_pos -= dim[k]; */

/*     if (d_pos < p_min) { */
/*       double d = p_min - d_pos; */
/*       dist2 += d * d; */
/*     } else if (d_pos > p_max) { */
/*       double d = d_pos - p_max; */
/*       dist2 += d * d; */
/*     } */
/*   } */
...
/**
 * @brief Recursive HII ionization algorithm.
 *
 * @param r The runner.
 * @param c The current cell.
 * @param s The star particle (source).
 * @param r_search_2 Squared search radius (the 'educated guess').
 */
/* void runner_do_hii_recursion(struct runner *r, struct cell *c, struct spart
 * *s,  */
/*                              float r_search_2) { */
...
/*   /\* 1. If this cell is split, decide whether to recurse *\/ */
/*   if (c->split) { */
/*     for (int k = 0; k < 8; k++) { */
/*       if (c->progeny[k] != NULL) { */
/*         /\* Only enter child cells that overlap with our search radius *\/ */
/*         if (cell_overlaps_sphere(c->progeny[k], s->x, sqrtf(r_search_2))) {
 */
/*           runner_do_hii_recursion(r, c->progeny[k], s, r_search_2); */
/*         } */
/*       } */
/*     } */
/*   }  */
...
/*     /\* Stopping condition: If this cell's maximum active HII radius  */
/*      * is smaller than a certain threshold or if we've reached a  */
/*      * level where it's cheaper to just loop over everything. *\/ */
/*     if (c->stars.r_hii_max_active < sprops->min_hii_recursion_width) { */
/*         // Process this level directly or handle differently */
/*     } */

/*     for (int k = 0; k < 8; k++) { */
/*         if (c->progeny[k] != NULL) { */
/*             /\* Only recurse if the progeny could contain an active star  */
/*              * OR be reached by a star from a sibling progeny. *\/ */
/*             runner_do_hii_recursion(r, c->progeny[k], ...); */
/*         } */
/*     } */
/* }     */

/*   /\* 2. Leaf level: Process the gas *\/ */
/*   else { */
/*     if (count == 0) return; */

/*     struct radial_sort_entry sort_buffer[256]; */
/*     int neighbors_found = 0; */

/*     /\* A. Gather gas particles within the search radius *\/ */
/*     for (int j = 0; j < count && neighbors_found < 256; j++) { */
/*       struct part *restrict p = &parts[j]; */

/*       /\* Skip already ionized or inhibited *\/ */
/*       if (part_is_inhibited(p, e) || part_is_ionized(p)) continue; */

/*       float dx[3] = {(float)(s->x[0] - p->x[0]),  */
/*                      (float)(s->x[1] - p->x[1]),  */
/*                      (float)(s->x[2] - p->x[2])}; */
/*       float r2 = dx[0]*dx[0] + dx[1]*dx[1] + dx[2]*dx[2]; */

/*       if (r2 < r_search_2) { */
/*         sort_buffer[neighbors_found].r2 = r2; */
/*         sort_buffer[neighbors_found].index = j; */
/*         neighbors_found++; */
/*       } */
/*     } */

/*     /\* B. Sort neighbors by radius (FIRE requirement) *\/ */
/*     if (neighbors_found > 1) { */
/*       runner_do_radial_sort(sort_buffer, neighbors_found); */
/*     } */

/*     /\* C. Ionize in order *\/ */
/*     for (int k = 0; k < neighbors_found; k++) { */
/*       if (s->m_ion_budget <= 0) break; */

/*       struct part *p = &parts[sort_buffer[k].index]; */
/*       double m_p = hydro_get_mass(p); */

/*       if (s->m_ion_budget >= m_p) { */
/*         do_ionize_part(p, e); */
/*         s->m_ion_budget -= m_p; */
/*         /\* Update the star's actual realized r_hii *\/ */
/*         s->r_hii = sqrtf(sort_buffer[k].r2); */
/*       } else { */
/*         /\* Probabilistic tail of the Strömgren sphere *\/ */
/*         if (random_unit_interval(s->id) < (s->m_ion_budget / m_p)) { */
/*           do_ionize_part(p, e); */
/*           s->r_hii = sqrtf(sort_buffer[k].r2); */
/*         } */
/*         s->m_ion_budget = 0; */
