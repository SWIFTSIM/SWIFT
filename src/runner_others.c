...
static void runner_debug_get_top_level_gparts_by_source_representation(
    const struct engine *e, long long counts[4], int nr_cells[4]) {
  ...
  for (int i = 0; i < s->nr_cells; i++) {
    ...
    enum runner_debug_source_class source_class = runner_debug_source_class_other;

    if (s->with_zoom_region) {
      source_class = runner_debug_get_source_class(c);
    }

    counts[source_class] += c->grav.multipole->m_pole.num_gpart;
      ...
      if (gpart_is_active(gp, e)) {
        ...
        if ((e->policy & engine_policy_self_gravity) &&
          ...
          if (gp->num_interacted !=
            ...
            const long long num_interacted_pm_long_range =
