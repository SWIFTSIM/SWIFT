        ...
        if ((e->policy & engine_policy_self_gravity) &&
          ...
          if (gp->num_interacted !=
            ...
            error(
                ...
                "num_gparts=%zd inhibited_gparts=%lld cell_type=%d cell_loc=[%f %f %f] rmax=%f grav_count=%d)",
                my_id, part_type_names[gp->type], gp->x[0], gp->x[1], gp->x[2], gp->num_interacted,
                e->total_nr_gparts, e->s->nr_gparts, e->count_inhibited_gparts, tl_cell_types[c->tl_cell_type],
                c->loc[0], c->loc[1], c->loc[2], c->grav.multipole->r_max, c->grav.count);
          }
        }
#endif
