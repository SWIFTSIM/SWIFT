...
static INLINE void runner_dopair_grav_mm_nonsym(struct runner *r,
  ...
  if (multi_j->num_gpart == 0)
    error("Multipole does not seem to have been set. "
          ...
          ci->type, cj->type, ci->depth, cj->depth);
  ...
  if (cj->grav.ti_old_multipole != e->ti_current)
    error(
        "Undrifted multipole cj->grav.ti_old_multipole=%lld cj->nodeID=%d "
        "ci->nodeID=%d e->ti_current=%lld ci->type=%d, "
        "cj->type=%d, ci->depth=%d, cj->depth=%d",
        cj->grav.ti_old_multipole, cj->nodeID, ci->nodeID, e->ti_current,
        ci->type, cj->type, ci->depth, cj->depth);
#endif
...
void runner_dopair_recursive_grav(struct runner *r, struct cell *ci,
  ...
  if (gcount_i == 0 || gcount_j == 0)
    error("Doing pair gravity on an empty cell! (ci->type=%d, "
          "ci->grav.count=%d, ci->depth=%d, cj->type=%d, "
          "cj->grav.count=%d, cj->depth=%d)",
          ci->type, ci->grav.count, ci->depth,
          cj->type, cj->grav.count, cj->depth);
...
void runner_do_grav_long_range(struct runner *r, struct cell *ci,
  ...
  if (!periodic) {
    ...
    int d = ceil(hr_max_distance * max3(s->zoom_props->buffer_iwidth[0],
    ...
    /* Maximal distance any interaction can take place
     * before the mesh kicks in, rounded up to the next integer */
    d = ceil(global_max_distance * max3(s->iwidth[0],
                                        s->iwidth[1],
                                        s->iwidth[2])) + 1;
    ...
    /* Maximal distance any interaction can take place
     * before the mesh kicks in, rounded up to the next integer */
    int d = ceil(global_max_distance * max3(s->iwidth[0],
                                            s->iwidth[1],
                                            s->iwidth[2])) + 1;
    ...
    /* We also need to loop over useful buffer cells. Although top is
     * garuanteed to be outside the buffer cells we can play the same games
     * walking out only a certain number of buffer cells to interact with
     * those we need to. */
