...
int cell_getid_zoom(const int cdim[3], const double x, const double y,
  ...
  if (s->with_zoom_region) {
    ...
    if (x >= zoom_region_bounds[0] && x < zoom_region_bounds[1] &&
      ...
      const int zoom_i = (x - (float)zoom_region_bounds[0]) * ih_x_zoom;
      const int zoom_j = (y - (float)zoom_region_bounds[2]) * ih_y_zoom;
      const int zoom_k = (z - (float)zoom_region_bounds[4]) * ih_z_zoom;
