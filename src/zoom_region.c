...
//  }
  ...
  if (verbose) {
    ...
    message("nr_tl_cells_in_zoom_region: [%d %d %d] nr_zoom_cells_in_tl_cell: [%d %d %d]",
            ...
            (int)floor((s->width[0] + 0.5 * s->zoom_props->iwidth[0]) * s->zoom_props->iwidth[0]),
            (int)floor((s->width[1] + 0.5 * s->zoom_props->iwidth[1]) * s->zoom_props->iwidth[1]),
            (int)floor((s->width[2] + 0.5 * s->zoom_props->iwidth[2]) * s->zoom_props->iwidth[2]));
  }
