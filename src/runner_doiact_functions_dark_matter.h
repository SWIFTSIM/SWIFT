...
void DOPAIR1_DM_NAIVE(struct runner *r, struct cell *restrict ci,
  ...
  for (int pid = 0; pid < count_i; pid++) {
    ...
    struct dmpart *restrict pi = &parts_i[pid];
    ...
    for (int pjd = 0; pjd < count_j; pjd++) {
      ...
      if (r2 < hig2 && pi_active) {
        ...
        runner_iact_nonsym_sidm(r2, dx, hi, hj, pi, pj, a, H);
#endif
      }
      if (r2 < hjg2 && pj_active) {
        ...
        IACT_DM_NONSYM(r2, dx, hj, hi, pj, pi, a, H);
#if (FUNCTION_TASK_LOOP == TASK_LOOP_FORCE)
        runner_iact_nonsym_sidm(r2, dx, hj, hi, pj, pi, a, H);
#endif
      }
    } /* loop over the parts in cj. */
  }   /* loop over the parts in ci. */

  TIMER_TOC(TIMER_DOPAIR_DM);
}
...
void DOPAIR2_DM_NAIVE(struct runner *r, struct cell *restrict ci,
  ...
  for (int pid = 0; pid < count_i; pid++) {
    ...
    for (int pjd = 0; pjd < count_j; pjd++) {
      ...
      /* Get a pointer to the jth particle. */
      struct dmpart *restrict pj = &parts_j[pjd];
      ...
      if (r2 < hig2 || r2 < hjg2) {
        ...
        if (pi_active && pj_active) {
          ...
          IACT_DM_NONSYM(r2, dx, hi, hj, pi, pj, a, H);
#if (FUNCTION_TASK_LOOP == TASK_LOOP_FORCE)
          runner_iact_nonsym_sidm(r2, dx, hi, hj, pi, pj, a, H);
#endif
        } else if (pj_active) {
...
void DOSELF1_DM_NAIVE(struct runner *r, struct cell *restrict c) {
  ...
  struct dmpart *restrict parts = c->dark_matter.parts;

  /* Loop over the parts in ci. */
  for (int pid = 0; pid < count; pid++) {

    /* Get a hold of the ith part in ci. */
    struct dmpart *restrict pi = &parts[pid];
    ...
    for (int pjd = pid + 1; pjd < count; pjd++) {
      ...
      /* Get a pointer to the jth particle. */
      struct dmpart *restrict pj = &parts[pjd];
 ...
 */
void DOPAIR_DM_SUBSET_NAIVE(struct runner *r, struct cell *restrict ci,
                         struct dmpart *restrict parts_i, int *restrict ind,
  ...
  const int count_j = cj->dark_matter.count;
  struct dnpart *restrict parts_j = cj->dark_matter.parts;
  ...
  for (int pid = 0; pid < count; pid++) {
    ...
    /* Get a hold of the ith part in ci. */
    struct dmpart *restrict pi = &parts_i[ind[pid]];
    double pix[3];
    ...
    for (int pjd = 0; pjd < count_j; pjd++) {
      ...
      if (r2 < hig2) {
        ...
        IACT_DM_NONSYM(r2, dx, hi, pj->h, pi, pj, a, H);
#if (FUNCTION_TASK_LOOP == TASK_LOOP_FORCE)
        runner_iact_nonsym_sidm(r2, dx, hi, pj->h, pi, pj, a, H);
#endif
      }
    } /* loop over the parts in cj. */
  }   /* loop over the parts in ci. */

  TIMER_TOC(timer_dopair_dm_subset_naive);
}
