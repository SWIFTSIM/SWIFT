...
void cell_activate_sink_formation_tasks(struct cell *c, struct scheduler *s) {
  ...
  /* Have we already unskipped that task? */
  if (c->hydro.sink_formation->skip == 0)
    return;
...
int cell_unskip_rt_tasks(struct cell *c, struct scheduler *s) {
  ...
  if (c->nodeID == nodeID) {
    if (cell_is_active_hydro(c, e)) {
      if (c->hydro.rt_in != NULL)
        scheduler_activate(s, c->hydro.rt_in);
      if (c->hydro.rt_ghost1 != NULL)
        scheduler_activate(s, c->hydro.rt_ghost1);
      if (c->hydro.rt_out != NULL)
        scheduler_activate(s, c->hydro.rt_out);
    }
  }
