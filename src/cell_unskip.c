...
int cell_unskip_hydro_tasks(struct cell *c, struct scheduler *s) {
  ...
  if (c->nodeID == nodeID && cell_is_active_hydro(c, e)) {
    ...
    for (struct link *l = c->hydro.density_pack; l != NULL; l = l->next) {
      scheduler_activate(s, l->t);
      if (l->t->ci != NULL) l->t->ci->pack_done = 0;
      if (l->t->ci != NULL) l->t->ci->gpu_done = 0;
      if (l->t->ci != NULL) l->t->ci->unpack_done = 0;
      if (l->t->cj != NULL) l->t->cj->pack_done = 0;
      if (l->t->cj != NULL) l->t->cj->gpu_done = 0;
      if (l->t->cj != NULL) l->t->cj->unpack_done = 0;
    }
    ...
    for (struct link *l = c->hydro.force_pack; l != NULL; l = l->next) {
      scheduler_activate(s, l->t);
      if (l->t->ci != NULL) l->t->ci->pack_done_f = 0;
      if (l->t->ci != NULL) l->t->ci->gpu_done_f = 0;
      if (l->t->ci != NULL) l->t->ci->unpack_done_f = 0;
      if (l->t->cj != NULL) l->t->cj->pack_done_f = 0;
      if (l->t->cj != NULL) l->t->cj->gpu_done_f = 0;
      if (l->t->cj != NULL) l->t->cj->unpack_done_f = 0;
    }
    ...
    for (struct link *l = c->hydro.gradient_pack; l != NULL; l = l->next) {
      scheduler_activate(s, l->t);
      if (l->t->ci != NULL) l->t->ci->pack_done_g = 0;
      if (l->t->ci != NULL) l->t->ci->gpu_done_g = 0;
      if (l->t->ci != NULL) l->t->ci->unpack_done_g = 0;
      if (l->t->cj != NULL) l->t->cj->pack_done_g = 0;
      if (l->t->cj != NULL) l->t->cj->gpu_done_g = 0;
      if (l->t->cj != NULL) l->t->cj->unpack_done_g = 0;
    }
