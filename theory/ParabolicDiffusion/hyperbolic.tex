...
This part is a first-principles derivation of the characteristic (wave)
speed of the hyperbolic (Cattaneo~\citeyear{Cattaneo1948}-relaxation)
diffusion system implemented under
\code{CHEMISTRY\_GEAR\_FVPM\_HYPERBOLIC\_DIFFUSION}
(\code{src/chemistry/GEAR\_FVPM\_DIFFUSION/hyperbolic/}), promised but not
derived where Section~\ref{sec:hyperbolic-relation} of
Part~\ref{part:parabolic} first mentions the system, and a direct
...
(\code{hyperbolic/chemistry\_getters.h:37--66}).

\subsection{Relation to the Parabolic Part}
...
The flux obeys (\code{chemistry\_flux.h:90--94}, docstring of
\code{chemistry\_part\_integrate\_flux\_source\_term}):
\begin{equation}
  \label{eq:cattaneo}
...
\code{chemistry\_compute\_flux}, \code{hyperbolic/chemistry\_flux.h:159--172}).
...
extrapolated to an interface. This is a structural limitation of the
scheme, not a simplifying choice available to relax: the FVPM
least-squares gradient estimator (Section~\ref{sec:gradient-estimator})
computes
only first derivatives of $\driver$ at each particle. Extrapolating
...
$\grad\driver$, i.e.\ a second derivative the scheme does not compute.
...
(\code{chemistry\_compute\_physical\_tau}, \code{hyperbolic/chemistry\_getters.h:75--92}):
\begin{align}
...
(Section~\ref{sec:diffusion-tensor-recap}): \code{diffusion\_mode} and
\code{relaxation\_time\_mode} are parsed as two independent strings
...
(\code{chemistry\_getters.h:196--285}):
\begin{align}
...
\code{isotropic\_constant}, $\driver=U_0/\rho$ otherwise). This is not a
...
snapshot field's own description (\code{chemistry\_io.h:185}) states it
...
(\code{chemistry\_riemann\_utils.h:210}) computes
...
\code{chemistry\_iact.h:295--298}), we treat $\Kmat$ (equivalently $\Deff$),
...
Directly from the code (\code{chemistry\_get\_hyperbolic\_flux},
\code{hyperbolic/chemistry\_getters.h:162--200}), the flux of $\Uvec$ in
...
\code{hyperbolic/chemistry\_getters.h:178--199}), not an independent
...
for all four flux components (\code{chemistry\_riemann\_HLL.h:112-179}),
...
\item $\taurelax\to0$: the first term drops and
  ...
  (\code{hyperbolic/chemistry\_flux.h:105--129}), which computes
...
\part{Relaxation Time and Timestep}
...
\subsection{Should $\taurelax$ Use the Regularised $\Splus$?}
...
\subsection{The Coupled Closed Form}
...
Romano et al.~\citeyear{Romano2022} note that a na\"ive Courant-like criterion
...
\code{startup\_fraction} parameter addresses the same pristine-gas
...
parabolic timestep, \code{chemistry\_timesteps.h:107--113}, using
$\lambda_1(\Kmat)=\kappaP$, exact for isotropic $\Kmat$; the first uses the identification
$\gammak h=\dx$: both are $O(1)$ multiples of the same local resolution
length, and the ratio is unaffected by resolution either way, only by
this $O(1)$ geometric factor between the kernel support radius and the
Gizmo particle size). The resolution $h$, diffusivity normalisation
...
The CFL length scale $\dx$ above is the Gizmo effective size,
$\dx=(V_i/V_{\rm unit\ sphere})^{1/d}$ (\code{chemistry\_timesteps.h}'s
\code{else} branch). An alternative is the Lanson \&
Vila~\citeyear{LansonVila2008} mean interparticle distance, their eq.~(58),
\begin{equation}
  \dx_i = \frac{1}{2\sum_l w_l\,\norm{A_{il}}_{\rm LV}} ,
  \label{eq:lanson-vila-psize}
\end{equation}
with $w_l$ the neighbour's volume and $A_{il}$ their one-sided
renormalized-gradient weight. Written in terms of the symmetrized
interface area $\norm{A_{ij}}$ that the flux exchange already builds,
each summand is $w_l\norm{A_{il}}_{\rm LV}=\norm{A_{il}}/V_l$, of
dimension length$^{-1}$; this is the form evaluated in
\code{chemistry\_timesteps.h}'s \code{GIZMO\_LANSON\_VILA\_PARTICLE\_SIZE}
branch, currently compiled out. Lanson \& Vila note directly below their
eq.~(58) that $\sum_l w_l\norm{A_{il}}_{\rm LV}=O(1/\epsilon_0)$, with
$\epsilon_0$ the smoothing length, so $\dx_i$ is of order the smoothing
length, the same resolution scale as the Gizmo size, differing from it
only by an $O(1)$ constant. Swapping definitions at a fixed $\CCFL$ is
therefore not by itself a route to larger timesteps; what the choice
changes is the constant, so the admissible $\CCFL$ has to be re-derived
for whichever length is used. For Eq.~\eqref{eq:lanson-vila-psize} that
derivation is Lanson \& Vila's own operator-based stability condition,
their eq.~(62), which bounds $\dt\,\sum_l w_l\norm{A_{il}}_{\rm LV}$
rather than assuming a Gizmo-calibrated Courant number carries over.
When evaluating the sum, note that dimensional analysis
alone does not pin down the summand: the kernel value $w_jh_j^{-d}$ also
combines with $\norm{A_{il}}$ to give a length$^{-1}$, but it is smaller
than $1/V_l$ by a factor of order the neighbour count $N_{\rm ngb}$, and
using it in place of $1/V_l$ inflates $\dx_i$ by that same factor.
\end{remark}

\subsection{Speed Criterion for \texttt{anisotropic\_gradient} Mode}
...
(\code{chemistry\_timesteps.h:111--117}), independent of the local
...
gradient-suppressed
$\expr=\norm{\driver}\dx/(\norm{\grad\driver}\dx+\norm{\driver})$
(\code{chemistry\_timesteps.h:118--124}). Redo the ratio for this mode,
...
($\lambda_1(\Shear)/\norm{\Shear}_F=1$); neither holds structurally in
this mode, so the two extra factors are new, not corrections to
something already accounted for.

Write $g\equiv\norm{\grad\driver}\dx/\norm{\driver}$ (the local gradient
steepness, in units of the resolution length: $g=0$ for a
perfectly-resolved/flat field, $g\to\infty$ for a steep, poorly-resolved
one) so that $\dx/\expr=1+g$, and
$r\equiv\norm{\Shear}_F/\lambda_1(\Shear)$. Corollary~\ref{cor:shear-regularisation-bound}'s
...
$r\in[\sqrt{3/2},\sqrt6\,]\approx[1.22,\,2.45]$, tight at the same
$\mathrm{diag}(2,-1,-1)$ (lower end) and $\mathrm{diag}(1,1,-2)$ (upper end) triples.
Setting Eq.~\eqref{eq:aniso_ratio} to $1$:
\begin{equation}
  \label{eq:aniso_threshold}
  \boxed{\;\taurelax_0 \gtrsim \frac{4r}{(1+g)^4}\;} ,
  \qquad r\in[\sqrt{3/2},\sqrt6\,] .
\end{equation}
...
(through $r$) and, more strongly (fourth power), on the local gradient
...
\item \textbf{Smooth/well-resolved field} ($g\to0$, i.e.\
  ...
  $\taurelax_0\gtrsim4r\in[4.9,\,9.8]$, \emph{higher} than the isotropic
...
through the $g$- or $r$-dependent factors, which are ratios of
same-mode quantities and are identification-independent.

\section{The Implemented Wave Speed}
\label{sec:confrontation}

\code{chemistry\_get\_physical\_hyperbolic\_soundspeed}
(\code{hyperbolic/chemistry\_getters.h:37--55}) is a single, general
...
\item \textbf{Unifying around $\Deff$ (Eq.~\eqref{eq:D_definition}), rather
  than special-casing $\Kmat$ per mode, is what makes every pairing
  consistent at once.} The formula contains no assumption about which
  \code{diffusion\_mode} produced $\Kmat$: it asks for $\Deff$ and takes
  its largest eigenvalue, which Proposition~\ref{prop:main} already proves
  is safe for any $\Kmat$ shape, and exact whenever $\Deff$ is isotropic
  (every mode paired with either relaxation-time model).
\item \textbf{No interface normal is needed anywhere}, confirming
  Remark~\ref{rem:lambda-max-not-n}'s conclusion: making the formula
  general is entirely about which particle-local tensor ($\Kmat$ vs.\
  $\Deff$) and which particle-local scalar ($\norm{\cdot}_F$ vs.\
  $\lambda_1$) to use, never about resolving $\chyp$ per-interaction.
\end{itemize}
...
(\code{chemistry\_riemann\_HLL.h:112--133}, the large-$\taurelax$ regime).
With
$\lambda_+=-\lambda_-\equiv\Lambda$ (the code's symmetric-ish estimate,
\code{chemistry\_riemann\_HLL.h:74--75}), the HLL dissipation term simplifies
to
\begin{equation}
...
\section{A Causal Bound on the Dissipation}
...
$r_{ij}\gtrsim\chyp\taurelax$, i.e.\ exactly in the wave-dominated regime
this document's characteristic analysis is about.
...
\section{Practical Guidance for Choosing $\taurelax_0$}
...
(\code{chemistry\_riemann\_compute\_hyperbolic\_blending\_factor},
\code{chemistry\_riemann\_utils.h:179--239}), which decides whether a
step runs through the Hopkins-limited parabolic branch ($\alpha\to0$) or
the plain, unlimited HLL branch just discussed ($\alpha\to1$).

$\alpha=\rho_\star/(0.1+\rho_\star)$ with
$\rho_\star=\taurelax_\star/\taurelax_{\rm num}$,
$\taurelax_\star$ the harmonic mean of the pair's two relaxation times
...
fixed, resolution/shear/$\kappaC$-independent geometric prefactor
(verified against the code's own formulas numerically). Since $\gammak h$
...
limiter of Section~\ref{sec:hyperbolic-limiter} softens this tension.

\section{A Hopkins-2017-Style Limiter for the Hyperbolic Branch}
\label{sec:hyperbolic-limiter}

\subsection{Why the Obvious Port Doesn't Work}
...
\section{The Half-Timestep Predictor}
\label{sec:predictor}

\code{chemistry\_gradients\_predict} (\code{hyperbolic/chemistry\_gradients.h:161--281})
...
over a half-timestep $h=\tfrac12\min(\dt_i,\dt_j)$. The two extrapolations
...
\begin{equation}
  U_0(\mathbf x_{\rm face},t^n+h) \approx
  U_0(\mathbf x_{\rm cell},t^n) + \grad U_0\cdot\Delta\mathbf x
  - h\,(\div\Fdiff)_{\mathbf x_{\rm cell}} ,
\end{equation}
...
$-h\,(\div\Fdiff)_{\mathbf x_{\rm cell}}$ are summed into a single
...
line 252, the same call that limits $\Fdiff$ below). The face limiter
...
inconsistent whenever $h\gtrsim\taurelax$: exactly why the source term
...
\begin{equation}
  ...
  \Fdiff(\mathbf x_{\rm face},t^n+h) =
  \big[\Fdiff(\mathbf x_{\rm cell},t^n)+\grad\Fdiff\cdot\Delta\mathbf x\big]\,
  e^{-h/\taurelax} + \Fdiff_{\rm parabolic}(\mathbf x_{\rm cell})\,
  \big(1-e^{-h/\taurelax}\big)
\end{equation}
(\code{chemistry\_gradients.h:253--271}): first spatially reconstruct
...
afterwards) is also conceivable. The two are not equivalent.

\begin{proposition}[Eq.~\eqref{eq:predictor-order} is the exact solution of the locally-available problem]
\label{prop:predictor}
Treat $\Fdiff_{\rm parabolic}$ as spatially uniform over the cell, equal to its only
available value at $\mathbf x_{\rm cell}$, the same approximation
already forced by Remark~\ref{rem:cell-centred-gradients-only}. Then
$\partial_t\Fdiff=-(\Fdiff-\Fdiff_{\rm parabolic})/\taurelax$ has no spatial-derivative
coupling: it is an independent ODE at every point $\mathbf x$, with
$\Fdiff_{\rm parabolic}$ constant in $\mathbf x$. Given the linear initial profile
$\Fdiff(\mathbf x,t^n)=\Fdiff(\mathbf x_{\rm cell},t^n)+\grad\Fdiff\cdot(\mathbf x-\mathbf x_{\rm cell})$
supplied by the gradient estimator, the exact solution at every
...
\end{equation}
Evaluating at $\mathbf x=\mathbf x_{\rm face}$, $t=t^n+h$ reproduces
Eq.~\eqref{eq:predictor-order} exactly.
\end{proposition}
\begin{proof}
With $\Fdiff_{\rm parabolic}$ constant in $\mathbf x$,
$\partial_t[\Fdiff-\Fdiff_{\rm parabolic}]=-[\Fdiff-\Fdiff_{\rm parabolic}]/\taurelax$ holds
separately at each $\mathbf x$, solved by
$[\Fdiff(\mathbf x,t^n)-\Fdiff_{\rm parabolic}]\,e^{-(t-t^n)/\taurelax}$ regardless
of how $\Fdiff(\mathbf x,t^n)$ depends on $\mathbf x$.
\end{proof}
...
\begin{equation}
  \Fdiff(\mathbf x_{\rm face},t^n+h) =
  \underbrace{\Fdiff(\mathbf x_{\rm cell},t^n)e^{-h/\taurelax}
  +\Fdiff_{\rm parabolic}(1-e^{-h/\taurelax})}_{\text{exact cell-centre relaxation}}
  \;+\;
  \underbrace{\big(\grad\Fdiff\cdot\Delta\mathbf x\big)\,e^{-h/\taurelax}}_{\text{gradient, decayed}} .
\end{equation}
The gradient contribution decays at the same rate as the mean, since
$\Fdiff_{\rm parabolic}$ carries no gradient of its own in this local approximation
-- both pieces of Proposition~\ref{prop:predictor}'s exact solution decay
identically. The reverse order (relax the cell-centred value first, then
add $\grad\Fdiff\cdot\Delta\mathbf x$ \emph{un}decayed) would instead give
$\Fdiff_{\rm parabolic}+[\Fdiff(\mathbf x_{\rm cell},t^n)-\Fdiff_{\rm parabolic}]e^{-h/\taurelax}
...
As $\taurelax\to\infty$, $e^{-h/\taurelax}\to1-h/\taurelax+O(\taurelax^{-2})$
...
$\grad\Fdiff\cdot\Delta\mathbf x$ before the exact time integration
(\code{chemistry\_gradients.h} line 242 runs before line 253), consistent
...
\part{Exact Solutions for Validation}

\section{Exact Solutions of the Cattaneo Equation}
...
\subsection{Comparing Against a Simulated Profile}
