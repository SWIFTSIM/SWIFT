...
\code{GIZMO\_LANSON\_VILA\_PARTICLE\_SIZE}, $\dx = a\,\delta x_i$ is
used instead, with $\delta x_i$ built from the same face-area
accumulator as Eq.~\eqref{eq:face_area}. Their eq.~(58) defines
$\delta x_i = 1/(2\sum_l w_l\|A_{il}\|)$ with $w_l$ the neighbour's
\emph{volume}, evaluated here as $\sum_l\|A_{il}\|/V_l$; the kernel
