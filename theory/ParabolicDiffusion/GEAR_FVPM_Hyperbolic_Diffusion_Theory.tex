...
on top of, and independent from, the missing Hopkins-2017-type limiter on
$F_{\rm diss}$ in that same regime (the companion document's parabolic
Hopkins-HLL solver, Section ``The Hopkins HLL-Type Flux'' there, has such
a limiter; no equivalent exists yet for the $\alpha\approx1$ hyperbolic
branch). Since
$\chyp=\sqrt{\lambda_1(\Deff)/\taurelax}$ is exact for isotropic $\Deff$
and a tight, safe bound otherwise (Section~\ref{sec:confrontation}), this
particular lever on the ``too diffusive at large $\taurelax$'' symptom is
already pulled as far as it goes; porting the Hopkins-2017 limiter itself
to the hyperbolic branch is the remaining, independent lever.
...
are the same knob, pointing in opposite directions, until the
Hopkins-2017 limiter is ported to the $\alpha\approx1$ hyperbolic branch
(Section~\ref{sec:hll-consequence}). Within the unlimited regime itself,
larger $\taurelax_0$ is not obviously worse: the numerical dissipation
magnitude $F_{\rm diss}\propto\chyp\propto\taurelax^{-1/2}$
\emph{shrinks} as $\taurelax_0$ grows, even though the Hopkins-style
protection against it disappears at essentially the same threshold where
the speed benefit appears.
...
As an interim starting point, pending the Hopkins-2017 hyperbolic port:
$\taurelax_0$ a few times the relevant speed-parity threshold (order
$4$--$10$ for \code{isotropic_smagorinsky}; toward the higher end, and
locally variable, for \code{anisotropic_gradient} per
Eq.~\eqref{eq:aniso_threshold}), validated against a matching parabolic
run on the same problem before trusting quantitative results -- not yet a
final, calibrated production value.
