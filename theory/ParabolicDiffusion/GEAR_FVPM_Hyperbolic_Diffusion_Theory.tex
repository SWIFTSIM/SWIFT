...
least-squares gradient estimator (parabolic companion document) computes
...
\begin{corollary}
\label{cor:isotropic}
$\Kmat=\kappaP\mathbf I$ in both \code{isotropic_constant} and
\code{isotropic_smagorinsky} modes, but the driver $\driver$ that fixes
$\kappaP$'s \emph{units} -- and hence $\mu=\driver/U_0$ -- differs between
them (Section~\ref{sec:quasilinear}):
\begin{align}
  \driver &= U_0 \quad (\mu=1)\quad \text{for \code{isotropic_constant}}
  && \big[\kappaP\ \text{has units}\ \text{length}^2\,\text{time}^{-1}\big] , \\
  \driver &= U_0/\rho \quad (\mu=1/\rho)\quad \text{for \code{isotropic_smagorinsky}}
...
$\driver$ convention is in force for the active \code{diffusion_mode}, or
it silently mixes an ordinary diffusivity with a mass diffusivity.
Section~\ref{sec:io-units-bug} documents one place this previously went
unnoticed.
...
\item $\taurelax\to0$: the first term drops and
  ...
  companion document (Eq.~1 there). This limit is consistent with
  \code{chemistry_part_integrate_flux_source_term}'s exact solution
  (\code{hyperbolic/chemistry_flux.h:105--129}), which computes
...
from $\Splus$ in \code{anisotropic_gradient} mode
...
\end{equation}
(the second expression is the code's \code{isotropic_smagorinsky}
parabolic timestep, \code{chemistry_timesteps.h:107--113}, using
$\lambda_1(\Kmat)=\kappaP$, exact for isotropic $\Kmat$ --
Section~\ref{sec:related-fixes}; the first uses the identification
...
\subsection{Speed Criterion for \code{anisotropic_gradient} Mode}
\label{sec:speed-criterion-aniso}
The derivation above is specific to \code{isotropic_smagorinsky}: its
...
(\code{chemistry_timesteps.h:111--117}), independent of the local
gradient. Neither holds for \code{anisotropic_gradient}: its $\kappaP$
has no $\norm{\Shear}_F$ factor at all (shear enters only through
$\Splus$'s eigenvalues), and its parabolic timestep uses the
gradient-suppressed
$\expr=\norm{\driver}\dx/(\norm{\grad\driver}\dx+\norm{\driver})$
(\code{chemistry_timesteps.h:118--124}). Redo the ratio for this mode,
...
something already accounted for. Verified against a direct numerical
evaluation of the code's own formulas (Python, $2\times10^4$ random
trials spanning $\CCFL$, $\kappaC$, $h$, $\gammak$, $\rho$, $\taurelax_0$,
$\dx$, $\norm{\driver}$, $\norm{\grad\driver}$, and random traceless
symmetric $\Shear$): relative agreement at machine precision
($<10^{-14}$).
...
\item \textbf{Smooth/well-resolved field} ($g\to0$, i.e.\
  ...
  \code{anisotropic_gradient} degenerate to
  \code{isotropic_smagorinsky}'s timestep form): the threshold reduces to
  $\taurelax_0\gtrsim4r\in[4.9,\,9.8]$ -- \emph{higher} than the isotropic
  $4$, because $\kappaP$ lacks the $\norm{\Shear}_F$ boost
  \code{isotropic_smagorinsky} gets, making $\Deff$ smaller for the same
...
\code{chemistry_get_physical_hyperbolic_soundspeed}
(\code{hyperbolic/chemistry_getters.h:37--55}) is a single, general
formula, valid for every \code{diffusion_mode} and
\code{relaxation_time_mode}:
\begin{equation}
...
\code{relaxation_time_mode} is needed inside the getter: $\taurelax$ is
...
\code{isotropic\_constant} + \code{turbulent\_mode} & $\Deff=\Kmat$ ($\mu=1$) isotropic: $\lambda_1(\Deff)=\kappaP$ for every $\nunit$ & \textbf{consistent (exact)} \\
\code{isotropic\_smagorinsky} + \code{turbulent\_mode} & reduces algebraically to Eq.~\eqref{eq:code_turbulent_new} exactly & \textbf{consistent (exact)} \\
\code{anisotropic\_gradient} + \code{turbulent\_mode} & $\lambda_1(\Deff)$, a tight bound on $\nunit^{\!\top}\Deff\nunit$ (Cor.~\ref{cor:frobenius}) & \textbf{consistent (safe bound)} \\
\bottomrule
\end{tabular}
...
with no cross-validation) against the general result of
Proposition~\ref{prop:main}. \textbf{All six pairings are consistent.}}
\end{table}
...
\item \textbf{Unifying around $\Deff$ (Eq.~\eqref{eq:D_definition}), rather
  ...
  \code{diffusion_mode} produced $\Kmat$ -- it asks for $\Deff$ and takes
...
\item \code{examples/ChemistryTests/MetalDiffusionOnePeak/params.yml}
...
Section~\ref{sec:effective-diffusivity} also apply to two more places in
the code.
...
(\code{chemistry_riemann_HLL.h:112--133}, the large-$\taurelax$ regime).
With
...
itself is limited in that regime the way the companion document's
parabolic Hopkins-HLL solver limits its own dissipation (Section ``The
Hopkins HLL-Type Flux'' there). Since
...
wave-speed lever is already pulled as far as it goes; Section~\ref{sec:hyperbolic-limiter}
...
\code{isotropic_smagorinsky}+\code{turbulent_mode}, substituting
$\taurelax=\taurelax_0/(\kappaC\norm{\Shear}_F)$ and
...
(empirically, a further $\sim\!10\%$ RMS-spread reduction and several-fold
better peak retention at $\taurelax_0=10^2$, on top of the wave-speed fix
above) but does not eliminate it: the limiter suppresses dissipation the
flux-memory signal identifies as unjustified, it does not restore full
Hopkins-style protection at $\alpha\approx1$. Within the unlimited regime
itself, larger $\taurelax_0$ is not obviously worse on the wave-speed
lever alone: the numerical dissipation magnitude
$F_{\rm diss}\propto\chyp\propto\taurelax^{-1/2}$ \emph{shrinks} as
$\taurelax_0$ grows, even though full Hopkins-style protection is still
only available at $\alpha\approx0$.

\begin{remark}[Existing empirical guidance predates several fixes in this document]
The \code{l5\_tau\_1e-1} through \code{l5\_tau\_1e2} runs that first
identified the ``too diffusive at large $\taurelax$'' symptom -- the
observation that motivated this entire investigation -- were made before
the $\taurelax_0$-$\kappaC$ coupling, the $\mu$/$\rho$ correction, and the
$\lambda_1(\Deff)$ unification (all derived in this document) were
implemented. Those fixes change what a given numerical $\taurelax_0$
value physically means (e.g.\ the isotropic threshold moved from
$\taurelax_0\gtrsim1/\kappaC$ to $\taurelax_0\gtrsim4$), so which
$\taurelax_0$ ``looks too diffusive'' should be re-checked against the
current code before being treated as settled.
\end{remark}

As an interim starting point: $\taurelax_0$ a few times the relevant
speed-parity threshold (order $4$--$10$ for \code{isotropic_smagorinsky};
toward the higher end, and locally variable, for
\code{anisotropic_gradient} per Eq.~\eqref{eq:aniso_threshold}), with the
flux-memory limiter (Section~\ref{sec:hyperbolic-limiter},
\code{hyperbolic_limiter_scope: AllComponents}) enabled, validated against
a matching parabolic run on the same problem before trusting quantitative
results -- not yet a final, calibrated production value.
...
(Section~\ref{sec:tau-modes}) already computed for the same pair.
...
monotonic power-law relationship, $r\propto\rho_\star^{-1/2}$, verified
against the code's own formulas (20 random trials over the free
parameters, agreement at machine precision). The swapped $r$ therefore
...
\subsection{Scope, and the Empirical Comparison}
Hopkins' original limiter was built for a 1-component state (density
only); $F_{\rm diss}$ here has 4 (density and 3 flux components). Whether
$\eta$ should also multiply the flux-component dissipation
($F_{\rm diss,1\text{-}3}$) was not obvious a priori: limiting it could
either suppress genuine numerical excess there too, or interfere with the
flux components' own physical decay toward $\Fdiff_{\rm parabolic}$,
making the memory persist \emph{longer}. Both scopes were implemented
(\code{GEARChemistry:hyperbolic\_limiter\_scope}, \code{Density} vs.\
\code{AllComponents}) and compared on the same \code{MetalDiffusionOnePeak}
sweep used throughout this document (\code{constant\_mode}+
\code{isotropic\_constant}, level 5, $\taurelax\in\{10^{-1},10^0,10^1,10^2\}$),
against the no-limiter baseline:
\begin{center}
\begin{tabular}{cccc}
\toprule
$\taurelax$ & RMS spread (limiter/baseline) & peak $\rho Z$ (limiter/baseline) \\
\midrule
$10^{-1}$ & $0.995$ (Density), $0.995$ (All) & $1.03\times$, $1.03\times$ \\
$10^{0}$  & $0.994$ (Density), $0.993$ (All) & $1.04\times$, $1.06\times$ \\
$10^{1}$  & $0.953$ (Density), $0.942$ (All) & $1.18\times$, $1.23\times$ \\
$10^{2}$  & $0.904$ (Density), $0.897$ (All) & $8.11\times$, $5.02\times$ \\
\bottomrule
\end{tabular}
\end{center}
Both scopes reduce the RMS spread further at every $\taurelax$ (beyond
the $\lambda_1$-vs-Frobenius fix alone, Section~\ref{sec:related-fixes}),
growing with $\taurelax$ -- exactly where the missing limiter's effect
was largest. \code{AllComponents} matches or beats \code{Density} on the
RMS spread (the more robust of the two metrics; peak $\rho Z$ is a
single-particle statistic, noisier) at every $\taurelax$ tested,
including $\taurelax=10^2$ where the two scopes' peak-retention numbers
diverge most -- i.e.\ the a priori concern that limiting the flux
components would preserve memory and make things worse is not borne out
here. \textbf{\code{AllComponents} is therefore the recommended default},
matching how the existing $\alpha$ blend already applies uniformly across
all 4 components; \code{Density} remains available
(\code{hyperbolic\_limiter\_scope: Density}) since it is not clearly
worse and matches Hopkins' original scope exactly.

Visual inspection of the profile plots confirms no pathological behaviour
introduced by the limiter (no oscillation, no negative dips): the
$\taurelax=10^2$ profile with the limiter sits visibly closer to the pure
Cattaneo (top-hat) analytic solution than the unlimited baseline did --
consistent with the limiter suppressing spurious numerical dissipation
rather than introducing a new artefact.

Mass conservation was verified exact (to machine precision) in every run,
with and without the limiter, at every $\taurelax$ and scope -- the
limiter only redistributes numerical dissipation among the HLL flux's
components, it cannot break the underlying conservation law.
...
\item \textbf{Proposition~\ref{prop:main} is the general result}: $\chyp(\nunit)
  ...
  \code{diffusion_mode} (Corollary~\ref{cor:isotropic}).
\item \code{chemistry_get_physical_hyperbolic_soundspeed} implements a
  single formula for every (\code{diffusion_mode},
  \code{relaxation_time_mode}) pairing:
  $\chyp=\sqrt{\lambda_1(\Deff)/\taurelax}$. $\lambda_1(\Deff)$ is the
  largest eigenvalue of the particle's own $\Deff$ (helpers
  ...
  elsewhere in this module) -- exact for isotropic $\Deff$ (every
  reachable pairing), and a tighter-than-Frobenius safe overestimate for
  anisotropic $\Deff$ (Corollary~\ref{cor:frobenius},
  Remark~\ref{rem:lambda-max-not-n}). No branch on
  \code{relaxation_time_mode} is needed inside the getter: $\taurelax$ is
  read from the already-correctly-computed
  \code{p->chemistry\_data.tau} (Section~\ref{sec:confrontation}).
\item The parabolic timestep (\code{chemistry_diffusion_timestep},
  non-hyperbolic branch) uses the same $\lambda_1(\Deff)$ construction,
  for the same reason (Section~\ref{sec:related-fixes}): the
  explicit-diffusion stability limit is set by $\Deff$'s largest
  eigenvalue, not its Frobenius norm.
...
\item \textbf{Snapshot units.} The \code{"DiffusionMatrices"} snapshot
  field wrote raw $\Kmat$ tagged with \code{UNIT\_CONV\_MASS\_DIFFUSIVITY}
  -- dimensionally wrong for \code{isotropic_constant}, where $\Kmat$ has
  ordinary ($\text{length}^2/\text{time}$) diffusivity units, not
  mass-diffusivity units (Section~\ref{sec:io-units-bug}). Fixed by
  outputting $\Deff$ (always ordinary-diffusivity units, by construction)
  with SWIFT's existing \code{UNIT\_CONV\_DIFFUSIVITY} tag instead.
\item \textbf{Missing flux output.} Snapshots for
  \code{CHEMISTRY_GEAR_FVPM_HYPERBOLIC_DIFFUSION} wrote the relaxation
  time and (in debug builds only) the \emph{norm} of the diffusion flux,
  but never the flux vector $\Fdiff$ itself. Added an unconditional (not
  debug-gated) \code{"DiffusionFluxes"} output field writing all three
  components per element.
\item \textbf{No bug, now proven.} The half-timestep predictor
  (Section~\ref{sec:predictor}, \code{chemistry\_gradients\_predict})
  extrapolates $\Fdiff$ to the interface space-first, then relaxes the
  whole extrapolated value toward the cell-centred $\Fdiff_{\rm parabolic}$ exactly.
  Proposition~\ref{prop:predictor} shows this is the exact solution of
  the locally-available relaxation problem (given $\Fdiff_{\rm parabolic}$ can only
  ever be evaluated at the cell centre); the reverse order would leave a
  spurious, undecayed gradient artefact in the $\taurelax\to0$ limit. No
  source change was needed.
\item \textbf{\code{anisotropic_gradient} speed criterion, derived.}
  Section~\ref{sec:speed-criterion-aniso} redoes
  Section~\ref{sec:speed-criterion}'s ratio for this mode:
  $\taurelax_0\gtrsim4r/(1+g)^4$
  (Eq.~\eqref{eq:aniso_threshold}), with $r=\norm{\Shear}_F/\lambda_1(\Shear)\in[\sqrt{3/2},\sqrt6\,]$
  the local shear anisotropy and $g=\norm{\grad\driver}\dx/\norm{\driver}$
  the local gradient steepness. Not a single constant like the isotropic
  $4$: at $g=0$ the threshold is \emph{higher} ($\in[4.9,9.8]$, since this
  mode's $\kappaP$ lacks the $\norm{\Shear}_F$ boost
  \code{isotropic_smagorinsky} gets); at large $g$ (steep,
  under-resolved gradients -- the SN-injection-site regime motivating
  this whole investigation) it falls as $g^{-4}$, well below $4$, since
  $\dt_{\rm par}$ shrinks with the gradient while $\dt_{\rm hyp}$ (a pure
  wave-speed CFL) does not.
