...
(GIZMO's own MFV/MFM sound-speed estimate is exactly analogous).

\textbf{The driver $\driver$ used in the hyperbolic flux law is not
uniformly $U_0/\rho$.} \code{chemistry_compute_flux}
(\code{hyperbolic/chemistry_flux.h:159--172}) and
\code{chemistry_riemann_compute_hyperbolic_blending_factor}
(\code{chemistry_riemann_utils.h}) both branch on \code{diffusion_mode}:
\begin{equation}
  \driver =
  \begin{cases}
    U_0 & \text{\code{isotropic_constant}} \\
    U_0/\rho & \text{otherwise (\code{isotropic_smagorinsky},
    \code{anisotropic_gradient})}
  \end{cases}
\end{equation}
Define $\mu\equiv\partial\driver/\partial U_0\in\{1,1/\rho\}$
accordingly ($\mu$ is exactly the $\driver^\star/U^\star$ ratio already
computed by \code{chemistry_riemann_compute_hyperbolic_blending_factor}
for a different purpose). Under the frozen-coefficient approximation,
$\driver=\mu U_0$ is linear in $U_0$ for either value of $\mu$, which is
all the Jacobian derivation below actually needs -- but $\mu$ itself is
\emph{not} a universal $1/\rho$, and an earlier version of this document
incorrectly assumed it was, missing the \code{isotropic_constant} case.
That error propagated into an initial fix of
\code{chemistry_get_physical_hyperbolic_soundspeed} that added an
unconditional $\rho^{-1}$; the corrected version (Section~\ref{sec:confrontation})
applies $\mu$, not $1/\rho$, per \code{diffusion_mode}.
...
Writing $\mathbf v \equiv \mu\Kmat\nunit/\taurelax$ (a 3-vector, with $\mu$
as defined in Section~\ref{sec:quasilinear} above), direct
...
fully anisotropic), any relaxation-time model, any unit interface
normal $\nunit$, and $\mu=\partial\driver/\partial U_0$ as defined in
Section~\ref{sec:quasilinear} ($\mu=1$ for \code{isotropic_constant},
$\mu=1/\rho$ otherwise), the projected flux Jacobian $\mathbf A(\nunit)$
...
and since $x_0\neq0$,
\begin{equation}
  \lambda^2 = \nunit\cdot\mathbf v = \frac{\mu\,\nunit\cdot(\Kmat\nunit)}{\taurelax} = \frac{\mu\,\nunit^{\!\top}\Kmat\nunit}{\taurelax} \; \geq 0
\end{equation}
(non-negative because $\Kmat$ is PSD and $\mu>0$), giving the two roots
...
which closure $\kappaP$ came from: the two modes differ by exactly the
factor $\mu$.
\end{corollary}
...
\item \textbf{Proposition~\ref{prop:main} is the general result}: $\chyp(\nunit)
  ...
  motivated this document: a mode-independent derivation is possible. The
