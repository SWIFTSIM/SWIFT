...
\title{Theory of the Hyperbolic (Cattaneo-Relaxation) FVPM
Diffusion Scheme}
...
(Berthon et al.~\cite{Berthon2007}).
...
flux-limiter calibration). A physically meaningful
...
$\dx=(V_i/V_{\rm unit\ sphere})^{1/d}$ (\code{chemistry_timesteps.h}'s
\code{else} branch). An alternative is the Lanson \&
Vila~\cite{LansonVila2008} mean interparticle distance, their eq.~(58),
\begin{equation}
...
\code{chemistry_timesteps.h}'s \code{GIZMO_LANSON_VILA_PARTICLE_SIZE}
...
length -- the same resolution scale as the Gizmo size, not a larger one.
Neither definition therefore admits systematically larger timesteps than
the other: swapping definitions is not a route to a faster scheme. When
evaluating the sum, note that dimensional analysis
...
The hyperbolic branch of \code{chemistry\_riemann\_HLL.h} already caps
$F_{\rm diss}$ by a second, independent mechanism: a two-argument minmod
between $(1+\psi)F_{\rm transport}$ and $F_{\rm transport}+F_{\rm diss}$,
with $\psi=$\code{chem\_data->hll\_riemann\_solver\_psi} a free,
per-simulation-tuned parameter, mirroring the parabolic scheme's own
Hopkins-HLL construction referenced in
Section~\ref{sec:hll-consequence}. Both mechanisms rescale the same term
in pursuit of the same goal -- suppressing the raw HLL dissipation's
tendency to over-transport -- and are empirically redundant: enabling
either one alone already saturates the achievable reduction in
super-causal transport, and running both together adds nothing beyond
either alone. The causal bound is the preferable formulation of the two:
$\zeta_{ij}$ has no tunable parameter to calibrate per problem, and it is
derived directly from $\chyp$, the one quantity the wave-speed analysis
of this document (Proposition~\ref{prop:main},
Section~\ref{sec:confrontation}) already fixes as exact or a tight,
safe bound -- rather than from a free constant chosen to match a given
test problem.
...
Across the point-source, two-source, and half-box test problems, the
causal bound matches or improves on the tuned minmod/psi bound on every
transport metric examined, while removing the one free parameter the
latter requires.
