...
% ($c_{\rm hyp}$, $F_{\rm diss}$, ...), matching the old article.cls
...
Diffusion Scheme}
\subtitle{Theory document: Cattaneo relaxation, characteristic speed, and
exact solutions}
...
This is the theoretical and numerical reference for the hyperbolic
(Cattaneo-relaxation) variant of the GEAR FVPM metal-diffusion chemistry
scheme (\code{src/chemistry/GEAR\_FVPM\_DIFFUSION/hyperbolic/}), configured
with \code{--with-chemistry=GEAR-FVPM-HYPERBOLIC-DIFFUSION\_N}. It builds
on the parabolic companion document's FVPM discretisation and
Riemann-solver framework (\code{GEAR\_FVPM\_Diffusion\_Theory.tex}, this
directory); user-facing configuration and parameters are documented in
the RTD pages under \code{doc/RTD/source/SubgridModels/GEAR/diffusion/}.
...
(\code{GEAR\_FVPM\_Diffusion\_Theory.tex}, this directory): a first-principles
derivation of the characteristic (wave) speed of the hyperbolic
(Cattaneo~\citeyear{Cattaneo1948}-relaxation) diffusion system implemented under
\code{CHEMISTRY\_GEAR\_FVPM\_HYPERBOLIC\_DIFFUSION}
(\code{src/chemistry/GEAR\_FVPM\_DIFFUSION/hyperbolic/}), and a direct
