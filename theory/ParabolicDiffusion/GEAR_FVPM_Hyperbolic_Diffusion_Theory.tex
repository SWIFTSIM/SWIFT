...
Section~\ref{sec:related-fixes}). The resolution $h$, diffusivity
normalisation $\kappaC$, and local shear $\norm{\Shear}_F$ all cancel from
the ratio:
\begin{equation}
...
Outside the scope of this document's $\chyp$ review:
\begin{itemize}
\item Whether the Speed Criterion of Section~\ref{sec:speed-criterion}
  ($\taurelax_0\gtrsim4$) transfers to \code{anisotropic_gradient} mode.
  That derivation is specific to \code{isotropic_smagorinsky}: in
  \code{anisotropic_gradient} mode $\kappaP$ carries no $\norm{\Shear}_F$
  factor (Section~\ref{sec:diffusion-tensor-recap}), so $\lambda_1(\Deff)$
  scales with $\lambda_1(\Shear)$ rather than $\norm{\Shear}_F$, and
  $\dt_{\rm par}$ picks up the additional gradient-suppression factor
  $(\norm{q}\dx)/(\norm{\grad q}\dx+\norm{q})$
  (\code{chemistry\_timesteps.h:118--124}) that has no isotropic
  counterpart. The two effects push the threshold in opposite directions,
  so it is not simply $4$ -- not yet derived.
\end{itemize}
