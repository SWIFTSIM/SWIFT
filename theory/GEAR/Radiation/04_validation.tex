...
entire benefit over uniform disappears at slow rebuild cadences, not
just numerically.} Comparing the \code{rebuild=0.1\,Myr} slices
directly (Figure~\ref{fig:gas20-slices}, right column): uniform and
angular show the \emph{same} shape -- a round bubble flattened right at
the clump boundary, $r_{\rm HII}$ within 0.5\,pc of each other
(13.60\,pc vs.\ 14.04\,pc). The per-pixel budget floor that protects
angular from total clump monopolization at a fast cadence provides no
visible protection once the cadence is slow enough that a starved pixel
has nothing left to spend for most of a rebuild cycle regardless. This
...
\textbf{Is angular better than uniform in general?} The mechanism is
real and structurally sound: each pixel's guaranteed budget share means
the clump can never fully monopolize a star's photon supply the way it
can under uniform's single shared budget, which is exactly the failure
mode both reference papers designed HealPix splitting to avoid
(Section~\ref{sec:angular-splitting}). But across this matrix, that
mechanism translates into a demonstrably \emph{larger true ionized
region} in exactly one place: \code{gas\_mass=20} at
\code{rebuild=0.01\,Myr} (Section~\ref{sec:matrix-gas20}). Everywhere
else tested here -- \code{gas\_mass=20} at slower cadences,
\code{gas\_mass=95} at any cadence -- the true ionized region
($r_{\rm physics}$) is essentially the same size for both schemes; only
angular's own $r_{\rm HII}$ bookkeeping is noisier (both understating
it, Section~\ref{sec:matrix-gas20}, and overstating it, this section,
depending on resolution and cadence). \textbf{The isotropy/mass-bias
protection is a real, always-present property of the mechanism; a
measurably larger ionized region is not a given consequence of it at
every resolution or cadence tested so far, and $r_{\rm HII}$ itself
should be trusted less, not more, when the angular scheme is in use.}
