...
half-width, $R_{\rm st}=26.4\,$pc for the default
...
curve. The final-time relative error is 41.7\% (41.6\,pc simulated vs.\
71.3\,pc analytic at $t=5.38\,$Myr) -- already a clear fail against the
default 30\% tolerance -- but the shape mismatch is more informative
than that one number: the simulated curve is a \emph{staircase}, flat
between rebuilds and jumping at each one, while the analytic curve is
smooth and already past $R_{\rm st}$ from very early times. The
simulated curve does not even first cross $R_{\rm st}$ until
$t\approx3.8\,$Myr -- more than 80\% of the run -- whereas the D-type
solution reaches $R_{\rm st}$ essentially at $t=0$ by construction
(the classical solution assumes the R-type phase that establishes
$R_{\rm st}$ is effectively instantaneous, then D-type expansion
proceeds from there).
...
\begin{figure}
  ...
  \caption{Simulated $r_{\rm HII}(t)$ (blue, algorithm bookkeeping) vs.\
    the analytic Spitzer (1978) D-type solution (orange dashed) and the
    equilibrium $R_{\rm st}$ (grey dotted), single isolated star,
    $Z/Z_\odot\approx0.231$, \texttt{rebuild\_time\_myr=0.5}. The
    simulated curve's staircase shape and $\sim$3.8\,Myr delay to first
    reach $R_{\rm st}$ show the mismatch is in the trajectory, not just
    the final value.}
  \label{fig:e10-dtype}
\end{figure}
