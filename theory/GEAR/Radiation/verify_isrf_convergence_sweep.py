"""Dense h/lambda CONVERGENCE STUDY for the scheme's discrete steady state,
extending `verify_isrf_discrete_steady_state_production_corners.py`
...
the scheme's ACTUAL DISCRETE steady-state solution, solved on real glass
particle positions and real per-particle h_i/rho_i, using the exact
diffmode==1 div(F) / diffmode==0 grad(u) pairwise formulas and the
exact-relaxation staggered iteration, run to its own fixed point, matches
the actual
...
1. **A much faster real-glass solver.** The corners script's Part 5 timed
   ...
   (21-155 iterations per corner). `verify_isrf_timestepping_stability
...
# ---------------------------------------------------------------------------
# Kernel functions, verbatim from verify_isrf_discrete_steady_state_
...
print(
    f"  self-check: idealized-lattice ratio at h/lambda=0.05,0.1: "
    f"{_thin_check[0]:.3f}, {_thin_check[1]:.3f} (continuum limit, expect ~1)"
)
print(
    f"  self-check: idealized-lattice ratio at h/lambda=19.93 (deepest corner): "
...
def tile_glass(snap, reps):
    ...
    offs = (
        np.array(np.meshgrid(*[np.arange(reps)] * 3, indexing="ij")).reshape(3, -1).T
        * box
    )
    ...
    return dict(
        boxsize=box * reps, pos=pos_t, h=h_t, rho=rho_t, mass=mass_t, star_pos=star_t
    )
...
def solve_real_glass_gmres(
    pos, h, rho, mass, boxsize, star_pos, lam, pairs, rtol=1e-12, maxiter=3000
):
    ...
    fit, verify_isrf_timestepping_stability.py Part C.2 proves this
...
established = {
    ...
    ("m760", "LW"): dict(
        h_over_lam=19.93, prior_predicted=0.270, measured=None, inferred=False
    ),
}
...
print(
    f"{'Corner':<10}{'band':>5}{'h/lambda':>10}{'GMRES lam_eff/h':>18}"
...
for label, subdir in run_dirs.items():
    ...
    if not snap_glob:
        print(
            f"  {label}: no snapshots found, skipping (scratch may have been cleaned up)"
        )
        continue
    ...
    for band, sigma_d in (("FUV", SIGMA_D_FUV_CGS), ("LW", SIGMA_D_LW_CGS)):
        ...
        lam_cgs = analytic_lambda_cgs(
            Z_mean, rho_mean, snap["unit_length_cgs"], snap["unit_mass_cgs"], sigma_d
        )
        ...
        ratio_h = (
            lam_fit / h_med if lam_fit is not None and np.isfinite(lam_fit) else np.nan
        )
        ...
        print(
            f"  {key_root:<8}{band:>5}{h_over_lam:>10.2f}{ratio_h:>18.3f}"
        ...
        corner_records.append(
            dict(
                label=f"{key_root} {band}",
                h_over_lam=h_over_lam,
                ratio_h=ratio_h,
...
print()
print(
    f"Max deviation of the GMRES solve from the corners script's own iterative-scheme"
)
...
print(
    "reproduces the already-validated per-corner discrete predictions to "
...
_default_snaps = sorted(
    glob.glob(os.path.join(SCRATCH, "run", "snap", "snapshot_*.hdf5"))
)
...
print(
    f"Canonical real glass: N={len(default_snap['pos'])}, box={box0:.4e}, h_med={h_med0:.4e}"
)

pairs_reps1 = build_pairs_fast(
    default_snap["pos"], default_snap["h"], default_snap["boxsize"]
)
...
sweep_rows = []
print(
    f"{'h/lambda':>10}{'method':>14}{'ratio_to_analytic (real-glass)':>32}"
...
for hol in h_over_lam_sweep:
    ...
    if hol >= REPS1_FLOOR:
        ...
        lam_fit, converged = solve_real_glass_gmres(
            ...
            default_snap["rho"],
            default_snap["mass"],
            default_snap["boxsize"],
            default_snap["star_pos"],
            lam,
            pairs_reps1,
        )
        ratio_real = (
            lam_fit / lam if lam_fit is not None and np.isfinite(lam_fit) else np.nan
        )
    ...
    dt_pt = time.time() - t0
    sweep_rows.append(
        dict(h_over_lam=hol, ratio_real=ratio_real, ratio_lattice=ratio_lattice)
    )
    print(
        f"{hol:>10.3f}{method:>24}{ratio_real:>18.3f}{ratio_lattice:>30.3f}{dt_pt:>8.1f}s"
    )
print()
...
valid_overlap = [
    ...
    if np.isfinite(row["ratio_real"]) and np.isfinite(row["ratio_lattice"])
]
assert len(valid_overlap) > 10, "too few overlapping sweep points to compare methods"
overshoot = [
    row["ratio_lattice"] / row["ratio_real"]
    for row in valid_overlap
    if row["h_over_lam"] > 1.0
]
print(
    f"Idealized-lattice/real-glass overshoot ratio, h/lambda>1 ({len(overshoot)} points): "
    f"min={min(overshoot):.2f}, max={max(overshoot):.2f}"
)
print(
    "(the corners script's own Part 1-3 vs Part 5 found 1.3x-2.3x at the five "
...
print(
    f"{'Corner':<10}{'band':>5}{'h/lambda':>10}{'dense-curve interp':>20}{'own-glass (Part 3)':>20}{'dev':>8}"
)
...
for c in corner_records:
    ...
    interp_log_ratio = np.interp(
        np.log(c["h_over_lam"]), log_hol_dense[order], log_ratio_dense[order]
    )
    ...
    print(
        f"  {c['label']:<13}{c['h_over_lam']:>10.2f}{interp_ratio:>20.3f}"
        f"{own_ratio_to_analytic:>20.3f}{dev * 100:>7.1f}%"
    )
print(
    f"Max deviation: {max_universality_dev * 100:.1f}%, the dense curve built from one glass"
)
print(
    "generalises to the other three glasses' own h/lambda values, so Panel 2's overlay"
)
print(
    "of all four corners' own measurements onto one dense curve is not comparing unlike"
)
...
measured_c = [
    c for c in corner_records if c["measured"] is not None and not c["inferred"]
]
...
corner_ratio_to_analytic = np.array(
    [c["measured"] * c["h_over_lam"] for c in measured_c]
)
...
inferred_ratio_to_analytic = np.array(
    [c["measured"] * c["h_over_lam"] for c in inferred_c]
)
# ratio_to_analytic = (lambda_eff/h) * (h/lambda_analytic) = (measured lambda_eff/h) * h_over_lam

print(
    f"Measured-corner residual vs this script's own real-glass prediction, the five "
...
assert (
    np.max(corner_err) < 0.10
), "measured-vs-real-glass residual grew beyond the established band"
...
print(
    f"Reconciliation: the same five corners' continuum-Yukawa error under the corners"
)
print(f"script's OWN convention (denominator = lambda_analytic, not this script's own")
print(
    f"real-glass solve): {np.min(recon_err) * 100:.0f}%-{np.max(recon_err) * 100:.0f}% "
    f"(corners script's own headline: 51%-249%)"
)
assert (
    0.3 < np.min(recon_err) < 0.8 and 1.5 < np.max(recon_err) < 3.5
...
ax1.loglog(
    ...
    "-",
    color="C3",
    label="continuum Yukawa vs. real-glass solve",
)
ax1.loglog(
    hol_valid,
    err_lattice,
    "-",
    color="C1",
    label="idealized perfect lattice vs. real-glass solve",
)
ax1.loglog(
    corner_hol,
    corner_err,
...
ax1.text(
    0.07, 1.2e-2, "idealized-lattice-only\nbelow this line", fontsize=7, color="dimgrey"
)
ax1.set_ylabel(
    "relative error in lambda_eff\n(denominator: this script's real-glass solve)"
)
ax1.set_title("the scheme discrete steady state: convergence vs. resolution (h/lambda)")
ax1.legend(fontsize=7, loc="upper left")
ax1.grid(True, which="both", alpha=0.2)

ax2.loglog(hol_arr, ratio_real_arr, "-", color="C2", label="real-glass discrete solve")
ax2.loglog(
    hol_arr, ratio_lattice_arr, "-", color="C1", label="idealized perfect lattice"
)
ax2.axhline(
    1.0,
    color="C3",
    linestyle="--",
    label="continuum Yukawa (lambda_eff = lambda_analytic)",
)
ax2.loglog(
    corner_hol,
    corner_ratio_to_analytic,
    "o",
    color="C0",
    markersize=8,
...
OUTPUT_PNG = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "design_b_convergence_sweep.png"
)
