...
\begin{itemize}
  ...
  \item[$\square$] \textbf{T6} -- Variable ISRF (interstellar radiation
    ...
    LW $11.2$--$13.6\,$eV, a hard sub-band), but under a fixed assumed
    ISRF spectral shape (e.g.\ Draine 1978/Mathis et al.\ 1983 -- the
    same move already made for $Q_H$ vs.\ $L_{\rm bol}$,
    Section~\ref{sec:rp-vs-hii}) a single local $G_0$-equivalent proxy
    can drive both: fed directly into \code{isrf\_habing} for PE
    heating, and converted via \citet{DraineBertoldi1996}'s
    $k_{\rm LW}(\chi)$ relation for \code{RT\_H2\_dissociation\_rate}.
    Still not scoped: the propagation scheme that would actually produce
    that one $G_0$ number.
