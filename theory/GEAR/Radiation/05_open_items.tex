...
\begin{itemize}
  ...
  \item[$\square$] \textbf{T10 -- confirmed (2026-07-21), decision
    ...
    \textbf{Update (2026-07-21)}: that next target has now been
    re-tried (Section~\ref{sec:husmith-current}) with the exact same
    $T_i=10^4\,$K debug flag, after first fixing a matching
    ...
    actually used). Result: \textbf{still FAILS}, median $33.51\%$,
    essentially unchanged from the deprecated $32.4\%$ -- confirming,
    with a properly-margined re-run (search radius verified not
    capped), that $T_i$ genuinely fixes STARBENCH's single-source case
    but does \emph{not} fix this multi-source one. The actual
    multi-source residual mechanism is still unidentified; see
    Section~\ref{sec:husmith4star} for what has already been ruled out.
\end{itemize}
