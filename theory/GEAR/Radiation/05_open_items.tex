...
\begin{itemize}
  ...
  \item[$\square$] \textbf{T10} -- STARBENCH re-validation, run against
    the current committed default (Section~\ref{sec:starbench-current}),
    \textbf{FAILS at 22.76\%} (tolerance 10\%). The likely cause is the
    same $T_i$-convention mismatch already flagged in this session's
    e10 isolation tests (Section~\ref{sec:e10}): the current default
    build gives Z=0 gas a natural, metallicity-dependent $T_i\approx
    47500\,$K, not STARBENCH's own assumed flat $10^4\,$K. The only
    PASS found anywhere in this project's history
    (Section~\ref{sec:starbench}) needed \emph{all three} of the
    loosened search-radius cap, adaptive rebuild cadence, \emph{and}
    $T_i=10^4\,$K forced via an uncommitted compile-time debug flag --
    never buildable by default. \textbf{Not yet done}: re-confirm this
    by rerunning with the debug flag on top of today's (changed)
    cap/cadence defaults; then decide whether matching STARBENCH's flat-$T_i$
    convention should become an actual default-buildable option (not
    just a debug flag) if confirmed necessary, or whether this code's
    natural non-isothermal $T_i$ is the intended, more physical
    behaviour and STARBENCH comparison is understood to always require
    an explicit flat-$T_i$ mode. This blocks calling STARBENCH
    validated and, per the restart ordering
    (Section~\ref{sec:validation-targets}), blocks moving on to the
    Hu et al.\ (2017)/Smith et al.\ (2021) 4-star target next.
\end{itemize}
