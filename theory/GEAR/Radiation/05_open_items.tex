...
\begin{itemize}
  ...
  \item[$\square$] \textbf{T10 -- confirmed (2026-07-21), decision
    ...
    \textbf{Update (2026-07-21) -- resolved}: that next target has now
    been re-tried (Section~\ref{sec:husmith-current}) with the exact
    same $T_i=10^4\,$K debug flag, after first fixing a matching
    repo-currency bug in \emph{its} committed \code{run.sh} (box size
    and search radius silently drifted from what the deprecated result
    actually used). Against the Bisbas et al.\ (2015) blend it still
    reads FAIL (median $33.51\%$, essentially unchanged from the
    deprecated $32.4\%$) -- but that turned out to be the wrong
    reference: Hu et al.\ (2017)/Smith et al.\ (2021)'s own published
    ``STARBENCH'' figures for this exact test match Raga-II directly,
    not the full blend. Checked against Raga-II: \textbf{PASS}, median
    $6.89\%$. The "multi-source residual" tracked since the deprecated
    result was never a real physics gap -- it was a wrong-reference-curve
    artifact. Both validation targets are now PASS.
\end{itemize}
