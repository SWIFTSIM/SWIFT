...
correct thing to fit against the scheme's own steady state, in the regime
Tier 1 actually exercises (finite absorption, i.e. `kappa > 0`,
`lambda < inf`), not the free-streaming (`kappa -> 0`) limit the
`~lambda/(3r)` known-gap note is about.

Background: the scheme's coupled steady state, for a point source
...
the theoretically correct target for validating the scheme's own P1
...
assert expanded.has(decaying) or expanded.has(
    growing
...
for lam_val in lam_samples:
    ...
    assert (
        limit_growing == sp.oo
    ), f"growing branch unexpectedly bounded at lam={lam_val}"
...
print(
    f"  max |residual| over the sweep: {max_abs_residual:.3e} (exactly 0 symbolically)"
)
...
print(
    "  residual =", residual_transparent, " (nonzero => different model, as expected)"
)
assert (
    residual_transparent != 0
), "transparent-medium field should NOT solve the screened ODE"
...
assert (
    sp.simplify(ratio_of_x - 3 * x * sp.exp(-x)) == 0
), "ratio does not match the design doc's own quoted 3*r/lambda*exp(-r/lambda)"
print(
    "Confirmed: matches the design doc's own quoted ratio, 3*r/lambda*exp(-r/lambda)."
)
print()
...
print()
print(
    f"Small-x (r<<lambda) limit of ratio/x: {small_x_limit}  "
...
assert (
    ratios[0] > 1.0
), "expected P1 to exceed the transparent-field value at r/lambda=1"
print()
...
same machinery verify_isrf_timestepping_stability.py Part C already
...
question. the scheme's P1-closed system has an EXACT steady-state solution,
...
against the EXACT analytic solution of the equations the scheme actually
