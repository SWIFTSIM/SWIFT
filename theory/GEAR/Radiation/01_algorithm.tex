...
eff}$ from $L$ and $R$ via Stefan--Boltzmann instead. Not yet checked
whether the two are numerically close across the relevant mass range,
or which one (if either) is authoritative where they disagree -- flagged
as item~T19, Section~\ref{sec:open}, not investigated this session.
...
the still-unimplemented adaptive rebuild cadence's recombination
timescale, item~T11, Section~\ref{sec:open}) assumes one free electron
...
to this model inherits the same approximation; see item~T10,
...
$Z/Z_\odot\approx0.231$ validation-matrix reference point (item~V8,
Section~\ref{sec:open}), chosen specifically because $\log_{10}$ makes
...
u_{\rm ionized}$). Every result in this logbook computed at
$Z/Z_\odot\approx0.231$ (the whole validation matrix,
Sections~\ref{sec:matrix-gas20}--\ref{sec:matrix-gas95}, T8's e10 check)
was therefore run at the wrong reference temperature; see item~T12,
Section~\ref{sec:open}.
