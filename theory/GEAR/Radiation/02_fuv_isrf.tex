...
closure, is not the same as for that placement: at the shipped
$(\alpha_{\max}, C_{\rm hyp}) = (0.5, 0.5)$ it is $1.54$ against $1.35$,
and $1.19$--$1.33$ times larger at the extreme rows of the scan. It is
bounded (the spectral radius stays at or below one) and vanishes with
the dissipation off. The dissipation conserves $\sum_i m_i u_i$
...
\begin{itemize}
  ...
  % Revised 2026-09-15: replaces "Inherited coefficients", re-derived for
  % the flux-first order with e-weighted dissipation.
  \item \textbf{Dissipation coefficients.} The amplification matrix of
    ...
    lattice maximum of the dissipation symbol is $3.31$, which puts the
    linear boundary at about twice the bound, where the stability
    boundary measured on the previous order also lies. The transient
