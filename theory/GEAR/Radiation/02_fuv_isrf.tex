...
with its own self-shielding prescription \citep{DraineBertoldi1996},
...
cross-sections per hydrogen nucleon in the two bands for a Milky-Way
grain population \citep{WeingartnerDraine2001},
% TODO(cite): Kim et al. (2023), the band-integrated tabulation of these
% two dust cross-sections per hydrogen nucleon; details unverified.
...
\begin{enumerate}
  ...
  \item \textbf{The band-edge Doppler shift}, $\Delta_{\rm edge}$ and
    ...
    \citep{DraineBertoldi1996}, to the rate this module supplies
...
(including H$_2$ nuclei). By default
(\code{photoelectric\_heating\_grackle\_option: 2}) the grain
photoelectric efficiency is constant, $A = 10^{-24}\,{\rm erg\,s^{-1}}$
and $\epsilon_{\rm PE} = 0.05$. Option 4 (a Grackle build that provides
it) uses the density-only fit of \citet[their Eqs.~A1--A2]{Smith2026} to
the solar-neighbourhood predictions of \citet{Wolfire2003},
\begin{equation}
...
Relative to the constant efficiency, option 4 heats by a factor $0.23$,
$0.39$, $0.67$, $1.14$ and $1.82$ at $n_H = 0.1$, $1$, $10$, $100$ and
$\ge 725\,{\rm cm^{-3}}$. Neither option uses the electron density, which
Grackle's own documentation flags as unreliable in dense, cold gas.
Grackle also scales its dust recombination cooling and
H$_2$ formation on dust by the same $G_0$. The H$_2$ photodissociation
...
self-shielding factor \citep{DraineBertoldi1996} to the sum. Grackle
...
a Grackle build that provides it) adds, following \citet[their
Appendix~A]{Smith2026},
\begin{equation}
...
\end{equation}
per unit volume: $0.4$\,eV from the dissociation itself and the
collisionally thermalised part of the UV-pumping energy, with the
critical density of \citet{Omukai2000} that Grackle already uses for
H$_2$ formation heating. $k_{\rm diss,shield}$ is the shielded total
rate (UV background plus $k_{\rm diss}$). \citet{Smith2026} write the
pumping denominator as $1 + n/n_{\rm crit}$; the form above, which
grows with density, is the one consistent with Grackle's formation
heating that they cite. Both quantities are clamped to be
...
configures Grackle for it: dust chemistry on, which selects
photoelectric heating (with the efficiency option above) and dust recombination cooling
together, and additionally H$_2$ formation on dust grains in the cooling
modes that track H$_2$ (modes 2 and 3); the per-particle ISRF field
...
\hline
\code{photoelectric\_heating\_grackle\_option} & & 2 & 2 or 4 &
  Grackle photoelectric efficiency: 2 constant, 4 density-only
...
\begin{itemize}
  ...
  \item \textbf{Screened steady state}, $h/\lambda$ from $0.6$ to $20$
    ...
    $n_H = 10^3\,{\rm cm^{-3}}$). Criterion: the fitted absorption
    length of the settled profile within $15\%$ of the scheme's own
    discrete fixed point, computed on the run's particle positions, in
    both bands; the amplitude identity Eq.~\ref{eq:fuv-amplitude-identity}
    reported. Outcome: pass at every corner, with amplitude agreement to
    $0.2\%$ or better; the LW profile at $h/\lambda = 20$ is
    unmeasurable because the field is zero on all but a few dozen
    particles.
...
\begin{itemize}
  ...
  \item \textbf{Grackle path.} The photoelectric efficiency is either
    constant or a function of density alone
    (Eq.~\ref{eq:fuv-pe-efficiency-density}); the latter assumes gas on
    the solar-neighbourhood thermal equilibrium curve and overestimates
    the efficiency at low metallicity \citep{Smith2026}. Neither follows
    the grain charging parameter, and the electron-density-dependent
    efficiency Grackle also offers \citep[their Eq.~2]{Wolfire1995} is
    not selectable. The H$_2$ cross-section is an effective
