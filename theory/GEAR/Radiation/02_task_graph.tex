...
\textbf{Open follow-up (item~T9, Section~\ref{sec:open}):} the existing
StromgrenSphere suite runs can only ever test ``does the cap correctly
avoid a spurious crash'' -- they are permanently pinned at the periodic
floor, so they can never exercise the actual feature (\code{h\_hii}
genuinely coarsening cells to preserve coverage at \code{cdim >= 4}). A
dedicated \code{cdim >= 4} configuration (a bigger box, or smaller
\code{gas\_mass} forcing \code{max\_top\_level\_cells} higher) would be
needed to verify the feature does something, not just that it doesn't
crash -- not built yet.
