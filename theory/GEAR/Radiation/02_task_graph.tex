...
of non-uniform setup that could. See item~T1 in the GEAR radiation
logbook's Open Items section.
...
\code{Stars:max\_HII\_search\_radius} (the GEAR radiation logbook's
Validation section)
...
The remaining schematics of the search (traversal structure, multi-leaf
gather with the global nearest-\code{max\_ngbs} buffer, and the unwired
two-regions-away case with its three guards) are kept alongside the
document in \code{figures/} (\code{fig1}, \code{fig2}, \code{fig5}).
