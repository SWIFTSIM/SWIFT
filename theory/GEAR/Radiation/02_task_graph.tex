...
but a search radius that grows dynamically over many of a star's own
HII rebuild passes, so no single tree level is guaranteed to contain
it; and (2) the
...
\begin{itemize}
  \item \textbf{\code{task\_subtype\_stars\_radiation\_in} /
    ...
    done. They \emph{are} split further, down to
    \code{radiation\_level} (Section~\ref{sec:radiation-level} below),
    but not by the generic \code{scheduler\_splittasks} pass that splits
    hydro/gravity: radiation tasks do not exist yet when that pass
    runs (they are created afterward, once the hydro-attach flags
    their split criterion depends on are available), so a separate,
    dedicated pass, \code{scheduler\_splittasks\_radiation}
    (\code{scheduler\_splittasks.c:963}), walks exactly the
    newly-created \code{radiation\_in} tasks and hands each to
    \code{scheduler\_splittask\_radiation\_subgrid}.
...
runs the radiation-specific \emph{tree}-rebuild check,
\code{cell\_need\_rebuild\_for\_radiation\_pair}: this decides whether
\swift's tree needs a full tree rebuild, not whether a star's own HII
rebuild pass is due (Section~\ref{sec:persistence} of the algorithm
document); the ordinary \code{cell\_need\_rebuild\_for\_stars\_pair}
is blind to $h_{\rm hii}$ growth and would under-trigger the tree
rebuild.
...
\subsubsection{The Per-Star Search Loop}
...
\subsubsection{Expansion Probes Can Outrun the Coherent Front}
...
\subsubsection{Gather-Time Pixel-Budget Starvation Guard}
...
\subsubsection{Sort Entries Live on a Global Axis}

A cell's sort entry along
...
\subsubsection{Any Axis Is Conservative}

A 1D projection never exceeds the 3D
...
\subsubsection{\texttt{sid} Selection and Its Degradation Bound}
...
\subsubsection{Reach Stays Inside the Wiring}

Sorting narrows the scan within
...
deferred outer shell is picked up after the next tree rebuild
re-levels the tasks.
...
\subsubsection{The Two-Regions-Away Gap and Its Three Guards}
...
\begin{figure}
  ...
  \caption{The two-regions-away gap: with \texttt{radiation\_level} at
    ...
    the gap; a tree rebuild coarsens the stencil so C eventually enters it
    (before/after insets).}
...
grid is sized once per tree rebuild by \code{space\_regrid()}
...
\subsubsection{Why This Needs a Cap, Not an Unconditional Fold}
...
\subsubsection{Fix: Cap \texttt{h\_hii}'s Own Contribution to \texttt{h\_max}}
...
\subsubsection{Effect on \texttt{cdim}}

At the periodic floor
...
Two further knobs are set at \code{./configure} time, not in the
...
Separately, any run requesting \code{GEARFeedback:HII\_angular\_nside
> 0} (i.e.\ angular splitting at all) must be configured with
\code{--with-chealpix}: the angular pixel assignment itself goes
through the HEALPix C API, and without it the code errors at start-up
rather than silently falling back to the spherical ($\text{nside}=0$)
case (\code{radiation.c:924}).
