...
\textbf{Stability: a real constraint, but the mild kind, not the
$c$-dependent kind, and not automatically enforced by precedent.} This
is still an explicit, time-marched update, so it inherits the ordinary
diffusion stability bound ($\alpha\lesssim$ some $O(1)$ limit tied to
the kernel/neighbour count, from Eq.~\ref{eq:fuv-discrete-correspondence}'s
$D\sim\alpha h^2/\Delta t$ -- the standard $\Delta
t\lesssim h^2/D$ explicit-diffusion condition), \emph{not} the much
harsher $\text{CFL}\sim h/c$ that ruled out M1 in the first place --
consistent with the original design goal (Section~\ref{sec:fuv-propagation}).
But it is not unconditionally stable either, and
\code{GEAR\_DIFFUSION}'s own precedent is a cautionary one: its
diffusion coefficient \code{C} (\code{GEARChemistry:diffusion\_coefficient})
has no enforced stability check at all --
\code{chemistry\_timestep()} unconditionally returns \code{FLT\_MAX}
despite doing real diffusion, leaving stability entirely up to the
user picking a small enough \code{C}. This scheme should not repeat
that silently-unchecked pattern; an explicit stability bound on
$\alpha$ given $h$, $\Delta t$, and the local neighbour count should be
added, not left implicit.

\textbf{Not yet done / open questions, this pass}:
\begin{itemize}
  \item Working out the actual stability bound on $\alpha$ for a given
    kernel/neighbour configuration, and deciding whether to enforce it
    via a timestep constraint (unlike \code{GEAR\_DIFFUSION}'s
    precedent) -- not scoped;
  \item whether a single explicit relaxation step per hydro step
    tracks the intended quasi-static equilibrium closely enough, or
    whether the field lags behind fast-changing sources (e.g.\ a star
    turning on) enough to need sub-cycling -- not checked;
