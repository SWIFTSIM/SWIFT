...
for this part; it is mentioned only in Section~\ref{sec:hyperbolic-relation}
to clarify that the parabolic equation solved here is recovered as its
relaxation-time-to-zero limit. The hyperbolic solver's own derivation,
characteristic speed, and exact solutions are covered in
Part~\ref{part:hyperbolic}.
...
\subsection{Hyperbolic Relation (TODO: Move to hyperbolic chapter)}
...
\begin{equation}
  \dt = \CCFL\, \frac{\dx}{v_{\max}} \,.
\end{equation}
As $\tau\to0$ the hyperbolic system's
flux equation collapses onto Eq.~\eqref{eq:ficks_law}, i.e.\ the
parabolic equation documented in this part is the relaxation limit of
...
hyperbolic solver's own physics, its relaxation-time model
(Section~\ref{sec:tau-modes}) and the blending factor between its HLL
flux and the parabolic flux of Eq.~\eqref{eq:flux_hll}
(Section~\ref{sec:effective-diffusivity}), is covered in
Part~\ref{part:hyperbolic} and not repeated here.
