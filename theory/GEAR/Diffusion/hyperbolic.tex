...
The remaining lever, capping $F_{\rm diss}$, is what the next two
subsubsections do, from two different angles: a causal bound that stops
the dissipation from outrunning $\chyp$ regardless of how far apart the
pair sits (Section~\ref{sec:causal-bound}), and a Hopkins-style limiter
that catches a second, physically distinct failure mode, a stale flux
that keeps transporting metal after the gradient that produced it has
moved on (Section~\ref{sec:hyperbolic-limiter}).
...
\begin{remark}[The causal bound is not the flux-memory limiter's replacement]
\label{rem:causal-vs-flux-memory}
Unlike its relationship to the minmod bound above, the causal bound is
\emph{not} a substitute for the flux-memory limiter of
Section~\ref{sec:hyperbolic-limiter}, and dropping the latter is not a
safe simplification. The two rescale the same $F_{\rm diss}$ but answer
different questions, and each is blind to the other's failure mode:
\begin{itemize}
\item $\zeta_{ij}$ asks \emph{how fast} the dissipation is, relative to
  the one speed the Cattaneo system allows it to be, $\chyp$. It is built
  purely from geometry and $\chyp$ and knows nothing about the actual
  value of $\Fdiff$; it cannot tell a freshly-relaxed flux from a stale
  one, both get the same $\zeta_{ij}$ if they sit at the same pair with
  the same $\chyp$.
\item The flux-memory limiter of Section~\ref{sec:hyperbolic-limiter}
  asks \emph{how stale} the dynamical flux $\Fdiff$ is, i.e.\ how far it
  still is from its own Fickian relaxation target
  $\Fdiff_{\rm parabolic}$. It is built purely from $\Fdiff$ and
  $\Fdiff_{\rm parabolic}$ and knows nothing about $\chyp$ or $r_{ij}$; it
  cannot tell a causally-bounded dissipation rate from an excessive one.
\end{itemize}
An un-relaxed flux transporting at an otherwise-causal rate ($\zeta_{ij}$
close to $1$) is exactly the case the causal bound cannot see and the
flux-memory limiter exists to catch, and vice versa. This is why both
mechanisms are applied to $F_{\rm diss}$ in the code
(\code{hyperbolic/chemistry\_riemann\_HLL.h}), one after the other, rather
than either superseding the other the way the causal bound superseded the
minmod bound above.
\end{remark}
...
\subsubsection{A Hopkins-2017-Style Limiter for the Hyperbolic Branch}
