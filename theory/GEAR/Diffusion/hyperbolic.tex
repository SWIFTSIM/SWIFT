...
$\kappaC$ is retuned (e.g.\ when moving to a different problem). A physically meaningful
...
\subsection{The Berthon Blend with the Parabolic Solver}
...
(\code{chemistry\_riemann\_compute\_hyperbolic\_blending\_factor}), defined by
\begin{equation}
  \label{eq:berthon_alpha}
  \alpha = \frac{\Rstiff}{0.1+\Rstiff} ,
  \qquad \Rstiff = \frac{\taurelax_\star}{\taurelax_{\rm num}} ,
\end{equation}
in terms of a dimensionless \emph{stiffness ratio} $\Rstiff$: the pair's
relaxation time $\taurelax_\star$ (the harmonic mean of the two
particles' own $\taurelax$, equal to $\taurelax/2$ when
$\taurelax_L=\taurelax_R=\taurelax$) against the time
$\taurelax_{\rm num}=r_{ij}^2/D_\star$ metal would take to cross the pair
separation $r_{ij}=|\mathbf x_i-\mathbf x_j|$ by ordinary (parabolic)
diffusion alone. $\Rstiff\to0$ (relaxation much faster than that local
crossing time, the near-equilibrium regime) sends $\alpha\to0$;
$\Rstiff\to\infty$ (relaxation much slower, the wave-dominated regime)
sends $\alpha\to1$. How $\Rstiff$ behaves under the code's actual
$\taurelax_0$ normalisation is worked out in
Section~\ref{sec:practical-tau0}. Only the mass-density
...
\subsection{Controlling the Dissipation Term}
\label{sec:controlling-dissipation}

\subsubsection{Consequence of an Overestimated Wave Speed}
\label{sec:hll-consequence}

\textbf{The problem.} $\chyp$ is not just a timestep bound: it is also
the wave speed the Riemann solver uses to build $F_{\rm diss}$
(Eq.~\eqref{eq:hyp_F_transport}), and $F_{\rm diss}$ is exactly what the
Berthon blend passes through essentially unmixed once $\alpha\to1$ (the
large-$\taurelax$, wave-dominated regime of
Section~\ref{sec:berthon-blend}). So any looseness in $\chyp$ does not
just make the timestep more conservative than necessary; it becomes
\emph{extra numerical diffusion}, precisely in the regime where the
scheme relies most on the raw HLL flux, unmoderated by the parabolic
branch.
...
the diffusion-dominated regime this document is about) makes the
mechanism explicit:
\begin{equation}
  F_{\rm diss} = \frac{\lambda_+\lambda_-\,(U_R-U_L)}{\lambda_+-\lambda_-}
  = \frac{-\Lambda^2(U_R-U_L)}{2\Lambda} = -\frac{\Lambda}{2}(U_R-U_L) ,
\end{equation}
i.e.\ $F_{\rm diss}$ is \emph{linear} in the wave-speed estimate
$\Lambda$: doubling $\Lambda$ doubles the numerical dissipation, with no
saturation built in.

\textbf{The two independent levers, and which one is left to pull.}
There are exactly two ways to control this: tighten $\chyp$ itself, or
cap $F_{\rm diss}$ once assembled, the way Part~\ref{part:parabolic}'s
Hopkins-HLL solver caps its own dissipation (Section~\ref{sec:hll-flux}).
The first lever is already exhausted:
$\chyp=\sqrt{\lambda_1(\Deff)/\taurelax}$ is exact for isotropic $\Deff$
and a tight, safe bound otherwise (Section~\ref{sec:confrontation}), so
there is no slack left to squeeze out of the wave-speed estimate itself.
The remaining lever, capping $F_{\rm diss}$, is a causal bound that stops
the dissipation from outrunning $\chyp$ regardless of how far apart the
pair sits (Section~\ref{sec:causal-bound}); it is the only rescaling this
solver applies to $F_{\rm diss}$, for reasons
Section~\ref{sec:hyperbolic-limiter} sets out.

\subsubsection{A Causal Bound on the Dissipation}
...
Equation~\eqref{eq:causal-bound} is the hyperbolic solver's replacement
for the parabolic-style minmod TVD bound the hyperbolic branch used to
carry: a two-argument minmod between $(1+\psi)F_{\rm transport}$
...
The two mechanisms were not complementary: they rescaled the same term in
pursuit of the same goal, suppressing the raw HLL dissipation's tendency
to over-transport, so applying both to the hyperbolic branch was
redundant, and at a tight $\psi$ the minmod bound first and left
...
\item \textbf{Length scale.} The chemistry CFL condition ties $\dt$ to the
  ...
  $\chyp\dt_{ij}/(\text{length scale})$ is $O(\CCFL)$ for essentially every
  ...
  dissipation's reach, applied on (almost) every face every step. The
  implementation uses $\min(\dx_i,\dx_j)$, the smaller of the two
  particles' own physical sizes
  (\code{chemistry\_get\_physical\_particle\_size}), in place of $r_{ij}$
  in Eq.~\eqref{eq:causal-bound}, because the von Neumann/Lax-Wendroff
  stability floor this term is meant to respect, and the $\dt$ the CFL
  condition actually delivers, are both built from the particle size, not
  from $r_{ij}$ (a distinct, though comparable, length scale;
  Section~\ref{sec:practical-tau0}). For a close neighbour pair inside the
  kernel, $r_{ij}$ can be smaller than the particle size: using it in
  Eq.~\eqref{eq:causal-bound} would let $\zeta_{ij}$ read as less
  restrictive than the stability analysis the CFL condition is actually
  based on supports, for exactly the pairs where that matters most.
...
\item \textbf{Interaction with \code{hyperbolic\_limiter\_scope}.} The
  ...
  the mass-density dissipation) are actually rescaled follows the
  \code{hyperbolic\_limiter\_scope} choice: suppressing the
  flux-component dissipation unconditionally would preserve, rather than
  decay, an already-stale flux.
\end{itemize}

\subsubsection{Choosing $\taurelax_0$ in Practice}
...
speed threshold also fixes the stiffness ratio $\Rstiff$ and hence the
blending factor $\alpha$ of Eq.~\eqref{eq:berthon_alpha}
(Section~\ref{sec:berthon-blend}), which decides whether a
...
($\alpha\to1$). Recall $\taurelax_\star$ is the harmonic mean of the
pair's two relaxation times (equal to $\taurelax/2$ when
$\taurelax_L=\taurelax_R=\taurelax$), and
...
\begin{equation}
  \Rstiff = \taurelax_0\,\frac{\sqrt3\,\gammak^2h^2}{2\,r_{ij}^2} ,
\end{equation}
i.e.\ $\Rstiff$ is \emph{exactly} proportional to $\taurelax_0$, with a
...
$\Rstiff=0.1$, $\alpha\approx0.91$ at $\Rstiff=1$,
$\alpha\approx0.98$ at $\Rstiff=4$. Since the speed-parity threshold
...
are the same knob, pointing in opposite directions. The causal bound of
Section~\ref{sec:causal-bound} softens this tension somewhat;
Section~\ref{sec:hyperbolic-limiter} explains why no second, independent
lever is added on top of it.

\subsubsection{Why $F_{\rm diss}$ Carries No Flux-Memory Limiter}
\label{sec:hyperbolic-limiter}

The causal bound of Section~\ref{sec:causal-bound} is the only rescaling
this solver applies to $F_{\rm diss}$. This section explains why a second
mechanism, built by comparing the dynamical flux $\Fdiff$ against its own
Fickian relaxation target, is not layered on top of it: the obvious port
of the parabolic solver's own limiter is redundant with the Berthon blend
$\alpha$ (Section~\ref{sec:limiter-naive-port} and
Remark~\ref{rem:limiter-redundant}), and a purpose-built flux-memory
signal fares no better (Section~\ref{sec:limiter-flux-memory}).
...
$\Rstiff=\taurelax_\star/\taurelax_{\rm num}$
(Section~\ref{sec:berthon-blend}) is $\propto\taurelax^{+1}$: an exact
monotonic power-law relationship, $r\propto\Rstiff^{-1/2}$. The swapped $r$ therefore
...
\subsubsection{Why a Flux-Memory Signal Fares No Better}
...
$\taurelax$ (Eq.~\eqref{eq:cattaneo}), and the two disagreeing,
$\Fdiff\neq\Fdiff_{\rm parabolic}$, is not by itself a defect: at large
$\taurelax$ it is exactly the correct wave physics the hyperbolic branch
exists to capture. A signal built from the instantaneous residual between
$\Fdiff$ and $\Fdiff_{\rm parabolic}$ cannot tell that case apart from the
one failure mode such a comparison would actually be useful for, an
un-relaxed flux that keeps transporting metal after the local gradient
that produced it has moved on, since both produce the same large
disagreement between $\Fdiff$ and its target. A limiter built on this
residual therefore suppresses the stabilising dissipation hardest
precisely in the large-$\taurelax$ wave regime where $\Fdiff\neq\Fdiff_{\rm
parabolic}$ is correct, not where it is stale.

The signal that Remark~\ref{rem:limiter-redundant} shows is missing from
the naive port, one that depends on the actual local state rather than
only on scheme parameters, does not have to be built from the flux
residual at all. The natural alternative, tracking the relaxation regime
directly through $\taurelax$, is what the Berthon blend $\alpha$
(Eq.~\eqref{eq:berthon_alpha}, Section~\ref{sec:berthon-blend}) already
does for every pair, so it adds no distinction $\alpha$ does not already
make. Between a residual-based signal that cannot separate correct
wave-lag from staleness and a $\taurelax$-based one that duplicates
$\alpha$, neither clears the bar Remark~\ref{rem:limiter-redundant} sets:
the causal bound of Section~\ref{sec:causal-bound} is therefore the sole
rescaling this solver applies to $F_{\rm diss}$.
...
Both closed forms below come from the same two-transform route: a spatial
Fourier transform turns Eq.~\eqref{eq:telegrapher} (isotropic scalar
$\Deff=\kappa\mathbf I$) into an ordinary differential equation in $t$ for
each wavenumber, and a Laplace transform in $t$ then solves that equation
algebraically. Because the diffusive term contributes only through the
Laplacian eigenvalue $-k^2$, the transformed equation does not know its
spatial dimension; the 1D and 3D solutions are two different real-space
inversions of the \emph{same} transformed object, not two independent
calculations.

\begin{proposition}[Transform solution]
\label{prop:transform-solution}
Let $\hat U_0(\mathbf k,t)=\int U_0(\mathbf x,t)\,e^{-i\mathbf k\cdot\mathbf
x}\,\dd^d\mathbf x$ be the spatial Fourier transform of the
isotropic-scalar solution of Eq.~\eqref{eq:telegrapher} with the initial
data of this section, and let
$\tilde U_0(\mathbf k,p)=\int_0^\infty\hat U_0(\mathbf k,t)\,e^{-pt}\,\dd t$
be its Laplace transform in time. Then, independently of the spatial
dimension $d$,
\begin{equation}
  \label{eq:transform-solution}
  \tilde U_0(\mathbf k,p) = \frac{q_0(\taurelax p+1)}{\taurelax p^2+p+\kappa k^2} ,
  \qquad k\equiv|\mathbf k| .
\end{equation}
\end{proposition}

\begin{proof}
The Fourier transform replaces $\grad$ by $i\mathbf k$ termwise, so
$\div(\Deff\grad U_0)\to-\kappa k^2\hat U_0$ for any $d$ (only $|\mathbf
k|^2$ survives), turning Eq.~\eqref{eq:telegrapher} into
\begin{equation}
  \taurelax\,\ddot{\hat U}_0 + \dot{\hat U}_0 = -\kappa k^2\hat U_0 ,
  \qquad \hat U_0(\mathbf k,0)=q_0, \quad \dot{\hat U}_0(\mathbf k,0)=0 ,
\end{equation}
the initial data following from $\int q_0\delta^d(\mathbf
x)e^{-i\mathbf k\cdot\mathbf x}\dd^d\mathbf x=q_0$ and, via
Eq.~\eqref{eq:mass_conservation}, $\dot U_0(\mathbf x,0)=-\div\Fdiff(\mathbf
x,0)=0$. Laplace transforming in $t$,
\begin{equation}
  \taurelax\big[p^2\tilde U_0-pq_0\big] + \big[p\tilde U_0-q_0\big] = -\kappa k^2\tilde U_0 ,
\end{equation}
and collecting $\tilde U_0$ gives $\tilde U_0(\taurelax
p^2+p+\kappa k^2)=q_0(\taurelax p+1)$, which is
Eq.~\eqref{eq:transform-solution}.
\end{proof}

\subsubsection{One Dimension}
\label{sec:exact-1d}

\begin{proposition}[1D exact Green's function]
\label{prop:exact-1d}
In 1D the solution of Eq.~\eqref{eq:transform-solution} has two pieces: a
ballistic front carrying a finite fraction of the mass at $|x|=\chyp t$,
and a smooth interior behind it \cite{MasoliverPorraWeiss1993} (restated in
...
\begin{proof}
At $d=1$, Eq.~\eqref{eq:transform-solution} is a Lorentzian in $k$,
\begin{equation}
  \tilde U_0(k,p) = \frac{q_0(\taurelax p+1)}{\kappa}\cdot\frac{1}{k^2+\lambda(p)^2} ,
  \qquad
  \lambda(p) \equiv \sqrt{\frac{p(\taurelax p+1)}{\kappa}} = \frac{1}{\chyp}\sqrt{(p+a)^2-a^2} ,
\end{equation}
using $\kappa=\chyp^2\taurelax$ and $a=1/(2\taurelax)$; $\lambda(p)$ is the
Laplace-domain field's spatial decay rate, real and finite for $p>0$ since
$\Deff\propto\kappa>0$ damps every wavenumber. Its inverse Fourier
transform is elementary,
$\tfrac1{2\pi}\int_{-\infty}^\infty\tfrac{e^{ikx}}{k^2+\lambda^2}\dd
k=\tfrac{e^{-\lambda|x|}}{2\lambda}$, giving the Laplace-domain Green's
function
\begin{equation}
  \label{eq:U1-laplace}
  \tilde U_0(x,p) = \frac{q_0(\taurelax p+1)}{2\kappa\,\lambda(p)}\,e^{-\lambda(p)|x|} .
\end{equation}
Eq.~\eqref{eq:U1-laplace} is the Laplace transform of the persistent
random walk (telegraph process) propagator; inverting it in $t$ is a
classical result \cite{MasoliverPorraWeiss1993} (restated in
\cite{PruestelMeierSchellersheim2013}, Eqs.~(2.1)--(2.5)) and reproduces
Eq.~\eqref{eq:exact-green} term by term: the leading $e^{-\lambda(p)|x|}$
singularity as $p\to\infty$ inverts to the ballistic-front delta at
$|x|=\chyp t$, and the regular remainder inverts to the modified-Bessel
interior.
\end{proof}
...
The 3D case is \emph{not} obtained by treating Eq.~\eqref{eq:exact-green}
as a 3D Gaussian-type solution in disguise; both the front and the
interior differ. It is, however, obtained from
Eq.~\eqref{eq:exact-green} exactly, through a dimension-reduction identity
that holds precisely because Eq.~\eqref{eq:transform-solution} depends on
$\mathbf k$ only through $k=|\mathbf k|$: the transformed 1D and 3D
problems share the same right-hand side, so their real-space solutions
are two different inversions of one function.

\begin{proposition}[Dimension reduction]
\label{prop:dimension-reduction}
For $r>0$, the 3D radial Laplace-domain Green's function is related to the
1D one, Eq.~\eqref{eq:U1-laplace}, by
\begin{equation}
  \label{eq:dim-reduction}
  \tilde U_0^{(3)}(r,p) = -\frac{1}{2\pi r}\,\partial_r\tilde U_0^{(1)}(r,p) .
\end{equation}
\end{proposition}

\begin{proof}
The 3D inverse Fourier transform of a function of $\mathbf k$ depending on
$\mathbf k$ only through $k=|\mathbf k|$ is, in radial coordinates,
$\tfrac1{2\pi^2r}\int_0^\infty(\cdot)\,k\sin(kr)\,\dd k$. Applying this
directly to $\tilde U_0(k,p)$ itself, the Laplace-domain solution of
Proposition~\ref{prop:transform-solution} (the same function of $k$ used
for the 1D case, since it does not depend on $d$), the standard integral
$\int_0^\infty\tfrac{k\sin(kr)}{k^2+\lambda^2}\dd k=\tfrac\pi2e^{-\lambda
r}$ gives
\begin{equation}
  \tilde U_0^{(3)}(r,p) = \frac{q_0(\taurelax p+1)}{4\pi\kappa r}\,e^{-\lambda(p) r} .
\end{equation}
Comparing with Eq.~\eqref{eq:U1-laplace} and using $\partial_r
e^{-\lambda r}=-\lambda e^{-\lambda r}$ reproduces
Eq.~\eqref{eq:dim-reduction} directly. Both sides are elementary
exponentials at this stage, so the identity is an ordinary algebraic fact
about $\tilde U_0^{(1)}$ and $\tilde U_0^{(3)}$ for every $p$ and every
$r>0$, not yet a distributional statement.
\end{proof}

\begin{corollary}[3D smooth interior]
\label{cor:3d-smooth}
For $r<\chyp t$,
\begin{equation}
  \label{eq:U3d-smooth}
...
\begin{proof}
Write the interior of Eq.~\eqref{eq:exact-green} as $q_0e^{-at}G(r,t)$,
$G(r,t)=\tfrac{a}{2\chyp}\big[I_0(as)+tI_1(as)/s\big]$, and recall
$\partial_rs=-r/(\chyp^2s)$. Since $r<\chyp t$ is away from the front,
Laplace inversion commutes with $\partial_r$ there, so
Eq.~\eqref{eq:dim-reduction} applies termwise to $G$. Using $I_0'=I_1$ and
the recurrence $I_1'(z)=I_0(z)-I_1(z)/z$,
\begin{equation}
  \partial_rG = \frac{a}{2\chyp}\,\partial_rs\left[aI_1(as)
    + t\,\partial_s\!\left(\frac{I_1(as)}{s}\right)\right]
  = -\frac{ar}{2\chyp^3s}\left[aI_1(as)
    + \frac{t\big(as\,I_0(as)-2I_1(as)\big)}{s^2}\right] ,
\end{equation}
where $\partial_s\big(I_1(as)/s\big)=\big(as\,I_1'(as)-I_1(as)\big)/s^2
=\big(as\,I_0(as)-2I_1(as)\big)/s^2$. Substituting into
Eq.~\eqref{eq:dim-reduction}, the explicit factor of $r$ in $\partial_rG$
cancels the $1/r$ prefactor, leaving Eq.~\eqref{eq:U3d-smooth}.
\end{proof}

\begin{remark}[The 3D front carries three distributions, not one]
\label{rem:3d-front-dipole}
Eq.~\eqref{eq:dim-reduction} extends to the front once the derivative is
read distributionally. Write the full 1D solution, front included, as
$U_0^{(1)}(r,t)=q_0e^{-at}\big[\tfrac12\delta(r-\chyp
t)+\Theta(\chyp t-r)G(r,t)\big]$ for $r>0$ ($G$ as in the proof of
Corollary~\ref{cor:3d-smooth}). Differentiating uses
$\partial_r\Theta(\chyp t-r)=-\delta(r-\chyp t)$ and, for the $r$-weighting
carried by the $-1/2\pi r$ prefactor of Eq.~\eqref{eq:dim-reduction}, the
distributional product rule
$f(r)\delta'(r-r_0)=f(r_0)\delta'(r-r_0)-f'(r_0)\delta(r-r_0)$ applied to
$f(r)=r$:
\begin{equation}
  \partial_rU_0^{(1)} = q_0e^{-at}\Big[\tfrac12\delta'(r-\chyp t)
    - G(\chyp t,t)\,\delta(r-\chyp t) + \Theta(\chyp t-r)\,\partial_rG\Big] ,
\end{equation}
using, for the $r$-weighting,
\begin{equation}
  r\,\delta'(r-\chyp t) = \chyp t\,\delta'(r-\chyp t) - \delta(r-\chyp t) ,
\end{equation}
with $G(\chyp t,t)=\lim_{s\to0}G(r,t)=\tfrac{a}{2\chyp}\big(1+\tfrac12at\big)$
(from $I_0(0)=1$ and $I_1(z)/z\to\tfrac12$ as $z\to0$). Three distinct
distributional contributions therefore appear at the front: the
$\delta'(r-\chyp t)$ from differentiating the front delta itself, a
$\delta(r-\chyp t)$ picked up from the $r$-weighting via the product rule
above, and a further $\delta(r-\chyp t)$ from the $\partial_r\Theta$
boundary term. Collected into a radial mass density (per unit $r$, i.e.\
$4\pi r^2$ times the front part of $-\tfrac1{2\pi r}\partial_rU_0^{(1)}$),
\begin{equation}
...
\end{equation}
using $2\chyp t\,G(\chyp t,t)=at\big(1+\tfrac12at\big)$: a signed dipole
layer plus a monopole shell. Mass conservation by itself does not fix this
split: any pair $c\,\delta'(r-\chyp t)+\delta(r-\chyp t)$ integrates to the
same total regardless of $c$, since a dipole layer carries no net mass.
What fixes both coefficients is the differentiation above, applied to the
actual density-spike solution rather than to an auxiliary problem; as an
independent check, Section~\ref{sec:exact-comparison} below shows the same
pair also closes the exact second-moment budget of
Eq.~\eqref{eq:moment-identity}. The dipole carries no net mass, so the 3D
front mass fraction is
\begin{equation}
...
Collecting the interior and the front into a single field, parallel in
structure to the 1D result Eq.~\eqref{eq:exact-green}, gives the complete
3D Green's function.

\begin{proposition}[3D exact Green's function]
\label{prop:exact-3d}
For $r>0$,
\begin{equation}
  \label{eq:exact-green-3d}
  \begin{split}
    U_0(r,t) = q_0e^{-at}\Bigg[\;
      &\underbrace{-\frac{\delta'(r-\chyp t)}{4\pi\chyp t}
        +\frac{at+\tfrac12(at)^2-1}{4\pi(\chyp t)^2}\,\delta(r-\chyp t)}_{\text{front}} \\
      +\;&\underbrace{\Theta(\chyp t-r)\,\frac{a}{4\pi\chyp^3}\left[
        \frac{t\big(as\,I_0(as)-2I_1(as)\big)}{s^3}+\frac{a\,I_1(as)}{s}\right]}_{\text{smooth interior}}
    \Bigg] .
  \end{split}
\end{equation}
\end{proposition}
...
\begin{proof}
The interior term is Corollary~\ref{cor:3d-smooth}. For the front,
$\mu_{\rm front}=4\pi r^2\,U_0^{\rm front}$ by construction
(Remark~\ref{rem:3d-front-dipole}), so $U_0^{\rm
front}(r,t)=f(r)\,\mu_{\rm front}(r,t)$ with $f(r)\equiv1/(4\pi r^2)$.
Applying the distributional product rule
$f(r)\delta'(r-r_0)=f(r_0)\delta'(r-r_0)-f'(r_0)\delta(r-r_0)$ to
Eq.~\eqref{eq:front-measure-3d}'s dipole term, with $f(\chyp
t)=1/(4\pi(\chyp t)^2)$ and $f'(\chyp t)=-1/(2\pi(\chyp t)^3)$, and
$f(r)\delta(r-\chyp t)=f(\chyp t)\delta(r-\chyp t)$ for its monopole term,
gives
\begin{equation}
  \begin{split}
    U_0^{\rm front}(r,t) = q_0e^{-at}\Bigg[\;
      &-\chyp t\left(\frac{\delta'(r-\chyp t)}{4\pi(\chyp t)^2}
        +\frac{\delta(r-\chyp t)}{2\pi(\chyp t)^3}\right) \\
      +\;&\Big(1+at+\tfrac12(at)^2\Big)\frac{\delta(r-\chyp t)}{4\pi(\chyp t)^2}
    \Bigg] ,
  \end{split}
\end{equation}
which collects to the front term of Eq.~\eqref{eq:exact-green-3d}.
\end{proof}

Because of the $\delta'$, Eq.~\eqref{eq:exact-green-3d} should be
evaluated from this closed form directly rather than by numerically
inverting the Laplace-domain solutions, Eq.~\eqref{eq:U1-laplace} or its
3D counterpart: Bromwich-type quadrature rings at the front, and the
closed form is both exact and cheaper.
...
scheme remains open: the HLL flux and the causal bound of
Section~\ref{sec:causal-bound} (the sole rescaling applied to
$F_{\rm diss}$, Section~\ref{sec:limiter-flux-memory}) act together in a
way not closed in closed form here, exactly as Part~\ref{part:parabolic},
