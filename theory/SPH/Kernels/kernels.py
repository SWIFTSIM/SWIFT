...
# Show h
plt.plot([h, h], [0.0, 0.63 * maxY], "k:", linewidth=0.5)
plt.text(
    h,
    maxY * 0.65,
...
# Show sigma
plt.plot([h / 2, h / 2], [0.0, 0.63 * maxY], "k:", linewidth=0.5)
plt.text(
    h / 2,
    maxY * 0.65,
    "$\\sigma\\equiv h/2$",
...
# Show <x>
plt.plot([dx, dx], [0.0, 0.63 * maxY], "k:", linewidth=0.5)
plt.text(
    dx,
    maxY * 0.65,
...
plt.plot(xx, Gaussian(xx, h), "k-", linewidth=0.7, label="${\\rm Gaussian}$")
plt.plot(xx, W_cubic_spline(xx), "b-", label="${\\rm Cubic~spline}$", lw=1.5)
plt.plot(xx, W_quartic_spline(xx), "c-", label="${\\rm Quartic~spline}$", lw=1.5)
plt.plot(xx, W_quintic_spline(xx), "g-", label="${\\rm Quintic~spline}$", lw=1.5)
plt.plot(xx, W_WendlandC2(xx), "r-", label="${\\rm Wendland~C2}$", lw=1.5)
plt.plot(xx, W_WendlandC4(xx), "m-", label="${\\rm Wendland~C4}$", lw=1.5)
plt.plot(xx, W_WendlandC6(xx), "y-", label="${\\rm Wendland~C6}$", lw=1.5)

# Show h
plt.plot([h, h], [0.0, 1.0], "k:", linewidth=0.5)

# Show sigma
plt.plot([h / 2, h / 2], [0.0, 1.0], "k:", linewidth=0.5)

# Show <x>
plt.plot([dx, dx], [0.0, 1.0], "k:", linewidth=0.5)
...
plt.xlim(0.0, 2.5 * h)
plt.ylim(1e-5, 0.7)
plt.xlabel("$r$", labelpad=-1)
plt.ylabel("$W(r,h)$", labelpad=0.5)
...
# def d2_cubic_spline(r):   return np.where(r > 1., 0., np.where(r < 0.5,
#                                                          18.*r - 6.,
#                                                          -6.*r + 6.) )

# def d2_quartic_spline(r): return np.where(r > 1., 0., np.where(r < 0.2,
#                                                          72.*r**2 - 4.8,
#                                                          np.where(r < 0.6,
#                                                                -48.*r**2 + 48.*r  - (48./5.),
#                                                                12.*r**2 - 24.*r + 12.)))

# def d2_quintic_spline(r): return np.where(r > 1., 0., np.where(r < 1./3.,
#                                                          -200.*r**3 + 120.*r**2 - (40./9.),
#                                                          np.where(r < 2./3.,
#                                                                100.*r**3 - 180.*r**2 + 100.*r - (140./9.),
#                                                                -20.*r**3 + 60.*r**2 - 60.*r + 20)))
# def d2_wendlandC2(r):     return np.where(r > 1., 0., 80.*r**3 - 180.*r**2 + 120.*r - 20.)
# def d2_wendlandC4(r):     return np.where(r > 1., 0., 653.3333*r**6 - 2688.*r**5 + 4200.*r**4 - 2986.667*r**3 + 840.*r**2 - 18.6667)
# def d2_wendlandC6(r):     return np.where(r > 1., 0., 3520.*r**9 - 20790.*r**8 + 50688.*r**7 - 64680.*r**6 + 44352.*r**5 - 13860.*r**4 + 792.*r**2 - 22)
...
plt.plot([0, 2.5 * h], [0.0, 0.0], "k--", linewidth=0.7)
plt.plot(xx, d_Gaussian(xx, h), "k-", linewidth=0.7, label="${\\rm Gaussian}$")
plt.plot(xx, dWdx_cubic_spline(xx), "b-", label="${\\rm Cubic~spline}$", lw=1.5)
plt.plot(xx, dWdx_quartic_spline(xx), "c-", label="${\\rm Quartic~spline}$", lw=1.5)
plt.plot(xx, dWdx_quintic_spline(xx), "g-", label="${\\rm Quintic~spline}$", lw=1.5)
plt.plot(xx, dWdx_WendlandC2(xx), "r-", label="${\\rm Wendland~C2}$", lw=1.5)
plt.plot(xx, dWdx_WendlandC4(xx), "m-", label="${\\rm Wendland~C4}$", lw=1.5)
plt.plot(xx, dWdx_WendlandC6(xx), "y-", label="${\\rm Wendland~C6}$", lw=1.5)

maxY = d_Gaussian(h / 2, h)

# Show h
plt.plot([h, h], [2 * maxY, 0.1], "k:", linewidth=0.5)

# Show sigma
plt.plot([h / 2, h / 2], [2 * maxY, 0.1], "k:", linewidth=0.5)

# Show <x>
plt.plot([dx, dx], [2 * maxY, 0.1], "k:", linewidth=0.5)


plt.xlim(0.0, 2.5 * h)
plt.gca().xaxis.set_ticklabels([])
plt.ylim(1.3 * maxY, -0.1 * maxY)
plt.xlabel("$r$", labelpad=0)
plt.ylabel("$\\partial W(r,h)/\\partial r$", labelpad=0.5)
plt.legend(
...
plt.subplot(212)
plt.plot([h / 2, h / 2], [0.77 * maxY, -0.15 * maxY], "k:", linewidth=0.5)
plt.plot([h, h], [0.77 * maxY, -0.15 * maxY], "k:", linewidth=0.5)
plt.plot([dx, dx], [0.77 * maxY, -0.15 * maxY], "k:", linewidth=0.5)
plt.text(
    h / 2,
    1.25 * maxY,
    "$\\sigma\\equiv h/2$",
...
plt.plot(xx, dWdh_cubic_spline(xx), "b-", label="${\\rm Cubic~spline}$", lw=1.5)
plt.plot(xx, dWdh_quartic_spline(xx), "c-", label="${\\rm Quartic~spline}$", lw=1.5)
plt.plot(xx, dWdh_quintic_spline(xx), "g-", label="${\\rm Quintic~spline}$", lw=1.5)
plt.plot(xx, dWdh_WendlandC2(xx), "r-", label="${\\rm Wendland~C2}$", lw=1.5)
plt.plot(xx, dWdh_WendlandC4(xx), "m-", label="${\\rm Wendland~C4}$", lw=1.5)
plt.plot(xx, dWdh_WendlandC6(xx), "y-", label="${\\rm Wendland~C6}$", lw=1.5)

plt.xlim(0.0, 2.5 * h)
plt.ylim(1.3 * maxY, -0.15 * maxY)
plt.xlabel("$r$", labelpad=-1)
plt.ylabel("$\\partial W(r,h)/\\partial h$", labelpad=0.5)


plt.savefig("kernel_derivatives.pdf")
