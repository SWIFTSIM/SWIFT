...
where ``pos`` and ``mass`` are arrays of dimention resp. 3xn and 1xn and with the same units than the one given to AGAMA.
  
An example of a potential AGAMA file corresponding to a spherical multipole expansion of a Plummer sphere is
given below::
...
Configuring SWIFT in order to link it with AGAMA is obtained with::
