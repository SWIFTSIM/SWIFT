...
For star creation, the displacement update per particle is handled by ``spart_add_displacement(spart, delta_pos)``. For the cells, the machinery is implemented in ``runner_do_star_formation_sink()`` and ``runner_do_star_formation()`` in ``runner_others.c`` and must be called after creating a new particle. Then, the cell's displacement needs to be propagated to the cell hierarchy.
