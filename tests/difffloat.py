...
for i in range(n_lines_to_check):
    for j in range(n_columns):
        ...
        if abs_diff > 1.1 * absTol[j]:
            print(
                "Absolute difference larger than tolerance (%e) for particle %d, column %s:"
                % (absTol[j], data1[i, 0], part_props[j])
            )
        ...
        if abs(data1[i, j]) + abs(data2[i, j]) < limTol[j]:
            continue

        if rel_diff > 1.1 * relTol[j]:
            print(
                "Relative difference larger than tolerance (%e) for particle %d, column %s:"
                % (relTol[j], data1[i, 0], part_props[j])
            )
            print("%10s:           a = %e" % ("File 1", data1[i, j]))
            print("%10s:           b = %e" % ("File 2", data2[i, j]))
