...
static void set_part(struct part *p, float kappa, float dt, float c_hyp) {
  ...
  fd->dt_prev = dt;
  fd->c_hyp = c_hyp;
  fd->kappa[ISRF_BAND_FUV] = kappa;
  fd->kappa[ISRF_BAND_LW] = kappa;
  fd->rho_prev = 1.f;
  fd->u_prev[ISRF_BAND_FUV] = u_FUV_0;
  fd->u_prev[ISRF_BAND_LW] = u_LW_0;
  fd->u[ISRF_BAND_FUV] = u_FUV_0;
  fd->u[ISRF_BAND_LW] = u_LW_0;
  fd->ngb_mean_abs_u_V[ISRF_BAND_FUV] = 1.f;
  fd->ngb_mean_abs_u_V[ISRF_BAND_LW] = 1.f;
  for (int k = 0; k < 3; k++) {
    fd->specific_flux[ISRF_BAND_FUV][k] = F_FUV_0[k];
    fd->specific_flux[ISRF_BAND_LW][k] = F_LW_0[k];
  }
}
...
static void run_case(double a, double H, float kappa, float dt, float c_hyp) {
  ...
  check_close("u_FUV", expected_decay * u_FUV_0,
              p.feedback_data.u[ISRF_BAND_FUV], 1e-5f);
  check_close("u_LW", expected_decay * u_LW_0, p.feedback_data.u[ISRF_BAND_LW],
              1e-5f);
  for (int k = 0; k < 3; k++) {
    check_close("specific_flux_FUV", expected_decay * F_FUV_0[k],
                p.feedback_data.specific_flux[ISRF_BAND_FUV][k], 1e-5f);
    check_close("specific_flux_LW", expected_decay * F_LW_0[k],
                p.feedback_data.specific_flux[ISRF_BAND_LW][k], 1e-5f);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (p.feedback_data.u[ISRF_BAND_FUV] != u_FUV_0 ||
      p.feedback_data.u[ISRF_BAND_LW] != u_LW_0)
    error("H = 0 is not a no-op on u: %.9e vs %.9e",
          p.feedback_data.u[ISRF_BAND_FUV], u_FUV_0);
  for (int k = 0; k < 3; k++)
    if (p.feedback_data.specific_flux[ISRF_BAND_FUV][k] != F_FUV_0[k] ||
        p.feedback_data.specific_flux[ISRF_BAND_LW][k] != F_LW_0[k])
