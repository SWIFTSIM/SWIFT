...
void test_special_flags(struct csds_writer *log) {
  ...
  csds_log_part(log, &p, &xp, &e, /* log_all */ 1, csds_flag_create,
                special_flag_value);
...
void test_gas_particle_fields(struct csds_writer *log) {
  ...
  printf(
      ...
      xp.csds_data.last_offset, p.x[0], p.x[1], p.x[2], p.v[0], p.v[1], p.v[2],
  ...
  printf(
      "  pos = [%f, %f, %f], vel = [%f, %f, %f], mass = %f, u = %f, h = %f, "
      "rho = %f, id = %lld\n",
      p.x[0], p.x[1], p.x[2], p.v[0], p.v[1], p.v[2], p.mass, p.u, p.h, p.rho,
      p.id);

  // Check values
  if (p.x[0] != 1.0 || p.x[1] != 1.1 || p.x[2] != 1.2 || p.v[0] != 0.1f ||
      p.v[1] != 0.2f || p.v[2] != 0.3f || p.mass != 2.0f || p.u != 0.5f ||
      p.h != 1.1f || p.rho != 3.3f || p.id != 42) {
